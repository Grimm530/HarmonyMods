 ...
 * log tag changed to [Cooking].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace CookingHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[Cooking] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[Cooking] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[Cooking] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[Cooking] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[Cooking] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[Cooking] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[Cooking] Permissions Service bind: " + ex.Message);
            }
        }
