...
namespace AirbourneSpawnHarmony
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[AirbourneSpawn] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[AirbourneSpawn] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[AirbourneSpawn] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[AirbourneSpawn] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[AirbourneSpawn] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[AirbourneSpawn] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[AirbourneSpawn] Permissions Service bind: " + ex.Message);
            }
        }
