 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to UberToolHarmony,
 * log tag changed to [UberTool].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace UberToolHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[UberTool] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[UberTool] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[UberTool] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[UberTool] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[UberTool] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[UberTool] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[UberTool] Permissions Service bind: " + ex.Message);
            }
        }
