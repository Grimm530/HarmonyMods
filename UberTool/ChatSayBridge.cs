...
{
    /// <summary>
    /// Shared chat.say command dispatcher for Harmony mods.
    /// Multiple mods patch ConVar.Chat.say; the first prefix that returns false skips the rest.
    /// Register handlers here and call <see cref="Dispatch"/> from every Chat.say prefix so
    /// /s (Shop) and /st (SkillTree) both work regardless of load order.
    /// Handler list is stored in AppDomain as List&lt;object&gt; so it is shared across mod assemblies.
    /// </summary>
    public static class ChatSayBridge
    {
        ...
        public static void Register(string id, Func<BasePlayer, string, bool> handler)
        {
            ...
            lock (list)
            {
                ...
                // object[] { id, handler } — neutral across assemblies
        ...
        /// <summary>Try every registered handler. Returns true if any consumed the message.</summary>
