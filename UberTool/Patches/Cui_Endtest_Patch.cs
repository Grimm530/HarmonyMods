...
namespace UberToolHarmony.Patches
{
    ...
    public static class Cui_Endtest_Patch
    {
        ...
        public static bool Prefix(ConsoleSystem.Arg args)
        {
            ...
            if (!string.Equals(marker, UberToolMod.CuiMarker, StringComparison.OrdinalIgnoreCase))
                return true;
            var mod = UberToolMod.Instance;
            if (mod == null) return true;
            try { mod.HandleCuiEndtest(args, a); }
            catch (Exception ex) { Debug.LogWarning("[UberTool] cui.endtest UBERTOOL: " + ex); }
            return false;
        }
    }
}
