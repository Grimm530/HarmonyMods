...
namespace ShopHarmony
{
    ...
    public class Shop : ShopPluginBase
    {
        ...
        private readonly HashSet<ulong> _closingShop = new();
        ...
        // Batch reporting data structures
        private readonly Dictionary<ulong, List<ShopTransaction>> _playerSales = new();
        private readonly Dictionary<ulong, List<ShopTransaction>> _playerPurchases = new();
        private Timer _batchReportTimer;

        // Wipe tracking
        private DateTime _wipeStartTime = DateTime.MinValue;
        private DateTime _wipeEndTime = DateTime.MinValue;
        ...
        private class Configuration
        {
            ...
            [JsonProperty(PropertyName =
                LangRu ? "Webhook Discord для отправки покупок/продаж" : "Discord webhook for logging sell/buy")]
            public string DiscordWeebhook = string.Empty;

            [JsonProperty(PropertyName = LangRu ? "Включить пакетную отправку в Discord?" : "Enable batch Discord reporting?")]
            public bool EnableBatchDiscord = false;

            [JsonProperty(PropertyName = LangRu ? "Использовать внешние иконки предметов в Discord?" : "Use external item icons in Discord?")]
            public bool UseExternalItemIcons = false;

            [JsonProperty(PropertyName = LangRu ? "Интервал отправки пакетных отчетов (часы)" : "Batch report interval (hours)")]
            public int BatchReportInterval = 12;
        ...
        public class SellContainers
        {
            ...
            public Item[] AllItems(BasePlayer player)
            {
                ...
                foreach (var cont in containers)
                    totalCount += cont.itemList.Count;
            ...
            public int AllItems(BasePlayer player, List<Item> items)
            {
                ...
                foreach (var itemContainer in GetContainers(player)) items.AddRange(itemContainer.itemList);
        ...
        public class SelectCurrencyUI
        {
            ...
            [JsonProperty(PropertyName = "Title")] public TextSettings Title;
        ...
        private class ShopCategory : ICloneable, IDisposable
        {
            ...
            public List<ShopItem> GetShopItems(BasePlayer player)
            {
                ...
                switch (CategoryType)
                {
                    case Type.Favorite:
                    {
                        if (_instance?.GetPlayerCart(player.userID) is not PlayerCartData playerCart)
                            return new List<ShopItem>();

                        var items = playerCart.GetFavoriteItems(player.userID);
                    ...
                    default:
                    {
                        ...
                        {
                            case Configuration.SortType.None:
                                return Items.FindAll(item => item.IsAvailableForPlayer(player.userID));

                            default:
                                if (_sortedItems == null) SortItems();

                                return _sortedItems?.FindAll(item => item.IsAvailableForPlayer(player.userID));
                        }
                    }
                }
            }
        ...
        public class ShopItem
        {
			...
			[JsonProperty(PropertyName = LangRu ? "Обход проверки владения" : "Bypass ownership check")]
			public bool BypassOwnershipCheck;
            ...
            public CuiElement GetImage(InterfacePosition position, string parent,
                string name = null)
            {
                ICuiComponent targetImage;
                
                // Check if custom image URL is provided
                if (!string.IsNullOrEmpty(Image))
                {
                    // Use RawImage for custom URLs or image names
                    var rawImage = new CuiRawImageComponent();
                    
                    // If it's a URL, check if local file exists first
                    if (Image.IsURL())
                    {
                        // For Dropbox URLs, check if local file exists
                        if (Image.Contains("dropbox.com"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(localPath))
                            {
                                ...
                                if (File.Exists(localFilePath))
                                {
                                    ...
                                    // This ensures we use the cached version
                                    rawImage.Png = _instance.GetImage(Image);
                                    // Trigger load if not already loaded
                                    _instance.AddImage(Image, Image);
                                }
                                else
                                {
                                    // No local file, use URL directly
                                    rawImage.Url = _instance.ConvertDropboxUrl(Image);
                                    // Also trigger background loading for caching
                                    _instance.AddImage(Image, Image);
                                }
                            }
                            else
                            {
                                // Couldn't determine local path, use URL
                                rawImage.Url = _instance.ConvertDropboxUrl(Image);
                                _instance.AddImage(Image, Image);
                            }
                        }
                        ...
                        {
                            // Not a Dropbox URL, use URL directly
                            rawImage.Url = _instance.ConvertDropboxUrl(Image);
                            // Also trigger background loading for caching
                            _instance.AddImage(Image, Image);
                        }
                ...
                {
                    // Fall back to item icon using ItemId and SkinId
                    targetImage = _image ?? (_image = new CuiImageComponent
                    {
                        ItemId = itemId,
                        SkinId = Skin
                    });
            ...
            {
                var selectedEconomy = _instance.API_GetShopPlayerSelectedEconomy(userID);
            ...
            public Dictionary<string, object> ToDictionary()
            {
                return new Dictionary<string, object>
                {
                    ...
                    ["Skin"] = Skin,
                    ["Blueprint"] = Blueprint,
					["BypassOwnershipCheck"] = BypassOwnershipCheck,
                    ["Buying"] = CanBuy,
                    ["Selling"] = CanSell,
                    ["Amount"] = Amount,
                    ["Price"] = Price,
            ...
            public ShopItem(Dictionary<string, object> dictionary)
            {
				...
				BypassOwnershipCheck = dictionary.ContainsKey("BypassOwnershipCheck") && (bool) dictionary["BypassOwnershipCheck"];
        ...
        private class EconomyEntry
        {
            ...
            /// <summary>
            /// True if this economy uses whole units only (e.g. RP). Sell payouts should be floored so 20 scrap = 1 RP, not 10 scrap = 1 RP.
            /// </summary>
            internal bool UsesIntegerCurrency()
            {
                switch (Type)
                {
                    case EconomyType.Plugin:
                        return Plug is "BankSystem" or "ServerRewards" or "IQEconomic";
                    case EconomyType.Item:
                        return true;
                    default:
                        return false;
                ...
                switch (Type)
                {
                    case EconomyType.Plugin:
                    {
                        ...
                        switch (Plug)
                        {
                            ...
                            case "ServerRewards":
                            case "IQEconomic":
                                plugin.Call(AddHook, player.userID.Get(), (int) Math.Floor(amount));
                                break;
                            default:
        ...
        private bool _canSaveConfig = false;
        ...
        private void UpdateConfigValues()
        {
            ...
            if (_config.Version != default)
            {
                ...
                if (_config.Version < new VersionNumber(2, 0, 0))
                {
                    ...
                    if (LangRu)
                    {
                        _config.UI.Color1.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 1", "Непрозрачность (0 - 100)"));
                        _config.UI.Color2.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 2", "Непрозрачность (0 - 100)"));
                        _config.UI.Color3.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 3", "Непрозрачность (0 - 100)"));
                        _config.UI.Color5.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 5", "Непрозрачность (0 - 100)"));
                        _config.UI.Color7.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 7", "Непрозрачность (0 - 100)"));
                        _config.UI.Color8.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 8", "Непрозрачность (0 - 100)"));
                        _config.UI.Color9.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 9", "Непрозрачность (0 - 100)"));
                        _config.UI.Color10.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 10", "Непрозрачность (0 - 100)"));
                        _config.UI.Color11.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 11", "Непрозрачность (0 - 100)"));
                        _config.UI.Color12.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 12", "Непрозрачность (0 - 100)"));
                        _config.UI.Color13.Alpha =
                            Convert.ToSingle(Config.Get("Интерфейс", "Цвет 13", "Непрозрачность (0 - 100)"));
                    }
                }
        ...
        private class PlayerData
        {
            ...
            {
                if (_instance?._usersData == null) return;
                var keys = new List<ulong>(_instance._usersData.Keys);
        ...
        private class CartData
        {
            ...
            {
                var sum = 0;
                foreach (var kvp in GetShopItems())
                    sum += kvp.Key.Amount * kvp.Value;
                return sum;
            }
            ...
            public double GetCartPrice(BasePlayer player, bool again = false)
            {
                var selectedEconomy = _instance.API_GetShopPlayerSelectedEconomy(player.userID);
                double sum = 0;
                foreach (var kvp in GetShopItems(again))
                    sum += kvp.Key.GetPrice(player, selectedEconomy) * kvp.Value;
                return sum;
            }
            ...
            public int GetCartItemsStacksAmount()
            {
                ...
                foreach (var kvp in GetShopItems())
                {
                    if (kvp.Key.Type == ItemType.Item && kvp.Key.ItemDefinition != null)
                    {
                        sum += kvp.Key.ProhibitSplit ? 1 : kvp.Key.GetStacks(kvp.Value).Count;
                    }
                }
                return sum;
            }
        ...
        private class PlayerCartData : CartData
        {
            ...
            public List<ShopItem> GetFavoriteItems(ulong userID)
            {
                ...
                foreach (var itemID in FavoriteItems)
                {
                    ...
                    if (shopItem != null && shopItem.IsAvailableForPlayer(userID))
                        list.Add(shopItem);
                }
        ...
        private void RegisterImagesFromUI(ref Dictionary<string, string> imagesList)
        {
            ...
            if (_uiData.IsFullscreenUISet) _uiData.FullscreenUI?.RegisterImages(ref imagesList);

            if (_uiData.IsInMenuUISet) _uiData.InMenuUI?.RegisterImages(ref imagesList);
        }
        ...
        public class ShopUI
        {
            ...
            public class ShopItemUI
            {
                ...
                [JsonProperty(PropertyName = "Info")] public ButtonSettings Info;
                ...
                public class AmountUI
                {
                    ...
                    [JsonProperty(PropertyName = "Value")] public TextSettings Value;
                }
                ...
                public class ActionButtonUI
                {
                    ...
                    [JsonProperty(PropertyName = "Price")] public TextSettings Price;
            ...
            public class ShopActionModalUI
            {
                    ...
                    string cmdClose = "")
                {
                    ...
                    {
                        container.Add(
                            DescriptionTitle.GetText(_instance?.Msg(player, UIShopActionDescriptionTitle),
                                ModalLayer + ".Main"));
                        container.Add(
                            ItemDescription.GetText(_instance?.Msg(player, item.Description), ModalLayer + ".Main"));
                    }

                    container.Add(AmountTitle.GetText(_instance?.Msg(player, UIShopActionAmountTitle),
                        ModalLayer + ".Main"));
                    ...
                    container.Add(Price.GetText(GetPlayerEconomy(player).GetPriceTitle(player, itemPrice),
                        ModalLayer + ".Main"));

                    container.AddRange(ButtonAction.GetButton(btnActionTitle, cmdAction, ModalLayer + ".Main",
                        close: ModalLayer));
            ...
            public class ShopConfirmationModalUI
            {
                    ...
                    (string title, string cmd) cancel)
                {
                    ...
                    container.AddRange(ButtonConfirm.GetButton(confirm.title, confirm.cmd, ModalLayer,
                        close: ModalLayer));
            ...
            public class ShopItemDescriptionModalUI
            {
                    ...
                    string cmdClose = "")
                {
                    ...
                    container.Add(Title.GetText(_instance?.Msg(player, UIShopItemDescriptionTitle),
                        ModalLayer + ".Main"));

                    container.Add(Description.GetText(_instance?.Msg(player, item.Description), ModalLayer + ".Main"));

                    container.AddRange(ButtonClose.GetButton(string.Empty, cmdClose, ModalLayer + ".Main",
                        close: ModalLayer));
                }
        ...
        {
            _instance = this;
            ShopExtensionMethods.ExtensionMethods.perm = ShopHost.Instance?.Permission;
        ...
        internal void OnServerInitialized()
        {
            ...
            // Initialize batch reporting timer if enabled
            if (_config.EnableBatchDiscord && !string.IsNullOrEmpty(_config.DiscordWeebhook))
            {
                var intervalSeconds = _config.BatchReportInterval * 3600; // Convert hours to seconds
                _batchReportTimer = timer.Every(intervalSeconds, () =>
                {
                    SendBatchedReport();
                    ClearBatchData();
                });
                PrintWarning($"Shop batch Discord reporting enabled - reports every {_config.BatchReportInterval} hours");
            }


            if (PlayerDLCAPI == null || !PlayerDLCAPI.IsLoaded)
                PrintWarning($"Strongly recommended to install PlayerDLCAPI(https://umod.org/plugins/player-dlc-api) for check the acces to DLCs and paid skins for players, otherwise the server may be blacklisted by Facepunch");
        }
        ...
        {
            // Record wipe end time
            _wipeEndTime = DateTime.UtcNow;
            
            // Send final wipe summary report before clearing data
            if (_config.EnableBatchDiscord && !string.IsNullOrEmpty(_config.DiscordWeebhook))
            {
                SendWipeSummaryReport();
            }

            // Clear batch data on wipe
            ClearBatchData();
            ...
            // Reset wipe start time - will be auto-detected from first sale
            _wipeStartTime = DateTime.MinValue;

            PrintWarning("Shop wipe detected! Final report sent and data cleared. Wipe start time will be auto-detected from first sale.");
        }
        ...
        internal object CanLootEntity(BasePlayer player, VendingMachine vendingMachine)
        {
        ...
        internal void OpenShopUI(ConsoleSystem.Arg arg)
        {
        ...
        internal void CmdShopOpen(IPlayer cov, string command, string[] args)
        {
        ...
        internal void CmdSetCustomVM(IPlayer cov, string command, string[] args)
        {
            var player = cov?.Object as BasePlayer;
            if (player == null) return;
        ...
        internal void CmdShopItem(ConsoleSystem.Arg arg)
        {
            ...
            if (!arg.HasArgs() || cmdArgs.Length < 2)
            {
                defaultInfo();
                return;
            }
        ...
        internal void CmdSetShopNPC(IPlayer cov, string command, string[] args)
        {
            var player = cov?.Object as BasePlayer;
            if (player == null) return;
        ...
        internal void CmdConsoleShop(ConsoleSystem.Arg arg)
        {
            var cmdArgs = arg.Args == null ? Array.Empty<string>() : Array.ConvertAll(arg.Args, value => value.ToString());
            var player = arg?.Player();
            ...
            switch (arg.GetString(0))
            {
                ...
                case "shop_search_input":
                {
                    ...
                    if (arg.HasArgs(2)) search = string.Join(" ", cmdArgs.Skip(1));

                    var shop = GetShop(player);
                    shop.OnChangeSearch(search, 0);

                    UpdateUI(player, container => { ShopContentUI(player, container); });
                    break;
                }
                ...
                {
                    if (!arg.HasArgs(3) ||
                        !int.TryParse(cmdArgs[1], out var categoryPage) ||
                        !int.TryParse(cmdArgs[2], out var targetShopPage)) return;

                    var search = string.Empty;
                    if (arg.HasArgs(4)) search = string.Join(" ", cmdArgs.Skip(3));
                ...
                {
                    if (!arg.HasArgs(2) ||
                        !int.TryParse(cmdArgs[1], out var id)) return;
                ...
                {
                    if (!arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var targetCategoriesPage)) return;

                    var shop = GetShop(player);
                    shop?.Update();
                    shop?.OnChangeCategoriesPage(targetCategoriesPage);

                    UpdateUI(player, container => { ShopCategoriesUI(container, player); });
                    break;
                }

                case "categories_change":
                {
                    if (!arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var targetCategoryIndex)) return;
                ...
                case "cart_page":
                {
                    if (!arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var page)) return;
                ...
                {
                    if (!arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var id)) return;
                ...
                case "cart_item_change":
                {
                    if (!arg.HasArgs(3) ||
                        ...
                        !int.TryParse(cmdArgs[2], out var itemID) ||
                        !int.TryParse(cmdArgs[3], out var amount)) return;
                ...
                {
                    if (!arg.HasArgs(3) ||
                        !int.TryParse(cmdArgs[1], out var itemId) ||
                        !int.TryParse(cmdArgs[2], out var amount) ||
                        amount <= 0) return;

                    var item = FindItemById(itemId);
                    if (item == null || !item.CanBuy || !(item.ForceBuy || !GetShop(player).GetUI().ShopBasket.UseShopBasket)) return;
                ...
                case "try_buy_item":
                {
                    if (!arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var itemId)) return;
                        ...
                        if (arg.GetString(2) == "all")
                            amount = Mathf.FloorToInt((float) (GetPlayerEconomy(player).ShowBalance(player) /
                                                               item.GetPrice(player,
                                                                   API_GetShopPlayerSelectedEconomy(player.userID))));
                        else
                            amount = arg.GetInt(2);
                ...
                case "try_sell_item":
                {
                    if (!arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var itemId))
                        return;
                ...
                case "sellitem":
                {
                    ...
                    try
                    {
                        ...
                        if (ItemCount(playerItems, item) >= totalAmount)
                        {
                            ...
                            // Floor so fractional payouts never round up (e.g. 10 scrap = 0.5 → 0). Don't call economy until payout is at least 1.
                            sellPrice = Math.Floor(sellPrice);

                            if (sellPrice < 1)
                            {
                                ErrorUi(player, Msg(player, SellMinimumPayout));
                                return;
                            }
                ...
                case "startedititem":
                {
                    if (!IsAdmin(player) || !arg.HasArgs(2) ||
                        !int.TryParse(cmdArgs[1], out var itemID)) return;
                ...
                case "edititem":
                {
                    ...
                    if (_itemEditing.ContainsKey(player.userID) && _itemEditing[player.userID].ContainsKey(key))
                    {
                        ...
                        switch (key)
                        {
                            ...
                            case "DisplayName":
                            {
                                newValue = string.Join(" ", cmdArgs.Skip(2));
                                break;
                            }
                ...
                case "saveitem":
                {
                    ...
                    else
                    {
                        ...
                        if (shopItem != null)
                        {
                            shopItem.ShortName = newItem.ShortName;
							...
							shopItem.BypassOwnershipCheck = newItem.BypassOwnershipCheck;
                ...
                case "selectitem":
                {
                    if (!IsAdmin(player) || !arg.HasArgs(2) || !int.TryParse(cmdArgs[1], out var category)) return;
                    ...
                    var page = 0;
                    if (arg.HasArgs(4))
                        int.TryParse(cmdArgs[3], out page);

                    var input = string.Empty;
                    if (arg.HasArgs(5))
                        input = string.Join(" ", cmdArgs.Skip(4));
                ...
                case "takeitem":
                {
                    if (!IsAdmin(player) || !arg.HasArgs(4) || !int.TryParse(cmdArgs[1], out var category) ||
                        !int.TryParse(cmdArgs[2], out var page)) return;

                    _itemEditing[player.userID]["ShortName"] = string.Join(" ", cmdArgs.Skip(3));
                ...
                {
                    if (!arg.HasArgs(2) || !ulong.TryParse(cmdArgs[1], out var targetId)) return;
                ...
                {
                    if (!arg.HasArgs(3) ||
                        !ulong.TryParse(cmdArgs[1], out var targetId) ||
                        !float.TryParse(cmdArgs[2], out var amount) ||
                        amount <= 0) return;
                    ...
                    var formattedAmount = selectedEconomy.GetPriceTitle(player, amount.ToString(_config.Formatting.BuyPriceFormat));

                    _config?.Notifications?.ShowNotify(player, SuccessfulTransfer, 0,
                        formattedAmount,
                        targetPlayer.displayName);

                    LogTransfer(player.displayName, player.UserIDString, formattedAmount, targetPlayer.displayName, targetPlayer.UserIDString);
                    break;
                }
                ...
                {
                    if (!arg.HasArgs(2) ||
                        !bool.TryParse(cmdArgs[1], out var selected))
                        return;

                    UpdateUI(player, container => { ShowChoiceEconomyUI(player, container, selected); });
                    break;
                }
                ...
                {
                    if (!arg.HasArgs(2) ||
                        !int.TryParse(cmdArgs[1], out var economyID))
                        return;
                ...
                {
                    if (!IsAdmin(player) || !arg.HasArgs(2)
                                         || !int.TryParse(cmdArgs[1], out var categoryID)) return;
                ...
                case "edit_category_localization":
                {
                    ...
                    var fieldName = cmdArgs[1];
                    var fieldValue = cmdArgs[2];
                    ...
                    switch (fieldName)
                    {
                        ...
                        case "Messages":
                        {
                            ...
                            var paramValue = cmdArgs[4];

                            var hashCode = Convert.ToInt32(cmdArgs[2]);
                            ...
                            switch (cmdArgs[3])
                            {
                                ...
                                case "value":
                                {
                                    if (!string.IsNullOrEmpty(paramValue))
                                        paramValue = string.Join(" ", cmdArgs.Skip(4));
                ...
                case "edit_category_field":
                {
                    ...
                    switch (field.FieldType.Name)
                    {
                        case "String":
                        {
                            resultValue = string.Join(" ", cmdArgs.Skip(2));
                            break;
                        }
                        case "Int32":
                        {
                ...
                case "save_edit_category":
                {
                    ...
                    if (generated)
                    {
                        ...
                        {
                            Mathf.FloorToInt((float) categories.Count /
                ...
                case "refresh_shop_item":
                {
                    ...
                    UpdateUI(player, container => { ShopItemButtonsUI(player, ref container, item, selectedEconomy); });
                    break;
                }
            }
			...
			Puts($"Main command used with: {string.Join(", ", cmdArgs)}");
#endif
        }
        ...
        internal void CmdChangeItemCategory(ConsoleSystem.Arg arg)
        {
            var cmdArgs = arg.Args == null ? Array.Empty<string>() : Array.ConvertAll(arg.Args, value => value.ToString());
            ...
            var itemId = Convert.ToInt32(cmdArgs[0]);
            var item = FindItemById(itemId);
            if (item == null)
            {
            ...
            var name = string.Join(" ", cmdArgs.Skip(1));
        ...
        internal void CmdConsoleRefill(ConsoleSystem.Arg arg)
        {
            if (!arg.IsAdmin) return;
        ...
        internal void CmdConsoleWipe(ConsoleSystem.Arg arg)
        {
            if (!arg.IsAdmin) return;
        ...
        internal void CmdConsoleReset(ConsoleSystem.Arg arg)
        {
            if (arg.Player() != null || !arg.IsServerside)
            {
                if (IsAdmin(arg.Player()))
        ...
        internal void CmdConsoleRemoveItem(ConsoleSystem.Arg arg)
        {
            ...
            switch (index)
            {
                ...
                case "items":
                {
                    ...
                    var itemId = Convert.ToInt32(cmdArgs[1]);
                    var item = FindItemById(itemId);
                    if (item == null)
                    {
                        SendReply(arg, $"Item \"{itemId}\" not found!");
        ...
        internal void CmdConsoleFillICC(ConsoleSystem.Arg arg)
        {
            var cmdArgs = arg.Args == null ? Array.Empty<string>() : Array.ConvertAll(arg.Args, value => value.ToString());
            if (!arg.IsAdmin) return;
            ...
            bool showGUI = true)
        {
            ...
            #region Scroll Fix Mock Element
        ...
        private void ShopHeaderUI(BasePlayer player, CuiElementContainer container)
        {
            ...
            if (_config.Transfer)
                container.AddRange(shopUI.ShopContent.Header.ButtonTransfer.GetButton(Msg(player, TransferTitle),
                    $"{CMD_Main_Console} transfer_start", Layer + ".Header"));

            if (_economics.Count > 1)
                ShowChoiceEconomyUI(player, container);

            container.Add(shopUI.ShopContent.Header.Balance.Background.GetImage(Layer + ".Header", Layer + ".Balance",
                Layer + ".Balance"));

            container.Add(
                shopUI.ShopContent.Header.Balance.Title.GetText(Msg(player, YourBalance), Layer + ".Balance"));

            BalanceUi(container, player);

            if (shopUI.ShopContent.Header.UseCloseButton)
                container.AddRange(shopUI.ShopContent.Header.ButtonClose.GetButton(Msg(player, CloseButton),
                    $"{CMD_Main_Console} closeui", Layer + ".Header", close: Layer));
        }
            ...
            BasePlayer player)
        {
            ...
            container.Add(
                shopUI.ShopCategories.Header.Background.GetImage(Layer + ".Categories", Layer + ".Categories.Header"));

            container.Add(shopUI.ShopCategories.Header.Title.GetText(Msg(player, CategoriesTitle),
            ...
            container.Add(shopUI.ShopCategories.CategoriesPanel.GetImage(Layer + ".Categories",
            ...
            if (shopUI.ShopCategories.UseScrollCategories)
            {
                ...
                var pageCategoriesCount = (shopUI.ShopCategories.UseScrollCategories
                    ? shop.Categories
            ...
            if (!shopUI.ShopCategories.UseScrollCategories)
            {
                container.AddRange(shopUI.ShopCategories.BackButton.GetButton(Msg(player, BtnBack),
                    shop.currentCategoriesPage != 0
                        ? $"{CMD_Main_Console} categories_change_local_page {shop.currentCategoriesPage - 1}"
                        : string.Empty, Layer + ".Categories.Content"));

                container.AddRange(shopUI.ShopCategories.NextButton.GetButton(Msg(player, BtnNext),
        ...
        private void CategoriesListUI(CuiElementContainer container, BasePlayer player)
        {
            ...
            if (IsAdmin(player))
            {
                ...
                container.Add(shopUI.ShopCategories.CategoryAdminPanel.CheckboxCategoriesDisplay.Background.GetImage(
                    targetLayer, targetLayer + ".Admin.Show.All", targetLayer + ".Admin.Show.All"));

                container.AddRange(
                    shopUI.ShopCategories.CategoryAdminPanel.CheckboxCategoriesDisplay.CheckboxButton.GetButton(
                        _showAllCategories.Contains(player.userID) ? "✔" : string.Empty,
                        $"{CMD_Main_Console} change_show_categories", targetLayer + ".Admin.Show.All",
                        targetLayer + ".Admin.Show.All.Check", targetLayer + ".Admin.Show.All.Check"));

                CreateOutLine(ref container, targetLayer + ".Admin.Show.All.Check",
                ...
                container.Add(shopUI.ShopCategories.CategoryAdminPanel.CheckboxCategoriesDisplay.Title.GetText(
                    Msg(player, UICategoriesAdminShowAllTitle), targetLayer + ".Admin.Show.All.Check"));

                #endregion Check Show All

                #region Add Category

                container.AddRange(shopUI.ShopCategories.CategoryAdminPanel.ButtonAddCategory.GetButton(
                    Msg(player, BtnAddCategory), $"{CMD_Main_Console} start_edit_category -1", targetLayer,
                    targetLayer + ".Admin.Add.Category", targetLayer + ".Admin.Add.Category"));
            ...
            ShopUI shopUI)
        {
            ...
            container.Add(
                ...
                .GetImage(Layer + $".Category.{category.ID}"));

            container.Add(
                (isSelectedCategory
                    ? shopUI.ShopCategories.ShopCategory.SelectedCategory
                    : shopUI.ShopCategories.ShopCategory.Category).Title
                .GetText(title, Layer + $".Category.{category.ID}"));
                ...
                Components =
                {
                    new CuiButtonComponent
                    {
                        ...
                        Command = !isSelectedCategory
                            ? $"{CMD_Main_Console} categories_change {categoryId}"
                            : string.Empty
                    }
                }
            });
            ...
            if (IsAdmin(player))
            {
                ...
                container.Add(shopUI.ShopCategories.ShopCategory.AdminPanel.Background.GetImage(
                    Layer + $".Category.{category.ID}", Layer + $".Category.{category.ID}.Settings",
                    Layer + $".Category.{category.ID}.Settings"));

                container.AddRange(shopUI.ShopCategories.ShopCategory.AdminPanel.ButtonEdit.GetButton(
                    Msg(player, BtnEditCategory), $"{CMD_Main_Console} start_edit_category {category.ID}",
                    Layer + $".Category.{category.ID}.Settings"));

                container.AddRange(shopUI.ShopCategories.ShopCategory.AdminPanel.ButtonMoveUp.GetButton(string.Empty,
                    $"{CMD_Main_Console} edit_category_move {category.ID} up",
                    Layer + $".Category.{category.ID}.Settings"));
                container.AddRange(shopUI.ShopCategories.ShopCategory.AdminPanel.ButtonMoveDown.GetButton(string.Empty,
                    $"{CMD_Main_Console} edit_category_move {category.ID} down",
                    Layer + $".Category.{category.ID}.Settings"));
            }
        }
            ...
            CuiElementContainer container)
        {
            ...
            if (shop.Categories.Count > 0)
            {
                ...
                if (inPageItems.Count > 0)
                {
                    ...
                    {
                        var maxLines = Mathf.CeilToInt((float) shop.categoryItemsCount / shopUI.ShopItem.ItemsOnString);
                        ...
                        container.Add(new CuiElement
                        {
                            ...
                            Components =
                            {
                                new CuiInputFieldComponent
                                {
                                    ...
                                    Font = shopUI.ShopContent.Content.Search.InputField.IsBold
                                        ? "robotocondensed-bold.ttf"
                                        : "robotocondensed-regular.ttf",
                                    ...
                                    ReadOnly = false,
                                    IsPassword = false,
                                    Text = string.IsNullOrEmpty(shop.search)
                    ...
                    if (!shopUI.ShopContent.Content.UseScrollToListItems)
                    {
                        ...
                        var hasPages = isSearch
                            ? shop.categoryItemsCount > 0 * shopTotalItemsAmount
                            : shop.categoryItemsCount > (shop.currentShopPage + 1) * shopTotalItemsAmount ||
                              shop.currentShopPage != 0;
                        if (hasPages)
                        {
                            container.AddRange(shopUI.ShopContent.Content.ButtonBack.GetButton(Msg(player, BackPage),
                                isSearch
                            ...
                            container.AddRange(shopUI.ShopContent.Content.ButtonNext.GetButton(Msg(player, NextPage),
                                isSearch
        ...
        private void ShopBasketUI(CuiElementContainer container, BasePlayer player)
        {
            ...
            container.Add(shopUI.ShopBasket.Header.Background.GetImage(Layer + ".PlayerCart",
                Layer + ".PlayerCart.Header", Layer + ".PlayerCart.Header"));

            container.Add(
                shopUI.ShopBasket.Header.Title.GetText(Msg(player, ShoppingBag), Layer + ".PlayerCart.Header"));

            #region Pages

            if (!shopUI.ShopBasket.Content.UseScrollShoppingBag && hasPages)
            {
                container.AddRange(shopUI.ShopBasket.Header.BackButton.GetButton(Msg(player, BackTitle),
                    shop.currentBasketPage != 0
                        ? $"{CMD_Main_Console} cart_page {shop.currentBasketPage - 1}"
                        : string.Empty, Layer + ".PlayerCart.Header"));

                container.AddRange(shopUI.ShopBasket.Header.NextButton.GetButton(Msg(player, NextTitle),
            ...
            container.Add(shopUI.ShopBasket.Content.Background.GetImage(Layer + ".PlayerCart",
            ...
            var pageShopItems = shopUI.ShopBasket.Content.UseScrollShoppingBag
                ...
                : playerCartData.GetShopItems()
                    .Skip(shop.currentBasketPage * shopUI.ShopCategories.ShopCategory.CategoriesOnPage)
                    .Take(shopUI.ShopCategories.ShopCategory.CategoriesOnPage);
            ...
            container.Add(shopUI.ShopBasket.Footer.Background.GetImage(Layer + ".PlayerCart",
        ...
        private void UpdateShopCartFooterUI(CuiElementContainer container, BasePlayer player, CartData playerCartData)
        {
            ...
            container.AddRange(
                ...
                .GetButton(Msg(player, BuyTitle),
                    shopUI.ShopBasket.ShowConfirmMenu
                        ? $"{CMD_Main_Console} cart_try_buyitems"
                        : $"{CMD_Main_Console} cart_buyitems", Layer + ".PlayerCart.Footer"));

            if (useBuyAgain)
                container.AddRange(shopUI.ShopBasket.Footer.BuyAgainButton.GetButton(string.Empty,
                    $"{CMD_Main_Console} cart_try_buy_again", Layer + ".PlayerCart.Footer",
                    Layer + ".PlayerCart.Footer.BuyAgain"));

            container.Add(shopUI.ShopBasket.Footer.ItemsCountTitle.GetText(Msg(player, UIBasketFooterItemsCountTitle),
                Layer + ".PlayerCart.Footer"));
            container.Add(shopUI.ShopBasket.Footer.ItemsCountValue.GetText(
                Msg(player, UIBasketFooterItemsCountValue, playerCartData.GetCartItemsAmount()),
                Layer + ".PlayerCart.Footer"));

            container.Add(shopUI.ShopBasket.Footer.ItemsCostTitle.GetText(Msg(player, UIBasketFooterItemsCostTitle),
                Layer + ".PlayerCart.Footer"));
            container.Add(shopUI.ShopBasket.Footer.ItemsCostValue.GetText(
                GetPlayerEconomy(player).GetFooterPriceTitle(player,
                    playerCartData.GetCartPrice(player, !(playerCartData.Items.Count > 0))
                        .ToString(_config.Formatting.ShoppingBagCostFormat)), Layer + ".PlayerCart.Footer"));
        }

        private void ShopCartItemsListUI(CuiElementContainer container, BasePlayer player,
            Dictionary<ShopItem, int> pageShopItems,
            bool hasPages)
        {
            var shopUI = GetShop(player).GetUI();
            ...
            foreach (var (item, amount) in pageShopItems)
            {
                container.Add(new CuiPanel
                    {
                        RectTransform =
                        {
                            AnchorMin = "0 1", AnchorMax = "0 1",
                            OffsetMin =
                                $"{shopUI.ShopBasket.BasketItem.LeftIndent} {ySwitch - shopUI.ShopBasket.BasketItem.Height}",
                            OffsetMax =
                                $"{shopUI.ShopBasket.BasketItem.LeftIndent + shopUI.ShopBasket.BasketItem.Width} {ySwitch}"
                        },
                        Image =
                        {
                            Color = "0 0 0 0"
                ...
                if (shopUI.ShopBasket.BasketItem.ShowImageBackground)
                    container.Add(
                        shopUI.ShopBasket.BasketItem.ImageBackground.GetImage(
                            Layer + $".PlayerCart.Item.{i}.Background"));
        ...
        private void ShopCartItemUI(BasePlayer player, CuiElementContainer container, int index, ShopItem item,
            int amount)
        {
            var shopUI = GetShop(player).GetUI();
            ...
            container.Add(shopUI.ShopBasket.BasketItem.Title.GetText(item.GetPublicTitle(player),
                Layer + $".PlayerCart.Item.{index}"));

            container.Add(shopUI.ShopBasket.BasketItem.ItemAmount.GetText(
                Msg(player, AmountTitle, item.Amount * amount), Layer + $".PlayerCart.Item.{index}"));
            ...
            container.AddRange(shopUI.ShopBasket.BasketItem.ButtonMinusAmount.GetButton(Msg(player, MinusTitle),
                $"{CMD_Main_Console} cart_item_change {index} {item.ID} {amount - 1}",
                Layer + $".PlayerCart.Item.{index}"));
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiInputFieldComponent
                    {
                        ...
                        Font = shopUI.ShopBasket.BasketItem.InputAmount.IsBold
                            ? "robotocondensed-bold.ttf"
                            : "robotocondensed-regular.ttf",
            ...
            int amount = 1)
        {
            ...
            targetModalTemplate.GetModal(player, container, item, amount,
                buy
                    ...
                    : Math.Floor(item.GetSellPrice(player, selectedEconomy) * amount).ToString(_config.Formatting
                        .SellPriceFormat),
                buy
                    ? Msg(player, BuyTitle)
            ...
            int selectedEconomy)
        {
            ...
            else
            {
                ...
                container.Add(buttonTemplate.Price.GetText(shopItem.SellPrice <= 0.0
                        ? Msg(player, ItemPriceFree)
                        : GetPlayerEconomy(player).GetPriceTitle(player,
                            Math.Floor(shopItem.GetSellPrice(player, selectedEconomy))
        ...
        private void ShopItemButtonsUI(BasePlayer player, ref CuiElementContainer container, ShopItem shopItem,
            int selectedEconomy)
        {
            ...
            CuiElementContainer container)
        {
            ...
            container.Add(shopUI.ShopItem.Title.GetText(shopItem.GetPublicTitle(player) + (CanAccesToItem(player, shopItem) ? "" : " <color=red>*</color>"),
                Layer + $".Item.{shopItem.ID}"));
            ...
            if (IsAdmin(player))
            {
                container.Add(shopUI.ShopItem.AdminPanel.Background.GetImage(Layer + $".Item.{shopItem.ID}",
                    Layer + $".Item.{shopItem.ID}.Settings", Layer + $".Item.{shopItem.ID}.Settings"));

                container.AddRange(shopUI.ShopItem.AdminPanel.ButtonEdit.GetButton(Msg(player, BtnEditCategory),
                    $"{CMD_Main_Console} startedititem {shopItem.ID}", Layer + $".Item.{shopItem.ID}.Settings"));

                if (shop.canShowCategoriesMoveButtons)
                {
                    container.AddRange(shopUI.ShopItem.AdminPanel.ButtonMoveRight.GetButton("▶",
                        $"{CMD_Main_Console} edit_item_move {shopItem.ID} right",
                        Layer + $".Item.{shopItem.ID}.Settings"));

                    container.AddRange(shopUI.ShopItem.AdminPanel.ButtonMoveLeft.GetButton("◀",
                        $"{CMD_Main_Console} edit_item_move {shopItem.ID} left",
                        Layer + $".Item.{shopItem.ID}.Settings"));
                }
            }
        ...
        private void BalanceUi(CuiElementContainer container, BasePlayer player)
        {
            ...
            container.Add(shopUI.ShopContent.Header.Balance.Value.GetText(nowEconomy.GetBalanceTitle(player),
                Layer + ".Balance", Layer + ".Balance.Value", Layer + ".Balance.Value"));
        }
        ...
        private void ShowChoiceEconomyUI(BasePlayer player, CuiElementContainer container, bool selected = false)
        {
            ...
            container.AddRange(shopUI.ShopContent.Header.ButtonToggleEconomy.GetButton(
                Msg(player, UIContentHeaderButtonToggleEconomy), $"{CMD_Main_Console} economy_try_change {!selected}",
                Layer + ".Header", Layer + ".Change.Economy", Layer + ".Change.Economy"));
            ...
            if (selected)
            {
                ...
                container.Add(shopUI.SelectCurrency.Title.GetText(Msg(player, ChoiceEconomy),
                    Layer + ".Change.Economy.Panel"));
                ...
                foreach (var economyConf in _economics)
                {
                    ...
                    container.Add(shopUI.SelectCurrency.EconomyTitle.GetText(economyConf.GetTitle(player),
        ...
        private void EditUi(BasePlayer player, int itemId = 0, bool First = false)
        {
			...
			var bypassOwnership = Convert.ToBoolean(edit["BypassOwnershipCheck"]);
			...
			CheckBoxUi(ref container, EditingLayer + ".Main", CuiHelper.GetGuid(),
				...
				$"{xSwitch + 10} 20",
				bypassOwnership,
				$"{CMD_Main_Console} edititem BypassOwnershipCheck {!bypassOwnership}",
				"SkinCheck");
            ...
            string input = "")
        {
            ...
            GetAvailableItemsToSelect(selectedCategory, page, input, totalAmount, out var canSearch,
                out var itemsAmount, out var Items);

            ShowGridUI(container, 0, Items.Count, amountOnString, xMargin, yMargin, Width, Height, xSwitch, ySwitch,
                0.5f, 0.5f, 1f, 1f,
        ...
        private bool CanAccesToItem(BasePlayer player, ShopItem item)
        {
            ...
            if (!PlayerDLCAPI.Call<bool>("IsOwnedOrFreeItem", player, item.itemId, item.Skin)) return false;
            return true;
        }
        ...
        private IEnumerator GiveCartItems(BasePlayer player, List<KeyValuePair<ShopItem, int>> items, double price)
        {
            ...
            try
            {
                ...
                {
                    logItems.Add(cartItem.Key + $"[x{cartItem.Value}]");
                    ...
                    var itemPrice = cartItem.Key.GetPrice(player, selectedEconomy) * cartItem.Value;
                    LogBuySell(player, cartItem.Key, cartItem.Value, itemPrice, true);
                ...
                var playerEconomy = GetPlayerEconomy(player);
                var text = string.Format(lang.GetMessage(LogBuyItems, this), 
                    player.displayName, player.UserIDString, 
                    playerEconomy.GetPriceTitle(player, price.ToString(_config.Formatting.BuyPriceFormat)),
                    string.Join(", ", logItems));

                if (_config.LogToConsole) Puts(text);
                if (_config.LogToFile) LogToFile("Buy", $"[{DateTime.Now}] {text}", this);
            }
            finally
            {
        ...
        private (List<ShopItem> items, int totalItemsCount) GetPaginationShopItems(BasePlayer player)
        {
            ...
            var shopTotalItemsAmount = GetShopTotalItemsAmount(player);
            return (
                shopItems.SkipAndTake((isSearch ? shop.currentSearchPage : shop.currentShopPage) * shopTotalItemsAmount,
                    shopTotalItemsAmount), shopItems.Count);
        }
        ...
        internal string ConvertDropboxUrl(string url)
        {
            if (string.IsNullOrEmpty(url) || !url.Contains("dropbox.com")) return url;

            // For /scl/fi/ links that already have dl=1, use them as-is
            // User has confirmed these links work with dl=1 parameter
            if (url.Contains("/scl/fi/") && (url.Contains("?dl=1") || url.Contains("&dl=1")))
            {
                return url;
            }
            ...
            try
            {
                // Convert old format: https://www.dropbox.com/s/ID/filename?dl=1
                // To: https://dl.dropboxusercontent.com/s/ID/filename
                if (url.Contains("/s/") && !url.Contains("/scl/"))
                {
                    url = url.Replace("https://www.dropbox.com/s/", "https://dl.dropboxusercontent.com/s/");
                ...
                // Convert new format: https://www.dropbox.com/scl/fi/ID/filename?rlkey=...&st=...&dl=1
                // Note: /scl/fi/ links may not work reliably with dl.dropboxusercontent.com conversion
                // Try conversion first, but the original URL format may be needed as fallback
                if (url.Contains("/scl/fi/"))
                {
                    // For /scl/fi/ links, try using the original URL with dl=1 parameter
                    // Some Dropbox links don't work when converted to dl.dropboxusercontent.com
                    var uri = new Uri(url);
                    var query = uri.Query;
                    
                    // Ensure dl=1 is in the query string
                    if (!query.Contains("dl=1"))
                    {
                        if (string.IsNullOrEmpty(query) || query == "?")
                        {
                            query = "?dl=1";
                        }
                        else if (!query.Contains("dl="))
                        {
                            query = $"{query}&dl=1";
                        }
                        else
                    ...
                    // Return original format with dl=1 - UnityWebRequest may handle this better
                    return $"{uri.Scheme}://{uri.Host}{uri.AbsolutePath}{query}";
                }
            }
            catch (Exception ex)
            {
                PrintError($"Error converting Dropbox URL: {url} - {ex.Message}");
            }

            return url;
        }

        internal void AddImage(string url, string fileName, ulong imageId = 0UL)
        {
            if (string.IsNullOrEmpty(url)) return;

            var imageName = !string.IsNullOrEmpty(fileName) ? fileName : url;

            // If already loaded, skip
            if (_loadedImages.ContainsKey(imageName)) return;

            // If currently loading, skip
            if (_loadingImages.Contains(imageName)) return;

            // Convert Dropbox URLs to direct links
            var directUrl = ConvertDropboxUrl(url);

            // If it's a URL, check if local file exists first (for Dropbox URLs)
            if (directUrl.IsURL() && url.Contains("dropbox.com"))
            {
                ...
                if (!string.IsNullOrEmpty(localPath))
                {
                    ...
                    if (File.Exists(localFilePath))
                    {
                        ...
                        _loadingImages.Add(imageName);
                        LoadImageFromFS(imageName, localPath, url, directUrl);
                        return;
                    }
                }
            }
            ...
            {
                _loadingImages.Add(imageName);
                Global.Runner.StartCoroutine(LoadImageFromUrl(imageName, directUrl));
            }
        }
        ...
        internal string GetImage(string name)
        {
            ...
            if (_loadedImages.TryGetValue(name, out var imageId))
                return imageId;

            // If it's a URL, try to load it immediately (synchronously if possible, or trigger async load)
            if (name.IsURL())
            {
                var directUrl = ConvertDropboxUrl(name);
                
                // Check if we already have this exact URL loaded (in case of URL conversion)
                foreach (var kvp in _loadedImages)
                {
                    // If the original URL was converted, we might have stored it with the original URL as key
                    // But we're looking it up with the original URL, so check both
                    if (kvp.Key == name || kvp.Key == directUrl)
                    {
                        // Update the mapping to include the original URL
                        if (kvp.Key != name)
                        {
                            _loadedImages[name] = kvp.Value;
                        }
                        return kvp.Value;
                    }
                }
                
                // For Dropbox URLs, check if local file exists and load it
                if (name.Contains("dropbox.com") && !_loadingImages.Contains(name) && !_loadingImages.Contains(directUrl))
                {
                    ...
                    if (!string.IsNullOrEmpty(localPath))
                    {
                        ...
                        if (File.Exists(localFilePath))
                        {
                            ...
                            LoadImageFromFS(name, localPath, name, directUrl);
                            // Return empty for now - will be available after async load
                            return string.Empty;
                        }
                    }
                }
                ...
                if (!_loadingImages.Contains(name) && !_loadingImages.Contains(directUrl))
                {
                    _loadingImages.Add(name);
                    Global.Runner.StartCoroutine(LoadImageFromUrl(name, directUrl));
                }
            }

            // Return empty string - client will handle missing images gracefully
            // The image will be loaded asynchronously and available on next UI refresh
            return string.Empty;
        }

        private bool HasImage(string name)
        {
            return !string.IsNullOrEmpty(name) && _loadedImages.ContainsKey(name);
        }

        private void SendImage(BasePlayer player, string imageName)
        {
            // Images loaded via FileStorage are automatically available to clients
            // This method ensures the image is loaded/loading
            if (player == null || !player.IsConnected || string.IsNullOrEmpty(imageName)) return;

            // Ensure image is being loaded if it's a URL
            if (imageName.IsURL() && !_loadedImages.ContainsKey(imageName) && !_loadingImages.Contains(imageName))
            {
        ...
        private void LoadImages()
        {
            ...
            foreach (var (name, url) in imagesList.ToArray())
            {
                ...
                if (url.StartsWith("TheMevent/"))
                {
                    ...
                    // Replace TheMevent/Images/Shop/ with Shop/Images/ for local images
                    var localPath = url.Replace("TheMevent/Images/Shop/", "Shop/Images/");
                    // Also handle paths that start with TheMevent/ but don't have Images/Shop/
                    if (localPath == url && url.StartsWith("TheMevent/"))
                    {
                        localPath = url.Replace("TheMevent/", "Shop/Images/");
                    }
                    
                    LoadImageFromFS(name, localPath);
                }
            }

            // Load all URL images using our FileStorage system
            foreach (var (name, url) in imagesList)
            {
                if (url.IsURL())
                {
                    AddImage(url, name);
        ...
        internal string GetLocalImagePath(string url)
        {
            ...
            try
            {
                ...
                // If no filename or invalid, use hash
                if (string.IsNullOrEmpty(filename) || filename.Length < 3 || !filename.Contains("."))
                {
                    filename = null;
                }
            }
            ...
            // If we couldn't get a good filename, use URL hash
            if (string.IsNullOrEmpty(filename))
            {
                using (var md5 = MD5.Create())
                {
                    var hash = md5.ComputeHash(Encoding.UTF8.GetBytes(url));
                    filename = BitConverter.ToString(hash).Replace("-", "").ToLower() + ".png";
                }
            }
            else
            {
        ...
        {
            // First, try to load from local cached file if it exists
            var localPath = GetLocalImagePath(url);
            if (!string.IsNullOrEmpty(localPath))
            {
                var localFilePath = Interface.Oxide.DataDirectory + Path.DirectorySeparatorChar + localPath;
                if (File.Exists(localFilePath))
                {
                    ...
                    var fileUrl = "file://" + localFilePath;
                    ...
                    if (www.result == UnityWebRequest.Result.Success)
                    {
                        ...
                        try
                        {
                            ...
                            var textureId = FileStorage.server.Store(image, FileStorage.Type.png, CommunityEntity.ServerInstance.net.ID)
                                .ToString();
                            
                            _loadedImages.TryAdd(name, textureId);
                            if (name != url && !_loadedImages.ContainsKey(url))
                            {
                                _loadedImages.TryAdd(url, textureId);
                            }
                            var originalUrl = name.Contains("dropbox.com") ? name : url;
                            if (originalUrl != name && originalUrl != url && !_loadedImages.ContainsKey(originalUrl))
                            {
                                _loadedImages.TryAdd(originalUrl, textureId);
                            }
                        }
                        finally
                        {
            ...
            // If local file doesn't exist or failed to load, try direct URL first
                ...
                if (www.result == UnityWebRequest.Result.Success)
                {
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(localPath))
                        {
                            ...
                            {
                                PrintWarning($"Failed to save image to local file: {localPath} - {ex.Message}");
                            }
                        }
            ...
            // If direct URL failed, try downloading as raw bytes and saving to local file
            if (!string.IsNullOrEmpty(localPath))
            {
                using (var www = new UnityWebRequest(url))
                {
                    ...
                    if (www.result == UnityWebRequest.Result.Success)
                    {
                        ...
                        // Save file outside try-catch to avoid yield issues
                        try
                        {
                        ...
                        {
                            PrintError($"Failed to save downloaded image to local file: {localPath} - {ex.Message}");
                            _loadingImages.Remove(name);
                            yield break;
                        }

                        // Now load from local file (yield must be outside try-catch)
                        if (!string.IsNullOrEmpty(localFilePath) && File.Exists(localFilePath))
                        {
                            var fileUrl = "file://" + localFilePath;
                            using (var www2 = UnityWebRequestTexture.GetTexture(fileUrl))
                            {
            ...
            // All methods failed
            _instance?.PrintError($"Failed to load image from URL: {url} - All loading methods failed");
            _loadingImages.Remove(name);
        }

        private static void RegisterImage(string image, ref Dictionary<string, string> imagesList)
        {
            if (string.IsNullOrEmpty(image) || !image.IsURL()) return;
        ...
        private void LoadImageFromFS(string name, string path, string originalUrl = null, string directUrl = null)
        {
            if (string.IsNullOrEmpty(name) || string.IsNullOrEmpty(path)) return;

            Global.Runner.StartCoroutine(LoadImage(name, path, originalUrl, directUrl));
        }
        ...
        private IEnumerator LoadImage(string name, string path, string originalUrl = null, string directUrl = null)
        {
            ...
            if (www.result != UnityWebRequest.Result.Success)
            {
                ...
                // Remove from loading list on failure
                _loadingImages.Remove(name);
            }
            else
            {
                ...
                try
                {
                    ...
                    // Store with multiple keys for proper lookup
                    _loadedImages.TryAdd(name, textureId);
                    
                    // Also store with URL keys if provided (for Dropbox URLs)
                    if (!string.IsNullOrEmpty(originalUrl) && originalUrl != name && !_loadedImages.ContainsKey(originalUrl))
                    {
                        _loadedImages.TryAdd(originalUrl, textureId);
                    }
                    
                    if (!string.IsNullOrEmpty(directUrl) && directUrl != name && directUrl != originalUrl && !_loadedImages.ContainsKey(directUrl))
                    {
                        _loadedImages.TryAdd(directUrl, textureId);
                    }
                }
                finally
                {
        ...
        private void LogBuySell(BasePlayer player, ShopItem item, int amount, double price, bool isBuy)
        {
            ...
            var itemText = item.ToString() + $"[x{amount}]";
            ...
            // Handle Discord logging - either immediate or batched
            if (!string.IsNullOrEmpty(_config.DiscordWeebhook))
            {
                if (_config.EnableBatchDiscord)
                {
                    // Store transaction for batch reporting
                    // Store transaction for batch - pass price as double, not formatted string
                    StoreTransactionForBatch(isBuy ? "Buy" : "Sell", isBuy ? LogBuyItems : LogSellItem, 
                        player.displayName, player.UserIDString, price.ToString(), itemText, item);
                }
                else
                {
                    // Send immediately with enhanced formatting and item icons
                    SendDiscordMessageBuySell(_config.DiscordWeebhook, text, isBuy, player.displayName, 
                        player.UserIDString, item.PublicTitle ?? itemText, item.ShortName, item.Skin, 
                        amount * item.Amount, formattedPrice, item.Image);
                }
            }
        }
        ...
        private void LogTransfer(string playerName, string steamId, string amount, string targetName, string targetId)
        {
            ...
            if (!string.IsNullOrEmpty(_config.DiscordWeebhook))
            {
                ...
                {
                    // Store transaction for batch reporting
                    StoreTransactionForBatch("Transfer", "Transfer", playerName, steamId, amount, $"{targetName} ({targetId})");
                }
                else
                {
                    // Send immediately (original behavior)
                    SendDiscordMessage(_config.DiscordWeebhook, text);
                }
            }
        }
        ...
        private void Log(string filename, string key, params object[] obj)
        {
            ...
            if (!string.IsNullOrEmpty(_config.DiscordWeebhook))
            {
                ...
                {
                    // Store transaction for batch reporting
                    StoreTransactionForBatch(filename, key, obj);
                }
                else
                {
                    // Send immediately (original behavior)
        ...
        private void StoreTransactionForBatch(string filename, string key, params object[] obj)
        {
            try
            {
                var playerName = obj[0]?.ToString();
                var steamId = obj[1]?.ToString();
                var priceStr = obj[2]?.ToString();
                var item = obj[3]?.ToString();
                ShopItem shopItem = obj.Length > 4 && obj[4] is ShopItem ? (ShopItem)obj[4] : null;

                if (string.IsNullOrEmpty(playerName) || string.IsNullOrEmpty(steamId)) return;

                var steamIdUlong = Convert.ToUInt64(steamId);
                
                // Parse price as double
                double price = 0;
                if (!string.IsNullOrEmpty(priceStr) && double.TryParse(priceStr.Replace("$", "").Replace(",", ""), out double parsedPrice))
                {
                    price = parsedPrice;
                }
                
                // Extract amount from item string (format: itemname[xamount])
                int amount = 1;
                if (!string.IsNullOrEmpty(item))
                {
                    var amountMatch = Regex.Match(item, @"\[x(\d+)\]");
                    if (amountMatch.Success && int.TryParse(amountMatch.Groups[1].Value, out int parsedAmount))
                    {
                        amount = parsedAmount;
                    }
                }
                ...
                // Store item info if available for better Discord formatting
                if (shopItem != null)
                {
                    transaction.ItemShortName = shopItem.ShortName;
                    transaction.ItemSkin = shopItem.Skin;
                    transaction.ItemImage = shopItem.Image;
                    transaction.ItemDisplayName = shopItem.PublicTitle ?? item;
                }

                if (filename.ToLower() == "sell")
                {
                    if (!_playerSales.ContainsKey(steamIdUlong))
                        _playerSales[steamIdUlong] = new List<ShopTransaction>();
                    _playerSales[steamIdUlong].Add(transaction);
                }
                else if (filename.ToLower() == "buy")
                {
                    if (!_playerPurchases.ContainsKey(steamIdUlong))
                        _playerPurchases[steamIdUlong] = new List<ShopTransaction>();
                    _playerPurchases[steamIdUlong].Add(transaction);
                }
            }
            catch (Exception ex)
            {
                PrintError($"Error storing transaction for batch: {ex.Message}");
            }
        }

        private void SendBatchedReport()
        {
            if (string.IsNullOrEmpty(_config.DiscordWeebhook)) return;

            var report = FormatBatchedReport();
            if (string.IsNullOrEmpty(report)) return;

            var embed = new Embed();
            embed.color = 0x00FF00;
            embed.title = "📊 Shop Activity Report";
            embed.AddField("Report", report, false, embed.color);
            
            // Find most popular item for thumbnail
            var mostPopularItem = GetMostPopularItemFromBatch();
            if (mostPopularItem != null)
            {
                var thumbnailUrl = GetItemIconUrl(mostPopularItem.ItemShortName, mostPopularItem.ItemSkin, mostPopularItem.ItemImage);
                if (!string.IsNullOrEmpty(thumbnailUrl))
                {
                    embed.thumbnail = new Thumbnail { url = thumbnailUrl };
                }
            }
            ...
            embed.footer = new Footer
            {
                ...
                icon_url = "https://www.dropbox.com/scl/fi/cfqwdj0sqdtn7ydog3g14/gr.png?rlkey=0ataku53xk5ouytcskmvt5vxx&st=dlljaqox&dl=1"
            };

            var discordMessageObj = new DiscordMessage("", embed);

            webrequest.Enqueue(_config.DiscordWeebhook, discordMessageObj.ToJson(), (code, response) => { },
                this,
                RequestMethod.POST,
        ...
        private ShopTransaction GetMostPopularItemFromBatch()
        {
            // Count item occurrences across all transactions
            var itemCounts = new Dictionary<string, (ShopTransaction transaction, int count)>();
            
            // Count from sales
            foreach (var playerSales in _playerSales.Values)
            {
                foreach (var transaction in playerSales)
                {
                    var key = $"{transaction.ItemShortName}_{transaction.ItemSkin}";
                    if (itemCounts.ContainsKey(key))
                    {
                        itemCounts[key] = (itemCounts[key].transaction, itemCounts[key].count + transaction.Amount);
                    }
                    else
                    {
                        itemCounts[key] = (transaction, transaction.Amount);
                    }
                }
            }
            
            // Count from purchases
            foreach (var playerPurchases in _playerPurchases.Values)
            {
                foreach (var transaction in playerPurchases)
                {
                    var key = $"{transaction.ItemShortName}_{transaction.ItemSkin}";
                    if (itemCounts.ContainsKey(key))
                    {
            ...
            // Find item with highest count that has an image
            ShopTransaction mostPopular = null;
            int maxCount = 0;
            
            foreach (var kvp in itemCounts)
            {
                var transaction = kvp.Value.transaction;
                var count = kvp.Value.count;
                
                // Prefer items that have images
                var hasImage = !string.IsNullOrEmpty(transaction.ItemImage) || 
                              !string.IsNullOrEmpty(transaction.ItemShortName);
                
                if (hasImage && (mostPopular == null || count > maxCount))
                {
                    mostPopular = transaction;
                    maxCount = count;
                }
            }
        ...
        private void SendWipeSummaryReport()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(report))
                {
                    ...
                    embed.title = "🏁 Shop Wipe Summary";
                    embed.AddField("Summary", report, false, embed.color);
                    
                    // Add footer with timestamp
                    embed.footer = new Footer
                    {
                        text = $"Wipe Period: {_wipeStartTime:yyyy-MM-dd HH:mm:ss} to {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}",
                        icon_url = "https://www.dropbox.com/scl/fi/cfqwdj0sqdtn7ydog3g14/gr.png?rlkey=0ataku53xk5ouytcskmvt5vxx&st=dlljaqox&dl=1"
                    };
                    ...
                    PrintWarning("Shop wipe summary report sent to Discord");
                }
            }
            catch (Exception ex)
            {
                PrintError($"Error sending shop wipe summary report: {ex.Message}");
            }
        }

        private string FormatBatchedReport()
        {
            var report = new StringBuilder();
            bool hasAnyTransactions = false;

            // Format sales
            if (_playerSales.Count > 0)
            {
                report.AppendLine("**Shop Sales**");
                foreach (var playerSales in _playerSales)
                {
                    var firstSale = playerSales.Value.Count > 0 ? playerSales.Value[0] : null;
                    var playerName = firstSale?.PlayerName ?? "Unknown";
                    var steamId = firstSale?.SteamId ?? "Unknown";
                    
                    report.AppendLine($"Player {playerName} ({steamId})");
                    
                    foreach (var transaction in playerSales.Value)
                    {
                        var cleanItem = CleanItemString(transaction.Item);
                        report.AppendLine($"- {transaction.Price}: {cleanItem} x{transaction.Amount}");
                    }
                    report.AppendLine();
                }
            ...
            // Format purchases
            if (_playerPurchases.Count > 0)
            {
                if (hasAnyTransactions) report.AppendLine();
                report.AppendLine("**Shop Purchases**");
                foreach (var playerPurchases in _playerPurchases)
                {
                    var firstPurchase = playerPurchases.Value.Count > 0 ? playerPurchases.Value[0] : null;
                    var playerName = firstPurchase?.PlayerName ?? "Unknown";
                    var steamId = firstPurchase?.SteamId ?? "Unknown";
                    
                    report.AppendLine($"Player {playerName} ({steamId})");
                    
                    foreach (var transaction in playerPurchases.Value)
                    {
                        var cleanItem = CleanItemString(transaction.Item);
                        report.AppendLine($"- {transaction.Price}: {cleanItem} x{transaction.Amount}");
                    }
            ...
            {
                return "No shop transactions in the last 12 hours.";
            }

            return report.ToString();
        }

        private (Dictionary<ulong, List<ShopTransaction>> sales, Dictionary<ulong, List<ShopTransaction>> purchases) ReadTransactionDataFromLogs()
        {
            var sales = new Dictionary<ulong, List<ShopTransaction>>();
            var purchases = new Dictionary<ulong, List<ShopTransaction>>();
            ...
            try
            {
                ...
                {
                    PrintWarning("[DEBUG] Log directory does not exist! Creating it...");
                    Directory.CreateDirectory(logDirectory);
                }

                // Auto-detect wipe start time from first sale in log files
                var wipeStart = FindFirstSaleTime(logDirectory);
                var wipeEnd = DateTime.UtcNow; // Use current time as wipe end
                
                PrintWarning($"[DEBUG] Auto-detected wipe start time: {wipeStart:yyyy-MM-dd HH:mm:ss}");
                PrintWarning($"[DEBUG] Wipe end time: {wipeEnd:yyyy-MM-dd HH:mm:ss}");

                // Read all shop_sell log files
                var sellFiles = Directory.GetFiles(logDirectory, "shop_sell-*.txt");
                PrintWarning($"[DEBUG] Found {sellFiles.Length} sell log files");
                foreach (var file in sellFiles)
                {
                    PrintWarning($"[DEBUG] Processing file: {Path.GetFileName(file)}");
                    var lines = File.ReadAllLines(file);
                    PrintWarning($"[DEBUG] File has {lines.Length} lines");
                    foreach (var line in lines)
                    {
                        if (string.IsNullOrEmpty(line)) continue;
                        
                        var transaction = ParseLogLine(line, "sell");
                        if (transaction != null && ulong.TryParse(transaction.SteamId, out ulong steamId))
                        {
                            PrintWarning($"[DEBUG] Parsed transaction: {transaction.PlayerName} at {transaction.Timestamp:yyyy-MM-dd HH:mm:ss}");
                            // Check if transaction occurred during this wipe period
                            if (IsTransactionInWipePeriod(transaction, wipeStart, wipeEnd))
                            {
                                PrintWarning($"[DEBUG] Transaction is within wipe period");
                                if (!sales.ContainsKey(steamId))
                                    sales[steamId] = new List<ShopTransaction>();
                                sales[steamId].Add(transaction);
                            }
                        }
                    }
                }

                // Read all shop_buy log files
                var buyFiles = Directory.GetFiles(logDirectory, "shop_buy-*.txt");
                foreach (var file in buyFiles)
                {
                    var lines = File.ReadAllLines(file);
                    foreach (var line in lines)
                    {
                        ...
                        var transaction = ParseLogLine(line, "buy");
                        if (transaction != null && ulong.TryParse(transaction.SteamId, out ulong steamId))
                        {
                            // Check if transaction occurred during this wipe period
                            if (IsTransactionInWipePeriod(transaction, wipeStart, wipeEnd))
                            {
                                if (!purchases.ContainsKey(steamId))
                                    purchases[steamId] = new List<ShopTransaction>();
                                purchases[steamId].Add(transaction);
                            }
                        }
                    }
                }
            ...
            {
                PrintError($"Error reading transaction data from logs: {ex.Message}");
            }
            
            PrintWarning($"[DEBUG] Final results - Sales: {sales.Count} players, Purchases: {purchases.Count} players");
            return (sales, purchases);
        }

        private DateTime FindFirstSaleTime(string logDirectory)
        {
            try
            {
                if (!Directory.Exists(logDirectory))
                {
                    PrintWarning($"[DEBUG] Log directory does not exist: {logDirectory}");
                ...
                {
                    PrintWarning("[DEBUG] No sell log files found, using current time as wipe start");
                    return DateTime.UtcNow;
                }

                // Sort files by name to get chronological order
                Array.Sort(sellFiles);
                ...
                foreach (var file in sellFiles)
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        if (transaction != null)
                        {
                            PrintWarning($"[DEBUG] Found first sale at: {transaction.Timestamp:yyyy-MM-dd HH:mm:ss} from file: {Path.GetFileName(file)}");
                            return transaction.Timestamp;
                        }
                    }
                }

                PrintWarning("[DEBUG] No valid sales found in log files, using current time as wipe start");
                return DateTime.UtcNow;
            }
            catch (Exception ex)
            {
                PrintError($"Error finding first sale time: {ex.Message}");
                return DateTime.UtcNow;
            }
        }

        private ShopTransaction ParseLogLine(string line, string type)
        {
            try
            {
                // Simple approach - just find the key parts without complex regex
                if (!line.Contains("sold item for") && !line.Contains("bought item"))
                    return null;

                // Extract timestamp
                var timestampStart = line.IndexOf('[') + 1;
                var timestampEnd = line.IndexOf(']');
                if (timestampStart <= 0 || timestampEnd <= timestampStart) return null;
                
                var timestampStr = line.Substring(timestampStart, timestampEnd - timestampStart);
                DateTime timestamp;
                if (!DateTime.TryParse(timestampStr, out timestamp))
                    timestamp = DateTime.Now;

                // Extract player name and Steam ID
                var playerStart = line.IndexOf("Player ") + 7;
                var playerEnd = line.IndexOf(" (", playerStart);
                if (playerStart <= 6 || playerEnd <= playerStart) return null;
                
                var playerName = line.Substring(playerStart, playerEnd - playerStart).Trim();
                
                var steamIdStart = playerEnd + 2;
                var steamIdEnd = line.IndexOf(")", steamIdStart);
                if (steamIdStart <= playerEnd || steamIdEnd <= steamIdStart) return null;
                
                var steamId = line.Substring(steamIdStart, steamIdEnd - steamIdStart);

                // Extract price - look for "for X <" pattern
                var forIndex = line.IndexOf(" for ");
                if (forIndex == -1) return null;
                
                var priceStart = forIndex + 5;
                var priceEnd = line.IndexOf(" <", priceStart);
                if (priceEnd <= priceStart) return null;
                
                var priceStr = line.Substring(priceStart, priceEnd - priceStart).Trim();
                double price;
                if (!double.TryParse(priceStr, out price))
                    return null;

                // Extract item info - look for [ITEM- pattern
                var itemStart = line.IndexOf("[ITEM");
                if (itemStart == -1) return null;
                
                var itemEnd = line.IndexOf("]", itemStart);
                if (itemEnd <= itemStart) return null;
                
                var itemId = line.Substring(itemStart + 1, itemEnd - itemStart - 1);
                
                // Extract item name and amount
                var nameStart = itemEnd + 1;
                var nameEnd = line.IndexOf("[x", nameStart);
                if (nameEnd <= nameStart) return null;
                
                var itemName = line.Substring(nameStart, nameEnd - nameStart).Trim();
                
                var amountStart = nameEnd + 2;
                var amountEnd = line.IndexOf("]", amountStart);
                if (amountEnd <= amountStart) return null;
                
                var amountStr = line.Substring(amountStart, amountEnd - amountStart);
                int amount;
            ...
            {
                PrintError($"Error parsing log line: {ex.Message}");
                return null;
            }
        }

        private bool IsTransactionInWipePeriod(ShopTransaction transaction, DateTime wipeStart, DateTime wipeEnd)
        {
            return transaction.Timestamp >= wipeStart && transaction.Timestamp <= wipeEnd;
        }
        ...
        private string FormatWipeSummaryReport()
        {
            ...
            var actualWipeStart = FindFirstSaleTime(logDirectory);
            var actualWipeEnd = DateTime.UtcNow;
            
            report.AppendLine($"*Wipe Period: {actualWipeStart:MM-dd-yyyy hh:mm:ss tt} to {actualWipeEnd:MM-dd-yyyy hh:mm:ss tt}*");
            report.AppendLine();

            // Read transaction data from log files
            var logData = ReadTransactionDataFromLogs();
            var salesData = logData.sales;

            // Calculate totals for the entire wipe
            double totalSalesValue = 0;
            int totalSalesCount = 0;

            // Format sales summary - only show sales data
            if (salesData.Count > 0)
            {
                // Calculate totals first
                foreach (var playerSales in salesData)
                {
                    foreach (var transaction in playerSales.Value)
                    {
                        totalSalesValue += transaction.Price; // Price is already total, not unit price
                        totalSalesCount += transaction.Amount;
                    }
                }

                // Add wipe totals first
                report.AppendLine("**📊 WIPE TOTALS**");
                report.AppendLine($"- Total Sales Value: ${totalSalesValue:F2}");
                report.AppendLine($"- Total Sales Items: {totalSalesCount}");
                report.AppendLine($"- Unique Players: {salesData.Count}");
                report.AppendLine();

                report.AppendLine("**📈 PLAYER SALES SUMMARY**");
                // Sort players by total sales value (highest first)
                var sortedPlayers = new List<KeyValuePair<ulong, List<ShopTransaction>>>();
                foreach (var kvp in salesData)
                {
                    double totalValue = 0;
                    foreach (var t in kvp.Value)
                        totalValue += t.Price;
                    sortedPlayers.Add(new KeyValuePair<ulong, List<ShopTransaction>>(kvp.Key, kvp.Value));
                }
                sortedPlayers.Sort((a, b) =>
                {
                    double totalA = 0, totalB = 0;
                    foreach (var t in a.Value) totalA += t.Price;
                    foreach (var t in b.Value) totalB += t.Price;
                    return totalB.CompareTo(totalA);
                });
                
                foreach (var playerSales in sortedPlayers)
                {
                    var firstSale = playerSales.Value.Count > 0 ? playerSales.Value[0] : null;
                    var playerName = firstSale?.PlayerName ?? "Unknown";
                    var steamId = firstSale?.SteamId ?? "Unknown";
                    
                    double playerTotalValue = 0;
                    int playerItemCount = 0;
                    
                    // Sort transactions by timestamp for better readability (newest first)
                    var sortedTransactions = new List<ShopTransaction>(playerSales.Value);
                    ...
                    {
                        // Price is already the total price for the transaction, not unit price
                        playerTotalValue += transaction.Price;
                        playerItemCount += transaction.Amount;
                        
                        report.AppendLine($"- {transaction.Timestamp:MM-dd-yyyy hh:mm:ss tt}: {transaction.Item} x{transaction.Amount} for ${transaction.Price:F2}");
                    }
                    
                    report.AppendLine($"- **Total Sales: ${playerTotalValue:F2} ({playerItemCount} items)**");
                    report.AppendLine();
                }
                hasAnyTransactions = true;
            }
            ...
            if (!hasAnyTransactions)
            {
                return "No shop sales found for wipe summary.";
            }

            return report.ToString();
        }

        private string CleanItemString(string item)
        {
            if (string.IsNullOrEmpty(item)) return item;

            // Remove HTML tags and clean up the item string
            var cleaned = Regex.Replace(item, @"<[^>]*>", "");
            cleaned = Regex.Replace(cleaned, @"\[ITEM[^\]]*\]", "");
            cleaned = Regex.Replace(cleaned, @"\(DN:[^)]*\)", "");
            cleaned = Regex.Replace(cleaned, @"\(SKIN:[^)]*\)", "");
        ...
        {
            _playerSales.Clear();
            _playerPurchases.Clear();
        }

        internal void CmdShopWipeSummary(ConsoleSystem.Arg arg)
        {
            try
            {
                var report = FormatWipeSummaryReport();
                if (!string.IsNullOrEmpty(report))
                {
                    ...
                    if (!string.IsNullOrEmpty(_config.DiscordWeebhook))
                    {
                        SendDiscordTextChunks(_config.DiscordWeebhook, report);
                    }
                }
                else
                {
                    PrintWarning("[Shop] No shop transactions found for wipe summary.");
                }
            }
            catch (Exception ex)
            {
                PrintError($"[Shop] Error generating wipe summary: {ex.Message}");
            }
        }

        internal void CmdSetWipeStart(ConsoleSystem.Arg arg)
        {
            var cmdArgs = arg.Args == null ? Array.Empty<string>() : Array.ConvertAll(arg.Args, value => value.ToString());
            if (!arg.IsAdmin) return;
            ...
            if (cmdArgs.Length == 0)
            {
                Puts($"Current wipe start time: {_wipeStartTime:yyyy-MM-dd HH:mm:ss}");
                Puts($"Current wipe end time: {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}");
                Puts("Usage: shop.setwipestart <datetime> (format: MM/dd/yyyy HH:mm:ss)");
                return;
            }

            if (DateTime.TryParse(cmdArgs[0], out DateTime newWipeStart))
            {
                _wipeStartTime = newWipeStart;
                Puts($"Wipe start time set to: {_wipeStartTime:yyyy-MM-dd HH:mm:ss}");
            }
            else
            {
                PrintError("Invalid datetime format. Use: MM/dd/yyyy HH:mm:ss");
            }
        }

        // Chunked content sender for Discord (<=1800 chars per message)
        private void SendDiscordTextChunks(string webhook, string message)
        {
            if (string.IsNullOrEmpty(webhook) || string.IsNullOrEmpty(message)) return;
            
            // Find the logical break between header/totals and player sales
            var playerSalesStart = message.IndexOf("**📈 PLAYER SALES SUMMARY**");
            if (playerSalesStart == -1)
            {
                // No player sales section, send as single message
            ...
            // Split into header/totals and player sales
            var headerAndTotals = message.Substring(0, playerSalesStart).Trim();
            var playerSales = message.Substring(playerSalesStart).Trim();
            
            // Send header and totals first
            var headerPayload = new Dictionary<string, object>
            {
                {"content", headerAndTotals},
                {"username", "Shop"},
                {"avatar_url", "https://www.dropbox.com/scl/fi/cfqwdj0sqdtn7ydog3g14/gr.png?rlkey=0ataku53xk5ouytcskmvt5vxx&st=dlljaqox&dl=1"}
            };
            webrequest.Enqueue(webhook, JsonConvert.SerializeObject(headerPayload), (code, response) => { }, this, RequestMethod.POST, new Dictionary<string, string>{{"Content-Type","application/json"}}, 30f);
            
            // Split player sales into chunks if needed
            const int maxLen = 1800;
            if (playerSales.Length <= maxLen)
            {
            ...
            {
                // Split player sales into chunks
                var chunks = new List<string>();
                int index = 0;
                while (index < playerSales.Length)
                {
                    int len = Math.Min(maxLen, playerSales.Length - index);
                    string chunk = playerSales.Substring(index, len);
                    chunks.Add(chunk);
                    index += len;
                }
                
                // Send player sales chunks
                for (int i = 0; i < chunks.Count; i++)
                {
                    var chunk = chunks[i];
                    var chunkIndex = i;
                    timer.Once(0.5f + (chunkIndex * 0.5f), () =>
                    {
                        var payload = new Dictionary<string, object>
                        {
                            {"content", chunk},
        ...
        private void CreatePlaceholderImage(string filePath, string text, int width = 64, int height = 64)
        {
            try
            {
                // Create a simple colored rectangle as placeholder
                var bytes = CreateSimplePng(width, height, text);
                File.WriteAllBytes(filePath, bytes);
                PrintWarning($"Created placeholder image: {Path.GetFileName(filePath)}");
            }
            catch (Exception ex)
            {
                PrintError($"Error creating placeholder image: {ex.Message}");
            }
        }

        private byte[] CreateSimplePng(int width, int height, string text)
        {
            // This is a very basic PNG creation - in practice, you'd want to use a proper image library
        ...
        {
            var embed = new Embed();
            embed.AddField("SHOP", message, false, 0xFFFFFF);
        ...
        private void SendDiscordMessageBuySell(string webhook, string message, bool isBuy, 
            string playerName, string steamId, string itemDisplayName, string itemShortName, 
            ulong itemSkin, int amount, string formattedPrice, string itemImage = null)
        {
            var embed = new Embed();
            
            // Set color based on buy/sell (green for buy, red for sell)
            embed.color = isBuy ? 0x00FF00 : 0xFF0000;
            
            // Add title with player info
            embed.title = isBuy ? "🛒 Purchase" : "💰 Sale";
            
            // Add main field with transaction details
            var fieldValue = $"**Player:** {playerName} ({steamId})\n" +
                           $"**Item:** {itemDisplayName}\n" +
                           $"**Amount:** {amount}\n" +
                           $"**Price:** {formattedPrice}";
            embed.AddField(isBuy ? "Purchase Details" : "Sale Details", fieldValue, false, embed.color);
            
            // Add footer with item icon (only if available)
            var itemIconUrl = GetItemIconUrl(itemShortName, itemSkin, itemImage);
            embed.footer = new Footer
            {
                text = $"{playerName} • {DateTime.Now:yyyy-MM-dd HH:mm:ss}",
                icon_url = itemIconUrl // Will be null if no custom image and external icons disabled
            };
        ...
        private string GetItemIconUrl(string shortName, ulong skin, string customImage = null)
        {
            // Priority 1: Use custom image if provided and is a URL
            // This uses your own hosted images (Dropbox, etc.) - no external dependency
            if (!string.IsNullOrEmpty(customImage) && customImage.IsURL())
            {
                // Convert Dropbox URLs to direct links if needed
                return ConvertDropboxUrl(customImage);
            }
            
            // Priority 2: Only use external service (rustedit.io) if explicitly enabled in config
            // This avoids external dependencies by default
            if (!string.IsNullOrEmpty(shortName) && _config.UseExternalItemIcons)
            {
                if (skin != 0)
            ...
            // No icon available - return null (Discord will just show no icon)
            return null;
        }
        ...
        private const string
            ...
            SellMinimumPayout = "SellMinimumPayout",
            BtnNext = "BtnNext",
            BtnBack = "BtnBack",
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [SellMinimumPayout] = "Not enough to sell for reward points. You need at least 20 scrap to get 1 RP.",
                [BtnBack] = "▲",
                [BtnNext] = "▼",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [SellMinimumPayout] = "Недостаточно для продажи за очки. Нужно минимум 20 металлолома для 1 RP.",
                [BtnBack] = "▲",
                [BtnNext] = "▼",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [BtnBack] = "▲",
                [BtnNext] = "▼",
                [SellMinimumPayout] = "不足以兑换奖励点数。至少需要20个废料才能获得1 RP。",
        ...
        private List<ShopItem> SearchItem(BasePlayer player, string search)
        {
            if (_searchCache.TryGetValue(search, out var searchInfo))
            {
                ...
                foreach (var x in searchInfo)
                {
                    if (string.IsNullOrEmpty(x.Permission) || player.HasPermission(x.Permission))
                        result.Add(x.Item);
                }
                return result;
            }
        ...
        private class OpenedShop
        {
            ...
            public OpenedShop(BasePlayer player, bool mainUI = true)
            {
                ...
                if (selectedCategory != null)
                {
                    ...
                    if (isSearch)
                        categoryItemsCount = _instance.SearchItemCount(Player, search);
                    else
        ...
        private class ShopInstaller
        {
            ...
            public void Finish()
            {
                ...
                if (fullscreenTemplate != null)
                {
                    ...
                    if (fullscreenTemplate.TemplateLang != null)
                        _instance.RegisterTemplateMessages(fullscreenTemplate.TemplateLang);
                }
                ...
                if (_instance.ServerPanel != null)
                {
                    ...
                    if (menuTemplate != null)
                    {
                        ...
                        if (menuTemplate.TemplateLang != null)
                            _instance.RegisterTemplateMessages(menuTemplate.TemplateLang);
                    }
                }
            ...
            public void LoadImages()
            {
                ...
                foreach (var shopTemplate in shopData.FullScreenTemplates)
                    _instance.AddImage(shopTemplate.BannerURL, shopTemplate.BannerURL);
        ...
        internal void CmdChatShopInstaller(BasePlayer player, string command, string[] args)
        {
            if (player == null) return;
        ...
        internal void CmdConsoleShopInstall(ConsoleSystem.Arg arg)
        {
            if (arg.Player() != null || !arg.IsServerside)
            {
                if (IsAdmin(arg.Player()))
        ...
        internal void CmdConsoleShopManage(ConsoleSystem.Arg arg)
        {
            if (arg.Player() != null || !arg.IsServerside)
            {
                if (IsAdmin(arg.Player()))
        ...
        internal void CmdDiscordTest(ConsoleSystem.Arg arg)
        {
            if (!arg.IsServerside)
            {
                SendReply(arg, "This command can only be run from the SERVER CONSOLE.");
            ...
            SendReply(arg, "Starting Discord webhook test...");
            SendReply(arg, $"Webhook URL: {_config.DiscordWeebhook}");

            if (string.IsNullOrEmpty(_config.DiscordWeebhook) || _config.DiscordWeebhook == "https://discord.com/api/webhooks/YOUR_WEBHOOK_URL_HERE")
            {
                SendReply(arg, "Discord webhook is not configured or is using the default placeholder URL.");
                SendReply(arg, "Please configure a valid Discord webhook URL in the Shop config file.");
                return;
            }

            try
            {
                var testMessage = $"🧪 **SHOP DISCORD TEST** 🧪\n" +
                                $"Server: {ConVar.Server.hostname}\n" +
                                $"Time: {DateTime.Now:yyyy-MM-dd HH:mm:ss}\n" +
                                $"Status: Discord webhook is working correctly!";

                SendReply(arg, "Sending test message to Discord...");
                SendDiscordMessage(_config.DiscordWeebhook, testMessage);
                SendReply(arg, "Discord test message sent successfully! Check your Discord channel.");
                SendReply(arg, "If you don't see the message, check that the webhook URL is valid and the Discord channel exists.");
            }
            catch (Exception ex)
            {
        ...
        internal void CmdConsoleShopInstaller(ConsoleSystem.Arg arg)
        {
            var player = arg?.Player();
            ...
            switch (arg.GetString(0))
            {
                ...
                case "select_template":
                {
                    ...
                    UpdateUI(player, container => { LoopTemplates(container, isFullScreen); });
                    break;
                }
...
namespace ShopHarmony.ShopExtensionMethods
{
    ...
    public static class ExtensionMethods
    {
        ...
        {
            perm ??= ShopHost.Instance?.Permission;
