...
namespace Game.Rust.Cui
{
    ...
    public static class CuiHelper
    {
        ...
        public static bool AddUi(BasePlayer player, string json)
        {
            if (player?.net != null)
            {
                // Clients only forward ConsoleGen commands. Rewrite compat-style Shop UI commands
                // callbacks to cui.endtest so ShopHarmony.Patches.Cui_Endtest_Patch can route them.
        ...
        /// <summary>
        /// Shop uses custom console commands (UI_Shop and UI_Shop_Installer) on CUI buttons. Under Harmony those
        /// never leave the client. Bridge through cui.endtest with SHOP / SHOPINST markers (installer must be rewritten first).
        /// </summary>
        private static string RewriteHarmonyButtonCommands(string json)
        {
            if (string.IsNullOrEmpty(json) || json.IndexOf("UI_Shop", StringComparison.Ordinal) < 0)
                return json;
            json = json.Replace("\"command\":\"UI_Shop_Installer", "\"command\":\"cui.endtest SHOPINST");
            json = json.Replace("\"command\":\"UI_Shop", "\"command\":\"cui.endtest SHOP");
            return json;
        }
