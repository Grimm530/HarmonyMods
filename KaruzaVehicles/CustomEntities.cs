...
namespace KaruzaVehicles
{
    ...
    public class CustomEntities : RustPlugin
    {
        ...
        private static readonly Dictionary<string, string> LangMessages = new Dictionary<string, string>
        {
            ...
            [MSG_CMD_PURGE_PLUGIN_MAKE_SURE_LOADED] = "Make sure it's loaded in. If it isn't, just delete the appropriate savefile from /HarmonyData/CustomEntities/ and it will have the same effect.",
        ...
        internal void CommandCountPlugin(IPlayer iplayer, string command, string[] args)
        {
            if (args.Length == 0)
            {
        ...
        internal void CommandPurgePlugin(IPlayer iplayer, string command, string[] args)
        {
            if (args.Length == 0)
            {
                iplayer.Reply(MSG(MSG_CMD_PROVIDE_PLUGIN_NAME, iplayer.Id));
        ...
        internal void CommandPurgePrefab(IPlayer iplayer, string command, string[] args)
        {
            if (args.Length == 0)
            {
        ...
        internal void CommandSpawnAtPlayerEyes(IPlayer iplayer, string command, string[] args)
        {
        ...
        private class BinaryData
        {
            ...
            public BinaryData(Plugin ownerPlugin, string optionalSuffix)
            {
                ...
                string ownerName = ownerPlugin?.Name;
                if (string.IsNullOrEmpty(ownerName))
                    ownerName = ownerPlugin?.GetType().Name;
                if (string.IsNullOrEmpty(ownerName))
                    ownerName = "unknown";

                filenameWithOrWithoutSuffixBuilder.Append(ownerName);
