...
namespace KaruzaVehicles
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[KaruzaVehicles] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[KaruzaVehicles] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[KaruzaVehicles] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[KaruzaVehicles] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[KaruzaVehicles] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[KaruzaVehicles] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[KaruzaVehicles] Permissions Service bind: " + ex.Message);
            }
        }
