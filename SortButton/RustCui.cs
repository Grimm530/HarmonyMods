...
namespace Oxide.Game.Rust.Cui
{
    ...
    public static class CuiHelper
    {
        ...
        public static bool AddUi(BasePlayer player, string json)
        {
            if (player?.net != null)
            {
                // Clients only forward ConsoleGen commands. Rewrite SortButton UI commands
                // to cui.endtest so SortButton.Patches.Cui_Endtest_Patch can route them.
        ...
        /// <summary>
        /// SortButton uses sortbutton.order / sortbutton.sort on CUI buttons. Under Harmony those never leave the client.
        /// Bridge through cui.endtest with SORTBUTTON marker.
        /// </summary>
        private static string RewriteHarmonyButtonCommands(string json)
        {
            if (string.IsNullOrEmpty(json))
                return json;
            json = json.Replace("\"command\":\"sortbutton.order", "\"command\":\"cui.endtest SORTBUTTON order");
            json = json.Replace("\"command\":\"sortbutton.sort", "\"command\":\"cui.endtest SORTBUTTON sort");
            return json;
        }
