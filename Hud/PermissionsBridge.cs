 ...
 * Adapted from Hud/PermissionsBridge.cs — namespace changed to CombatClassesHarmony,
 * log tag changed to [Hud].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace HudHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[Hud] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[Hud] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[Hud] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[Hud] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[Hud] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[Hud] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[Hud] Permissions Service bind: " + ex.Message);
            }
        }
