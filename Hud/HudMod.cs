// HudMod.cs — Harmony entry point for CombatClasses 1.0.0131
// Hosts Harmony.Plugins.Hud, lifecycle, chat + console commands.
...
using UnityEngine;
using GrimmCuiHarmony;
using HarmonyPlugin = Harmony.Plugins.Hud;
using GrimmCuiHarmony;
...
namespace HudHarmony
{
    ...
    public class HudMod : IHarmonyModHooks
    {
        public static HudMod Instance { get; private set; }
        ...
        public const string AppDomainApiKey = "Hud_ApiType";
        public const string CuiMarker = "HUD";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[Hud] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(HudMod)); }
            catch { }

            foreach (var cmd in new[] { "h" })
            {
                _chatCommandNames.Add(cmd);
                RegisterChatAliasConsole(cmd);
            }

            ChatSayBridge.Register("Hud", OnChatCommand);
            ...
            Debug.Log("[Hud] Harmony mod loaded. Chat: /h. Config: HarmonyConfig/Hud.json.");
        }
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[Hud] Permissions ready re-register: " + ex.Message);
            }
        }
        ...
        private void BindBetterChat()
        {
            try
            {
                ...
                mi?.Invoke(null, new object[] { "Hud", _betterChatTitle });
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[Hud] BetterChat bind: " + ex.Message);
            }
        }
        ...
        public void OnUnloaded(OnHarmonyModUnloadedArgs args)
        {
            ...
            InventoryLayerUiBridge.ClearAll();
            ...
            Instance = null;
            Debug.Log("[Hud] Harmony mod unloaded.");
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[Hud] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[Hud] Init: " + ex.Message); }
            ...
            _initCoroutine = null;
            Debug.Log("[Hud] Server initialized.");
        }
        ...
        public object Call(string method, params object[] args)
        {
            ...
            {
                Debug.LogWarning("[Hud] Call(" + method + "): " + ex.Message);
                return null;
            }
        }
        ...
        private static void InvokeChatMethod(HarmonyPlugin plugin, string methodName, BasePlayer player, string command, string[] args)
        {
            ...
            {
                Debug.LogWarning("[Hud] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[Hud] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedChatCommands()
        {
            ...
            {
                Debug.LogWarning("[Hud] RegisterAttributedChatCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[Hud] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[Hud] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            ...
            {
                Debug.LogWarning("[Hud] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            try
            {
                ...
                Debug.LogWarning("[Hud] cui.endtest HUD: command not registered: " + cmdName);
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[Hud] cui.endtest HUD: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[Hud] " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[Hud] cmd " + captured + ": " + ex.Message); }
                }
            };
