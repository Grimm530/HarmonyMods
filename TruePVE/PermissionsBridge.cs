 ...
 * Adapted from AdminMenu/PermissionsBridge.cs — namespace changed to TruePVEHarmony,
 * log tag changed to [TruePVE].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace TruePVEHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[TruePVE] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[TruePVE] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[TruePVE] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[TruePVE] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[TruePVE] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[TruePVE] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[TruePVE] Permissions Service bind: " + ex.Message);
            }
        }
