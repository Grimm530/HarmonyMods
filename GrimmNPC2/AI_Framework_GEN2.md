...
# Execution Boundary — What GrimmNPC2 Owns vs What the Game Owns

## Core Principle

GrimmNPC2 is a **support/runtime layer**, not a replacement AI system.

It exists to:

* provide identity, linkage, and metadata,
* expose safe access to GEN2 systems,
* supply profile/config surfaces,
* and assist plugins (e.g., BossMonster2),

**NOT** to reimplement or override stock GEN2 AI behavior.

## ✅ GrimmNPC2 SHOULD handle (Support Layer Responsibilities)

These are confirmed as correct responsibilities based on current implementation:

* Support/runtime identity and linkage (netId, owner, squad)
* Profile templates / preset registry
* FSM kind detection and current-state helpers
* Spawn-time configuration (name, home, initial destination)
* Swim policy (`LimitedTurnNavAgent.canSwim`)
* Sense tuning (range, memory, initial target)
* Shooting configuration (`NpcShootingComponent` flags)
* Damage scaling (incoming/outgoing, turret, hit areas)
* Cover helpers (wrappers, not behavior replacement)
* Stock GEN2 component accessors (Sense, FSM, Nav, Blackboard, etc.)
* Assist/group propagation primitives (bounded, explicit)

👉 These are **safe, intended extension points**.

## ❌ GrimmNPC2 MUST NOT replace or override (Game-Owned Systems)

The following systems are **owned by stock GEN2** and must remain authoritative:

* **FSM graphs** (`Scientist2FSM`, `Scientist2FSM_Heavy`, `Scientist2FSM_Shotgun`)
* **State transitions** (`FSMTransitionBase`, `InitShared()` wiring)
* **State execution logic** (`FSMStateBase` behaviors)
* **Sense ticking and target selection loops** (`SenseComponent.Tick`)
* **Navigation stepping and pathfinding** (`LimitedTurnNavAgent`)
* **Encounter timing and pacing** (`NPCEncounterTimer`)
* **Cover selection logic** (`NpcCoverManager`, cover states)
* **Bark / voiceline scheduling** (`NpcBarkManager`)

👉 These systems are **not extension points**. They are **engine-controlled behavior layers**.

## 🚫 DO NOT (Hard Rules)

Agents and developers must NOT:

* Replace or rebuild `Scientist2FSM` graphs
* Inject custom logic into FSM tick loops
* Override or spam `SetDestination` every frame
* Reimplement targeting logic already handled by `SenseComponent`
* Duplicate navigation logic from GEN1 (`BaseNavigator`, etc.)
* Force state transitions every tick
* Patch core AI loops unless absolutely required and justified
* Introduce fallback retry loops that fight stock systems

## ✅ DO (Correct Extension Strategy)

When new behavior is needed:

1. **Check if GEN2 already provides it**
...
2. If YES:
   ...
   * Use it via:
     ...
     * blackboard values
     * spawn configuration
     * or bounded helper methods

3. If PARTIAL:

   * Add:

     * metadata fields (e.g., in `CustomNpcData2`)
     * validation
     * thin wrappers

4. If MISSING:

   * Implement **support-layer helpers only**
   * Keep behavior external (e.g., BossMonster2)

## 🎯 Design Intent Going Forward

All remaining work on GrimmNPC2 should focus on:

> **NpcSpawn-style support surface parity — NOT behavior replacement**

This includes:

* profile/config completeness,
* metadata for combat/targeting/loot/group logic,
* helper APIs that interface cleanly with GEN2,

But explicitly excludes:

* rebuilding AI brains,
* replacing FSM logic,
* or duplicating stock systems.

## 🔍 Mental Model (for Agents)

| Layer                          | Owner | Responsibility                             |
| ------------------------------ | ----- | ------------------------------------------ |
| **GEN2 Engine (Rust.Ai.Gen2)** | Game  | AI behavior, FSM, movement, targeting      |
| **GrimmNPC2**                  | You   | Support layer, metadata, safe wrappers     |
| **BossMonster2 / plugins**     | You   | Custom gameplay, boss logic, orchestration |
...
| **Water / swim (FSM + agent)** | `LimitedTurnNavAgent` (`canSwim`, `IsSwimming`, `desiredSwimDepth`), `Trans_IsSwimming.cs`, `Trans_IsInWater_Slow.cs`, `State_TryAmbushUnderwater.cs` (croc/amphibian) — blueprint §6.5.4 |

---

## 6.5 Blueprint — `FSMStateBase`, scientist FSM graphs, `NPCFlankSpot`, and the listed `State_*` types

This section is the **behavior blueprint** for the state types you listed: what each offers at a high level, how they connect to **`FSMStateBase`**, how the three **scientist** FSMs differ, and where **`NPCFlankSpot`** fits. **V** = verified from `Scientist2FSM*.cs` / inspected state bodies in `Rust.Ai.Gen2`.

### 6.5.1 `FSMStateBase` — how every `State_*` participates (**V**)

| Mechanism | Role |
|-----------|------|
| **`Owner`** | Set to the pawn’s `BaseEntity`; states use `Senses`, `Agent`, `AnimPlayer`, `Blackboard` accessors. |
| **`Name`** | Defaults from type: `GetType().Name` with `State_` stripped (e.g. `State_Patrol` → **`Patrol`**); graphs often assign custom names to `State_Nothing` nodes (`WaitForNavMesh`, `Root`, `Has target`, …). |
| **`transitions` / `endTransitions`** | Built in `FSMComponent` subclasses (`InitShared`): **tick** transitions (`AddTickTransition`, `AddTickBranchingTrans`) and **end** transitions (`AddEndTransition`, `AddFailureTransition`) drive combat flow. |
| **`OnStateEnter` / `OnStateUpdate` / `OnStateExit`** | Per-frame behavior; return **`EFSMStateStatus`** for end-transitions (success/failure/none). |
| **`GetRootFSM<T>()`** | Access the owning `FSMComponent` (e.g. scientist-specific fields). |
| **`Clone()`** | Shallow clone used when graphs duplicate states (e.g. `search.Clone()` as `SearchPastLKP`). |

Plugins **do not** get a supported API to inject new states into stock `InitShared()` graphs without replacing the `FSMComponent` or Harmony-patching `InitShared` — treat graphs as **read-only contracts** unless you own the full custom FSM.

### 6.5.2 `NPCFlankSpot` + `State_Flank` (**V**)

| Piece | What it offers |
|-------|------------------|
| **`NPCFlankSpot`** | **Static** helper: `Find(LimitedTurnNavAgent, enemyPositionNs, directPath, pathToFlank, pathFromFlankToEnemy, …)` samples lateral offsets from the direct-to-enemy path, validates **NavMesh** paths and angular separation, refines a join point. **Expensive** (multiple `NavMeshPath`s, pooled corner lists). |
| **`State_Flank`** | Scientist combat state that allocates paths, calls **`NPCFlankSpot.Find`**, then drives **`LimitedTurnNavAgent`** along flank segments (inherits speed/bark hooks). |

**Used in:** default **`Scientist2FSM`** and **`Scientist2FSM_Shotgun`** — **not** in **`Scientist2FSM_Heavy`** (**V** from field lists in each `Scientist2FSM*.cs`).

### 6.5.3 Scientist FSM graphs — what each prefab wires (**V**)

Concrete **fields** on the FSM component are the authoritative list of which `State_*` instances exist on that graph. Transitions are built in `InitShared()` (dense; read the `.cs` for edge cases).

| State type | Role in combat loop | In `Scientist2FSM` (default) | In `Scientist2FSM_Heavy` | In `Scientist2FSM_Shotgun` |
|------------|---------------------|------------------------------|--------------------------|----------------------------|
| `State_PatrolIdle` | `idle` | Yes (`idle`) | Yes | Yes |
| `State_Patrol` | `patrol` | Yes | Yes | Yes |
| `State_Search` | `search` (+ cloned `SearchPastLKP`) | Yes | Yes | Yes |
| `State_ScientistRush` | `chase` — rush toward LKP (`State_MoveToTarget` derivative) | Yes | Yes | Yes |
| `State_DogFight` | Close strafe / reposition combat | Yes | **No** | Yes |
| `State_MoveToCoverHiddenFromTarget` | `goHide` — run to cover | Yes | **No** | Yes |
| `State_MoveToPointWithLosOnTarget` | `popOut` — LOS reposition | Yes | Yes | **No** |
| `State_StayInCover` | Hold cover | Yes | **No** | Yes |
| `State_Flank` | Flank path via `NPCFlankSpot` | Yes | **No** | Yes |
| `State_ScientistSurprised` | Opener / surprise | Yes | Yes | Yes |
| `State_ThrowGrenade` | Live grenade | Yes | **No** | **No** |
| `State_ScriptedNade` | Scripted grenade | Yes | **No** | **No** |
| `State_ScientistDead` | Scientist death handling | Yes (`dead`) | **No** | **No** |
| `State_Dead` | Generic death | **No** | Yes (`dead`) | Yes (`dead`) |
| `State_Nothing` | Structural grouping (named nodes: `Root`, `WaitForNavMesh`, `Has target`, …) | Yes (many) | Yes | Yes |

**High-level behavioral differences (**V**):**

- **`Scientist2FSM` (default):** Full humanoid kit — **cover** (`goHide`, `stayInCover`, `popOut`), **dogfight**, **flank**, **grenades** (throw + scripted), **`State_ScientistDead`**. Richest graph.
- **`Scientist2FSM_Heavy`:** **Slimmer** — no dogfight/cover/flank/grenade states on the field list; emphasizes **pop-out LOS** (`popOut`), **chase**, **search**, and a **stationary shooting** sub-node (`State_Nothing` named `Shoot while still`). **`Hurt`** can detonate **backpack** when `backPackHealth` depleted. Uses **`State_Dead`**, not `State_ScientistDead`.
- **`Scientist2FSM_Shotgun`:** Middle ground — **dogfight**, **cover** (`goHide`, `stayInCover`), **flank**, **no** `popOut` / grenade states on the field list; uses **`State_Dead`**.

Spawn the **`ScientistNPC2` prefab** that already carries the desired `Scientist2FSM*` component — **graphs are not swapped at runtime** by stock code.

### 6.5.4 Swimming / water — `LimitedTurnNavAgent`, transitions, `State_TryAmbushUnderwater` (**V**)

GEN2 **does not** use Gen1 `BaseNavigator` swim logic. Swimming is coordinated by **`LimitedTurnNavAgent`** + **FSM transitions** + optional **water states** on animal/amphibian graphs. Below summarizes what the listed types **do** in assembly; **which transitions are wired** into `Scientist2FSM*` depends on the prefab graph (inspect `InitShared` / prefab).

#### Agent surface (shared prerequisite)

| API / field | Role |
|-------------|------|
| **`LimitedTurnNavAgent.canSwim`** | Serialized flag: when **false**, swim-style branches that gate on swimming should not apply; `Trans_IsSwimming` always fails (see below). |
| **`LimitedTurnNavAgent.IsSwimming`** | **Getter** (entity flag **`BaseEntity.Flags.Reserved1`**): true when the agent is in the swimming movement mode. |
| **`LimitedTurnNavAgent.desiredSwimDepth`** | **`ResettableFloat`** — underwater states (e.g. ambush) **set** a target depth on enter and **`Reset()`** on exit. |

#### `Trans_IsSwimming` (**V**)

- Extends **`FSMTransitionBase`** (evaluated on the normal transition pass).
- **`EvaluateInternal`:** returns **`Agent.canSwim && Agent.IsSwimming`**.
- **Meaning:** “allowed to swim **and** currently flagged as swimming.” If `canSwim` is false, transition is **false** even in deep water.

#### `Trans_IsInWater_Slow` (**V**)

- Extends **`FSMSlowTransitionBase`** — evaluated on a **throttled interval** (cheaper than per-tick for expensive checks).
- **`EvaluateAtInterval`:**  
  - If **`Agent.canSwim`**: returns **`Agent.IsSwimming`** (same idea as `Trans_IsSwimming`, but slow-tick).  
  - Else: returns **`WaterLevel.GetWaterDepth(transform.position, waves: false, volumes: false) >= 0.3f`** — **non-swimmers** still detect “standing in water” by **depth threshold** (e.g. wading / policy), without using the swim flag path.

#### `State_TryAmbushUnderwater` (**V**, croc / underwater AI — **not** on stock `Scientist2FSM` field lists)

- **Enter:** sets **`Agent.desiredSwimDepth.Value = 3f`**, records **`divePosition`**, then **`FindNewUnderwaterWaitingPosition()`**.
- **Update:** if not following a path, retries **FindNewUnderwaterWaitingPosition** (picks new wait points).
- **Exit:** **`Agent.ResetPath()`**, **`desiredSwimDepth.Reset()`**.
- **FindNewUnderwaterWaitingPosition:** samples positions in a **donut** around **`NavPosition`** (`distanceRange` default **10–20m**), optionally **reorders** toward **`divePosition`** if farther than **`maxDistFromDivingPoint` (50m)**; shuffles otherwise. For each candidate: **`SamplePosition`**, **`Agent.IsInWater(sample)`**, **`SetDestination(sample)`**. Sets speed **`Run`** when not swimming, **`Sneak`** when **`IsSwimming`**.
- **Use:** **underwater stalk / ambush** positioning for amphibious NPCs — **orthogonal** to humanoid scientist combat unless you add a custom FSM that references this state.

#### Related transitions (see §5.3 / §7 buckets)

Other stock GEN2 water-related transitions include e.g. **`Trans_IsTargetInWater`**, **`Trans_IsTargetTooFarFromWater`**, **`Trans_TooFarFromWater`**, **`State_LandOrSwimAttack`** — **croc / water-combat** heavy; scientists may only hit some of these on mixed maps depending on prefab wiring.

#### GrimmNPC2 alignment

Support-layer policy mirrors **`canSwim`** onto **`LimitedTurnNavAgent`** at spawn; **does not** replace FSM water transitions. See `HarmonyMods/GrimmNPC2/GEN2_PARITY_NOTES.md`.

### 6.5.5 Master catalog — listed `State_*` types (what each offers + scientist FSM usage)

**Legend — scientist columns:** **D** = `Scientist2FSM`, **H** = `Scientist2FSM_Heavy`, **S** = `Scientist2FSM_Shotgun`, **—** = not a field on those three graphs (may still be used by **predators / croc / other** FSMs or future content). **Display name** = default `FSMStateBase.Name` unless a graph sets `Name` explicitly (`State_Nothing` nodes often do).

| File / type | Display name (typical) | What it offers | D | H | S |
|-------------|-------------------------|----------------|---|---|---|
| `State_ApproachFire.cs` | ApproachFire | Move toward / interact with **fire** stimulus; environmental. | — | — | — |
| `State_ApproachFood.cs` | ApproachFood | Move toward **food** target; animal feeding. | — | — | — |
| `State_Attack.cs` | Attack | **Melee-style** attack: extends `State_PlayAnimationRM`, applies **damage** after delay (`Damage`, `DamageType`). Animal/melee. | — | — | — |
| `State_AttackUnreachable.cs` | AttackUnreachable | Attack pattern when target **cannot be reached** by normal path. | — | — | — |
| `State_AttackUnreachableWarped.cs` | AttackUnreachableWarped | Variant with **warp** / recovery behavior for unreachable targets. | — | — | — |
| `State_AttackWithTracking.cs` | AttackWithTracking | **Tracking** melee / commit attack (follows target motion). | — | — | — |
| `State_Bark.cs` | Bark | Play **bark** / VO-driven reaction (`NpcBarkComponent`). | — | — | — |
| `State_BringFoodBackToWater.cs` | BringFoodBackToWater | Croc / amphibian — carry food **back to water**. | — | — | — |
| `State_Circle.cs` | Circle | **Circle** prey at radius (predator). | — | — | — |
| `State_CircleDynamic.cs` | CircleDynamic | Dynamic / adjustable circling. | — | — | — |
| `State_CrocBreakFoundation.cs` | CrocBreakFoundation | **Melee structure** damage (foundations). | — | — | — |
| `State_CrocCharge.cs` | CrocCharge | **Charge** attack. | — | — | — |
| `State_CrocIntimidate.cs` | CrocIntimidate | Intimidate telegraph / stance. | — | — | — |
| `State_CrocTurn.cs` | CrocTurn | In-place / tight **turn** for croc. | — | — | — |
| `State_Dead.cs` | Dead | **Generic** death state (used by Heavy/Shotgun scientists). | — | **Y** | **Y** |
| `State_DeadlyAttack.cs` | DeadlyAttack | High-commit **finisher** attack (predator). | — | — | — |
| `State_DogFight.cs` | DogFight | **Close combat strafe**: recalculates offset paths, toggles lateral dodge (`shouldStopAtDestination = false`). Default scientist + Shotgun. | **Y** | — | **Y** |
| `State_DragCorpse.cs` | DragCorpse | Drag **corpse** (wolf / predator). | — | — | — |
| `State_EatFood.cs` | EatFood | Eat / consume food at location. | — | — | — |
| `State_FastSneak.cs` | FastSneak | **Fast sneak** movement profile. | — | — | — |
| `State_Flank.cs` | Flank | **Flank navigation** using `NPCFlankSpot.Find` + multi-segment paths. Default + Shotgun. | **Y** | — | **Y** |
| `State_Flee.cs` | Flee | Flee from threat (generic). | — | — | — |
| `State_FleeFire.cs` | FleeFire | Flee from **fire**. | — | — | — |
| `State_FleeToHide.cs` | FleeToHide | Flee toward **concealment**. | — | — | — |
| `State_GoBackToWater.cs` | GoBackToWater | Return to **water** (croc). | — | — | — |
| `State_Growl.cs` | Growl | Predator **growl** (audio/stance). | — | — | — |
| `State_Howl.cs` | Howl | Wolf **howl**. | — | — | — |
| `State_Hurt.cs` | Hurt | **Hit reaction** (short pain / stagger). | — | — | — |
| `State_HurtWithAdditive.cs` | HurtWithAdditive | Hurt with **additive** animation layer. | — | — | — |
| `State_Intimidated.cs` | Intimidated | Intimidated / suppressed stance. | — | — | — |
| `State_LandOrSwimAttack.cs` | LandOrSwimAttack | **Amphibious** attack (land vs swim). | — | — | — |
| `State_MoveToBreakFoundation.cs` | MoveToBreakFoundation | Move to **raid** / break building target (croc). | — | — | — |
| `State_MoveToCoverHiddenFromTarget.cs` | MoveToCoverHiddenFromTarget | **Seek cover** hidden from threat (`searchRadius`, sprint ramp, `NpcCoverManager` / cover points). Default + Shotgun. | **Y** | — | **Y** |
| `State_MoveToLastReachablePointNearTarget.cs` | MoveToLastReachablePointNearTarget | Move to **last reachable** nav point near target when path fails. | — | — | — |
| `State_MoveToPointWithLosOnTarget.cs` | MoveToPointWithLosOnTarget | **Reposition for LOS** on target (“pop out”). Default + Heavy. | **Y** | **Y** | — |
| `State_MoveToTarget.cs` | MoveToTarget | **Abstract** chase base: subclasses implement `GetMoveDestination`. On scientists, **`State_ScientistRush`** is the concrete type (`chase` field) — not a separate `State_MoveToTarget` field on the FSM. | — | — | — |
| `State_Nothing.cs` | Nothing* | **No-op** state; used as **graph container** with custom `Name` (*e.g.* `WaitForNavMesh`, `Root`, `Has target`). | **Y** | **Y** | **Y** |
| `State_Observe.cs` | Observe | Idle **observe** / focus. | — | — | — |
| `State_Patrol.cs` | Patrol | **Patrol** waypoints / route. All three scientists. | **Y** | **Y** | **Y** |
| `State_PatrolIdle.cs` | PatrolIdle | **Idle** between patrol legs. All three scientists. | **Y** | **Y** | **Y** |
| `State_PlayAnimation.cs` | PlayAnimation | One-shot **animation** play. | — | — | — |
| `State_PlayAnimationBase.cs` | PlayAnimationBase | Base for timed / gated animation states. | — | — | — |
| `State_PlayAnimationRM.cs` | PlayAnimationRM | **Root-motion** one-shot animation. | — | — | — |
| `State_PlayAnimLoop.cs` | PlayAnimLoop | **Looping** animation. | — | — | — |
| `State_PlayRandomAnimation.cs` | PlayRandomAnimation | Random pick from animation set. | — | — | — |
| `State_Roam.cs` | Roam | **Roam / wander** (open-world). | — | — | — |
| `State_Roar.cs` | Roar | Predator **roar**. | — | — | — |
| `State_ScientistDead.cs` | ScientistDead | **Scientist-specific** death (FX/flow). Default scientist only among the three. | **Y** | — | — |
| `State_ScientistRush.cs` | ScientistRush | **Rush to LKP** with zone + shooting hooks; extends `State_MoveToTarget`. All three scientists (`chase`). | **Y** | **Y** | **Y** |
| `State_ScientistSurprised.cs` | ScientistSurprised | **Surprise** opener branch. All three scientists. | **Y** | **Y** | **Y** |
| `State_ScriptedNade.cs` | ScriptedNade | **Scripted** grenade throw sequence. Default scientist only (among three). | **Y** | — | — |
| `State_Search.cs` | Search* | **Search** LKP area (`searchRadius`, iterations, optional `predict`); *clone* may be named `SearchPastLKP`. All three scientists. | **Y** | **Y** | **Y** |
| `State_StayInCover.cs` | StayInCover | **Hold cover**, peek / re-evaluate LOS. Default + Shotgun. | **Y** | — | **Y** |
| `State_ThrowGrenade.cs` | ThrowGrenade | **Grenade** throw combat state. Default scientist only (among three). | **Y** | — | — |
| `State_TryAmbushUnderwater.cs` | TryAmbushUnderwater | **Underwater** ambush: `desiredSwimDepth`, donut EQS samples, `IsInWater` + `SetDestination` — **§6.5.4**. | — | — | — |
| `State_WolfHurt.cs` | WolfHurt | Wolf-specific **hurt** branch. | — | — | — |

**Note:** Rows marked **—** across D/H/S are still part of the **shared GEN2 library**; they ship for **non-scientist** NPCs (`Wolf2`, `Crocodile`, `Tiger`, …) or for **custom** FSMs you add yourself.
...
| **Swimming / water** | `Trans_IsSwimming`, `Trans_IsInWater_Slow`, `Trans_IsTargetInWater`, `Trans_IsTargetTooFarFromWater`, `Trans_TooFarFromWater`, `State_LandOrSwimAttack`, `State_TryAmbushUnderwater` | Partial | **`Trans_IsSwimming` / `Trans_IsInWater_Slow`** tie to **`LimitedTurnNavAgent.canSwim` + `IsSwimming`** (and slow depth for non-swimmers) — see **§6.5.4**. Underwater **ambush** is **`State_TryAmbushUnderwater`** (croc-style). Scientists may use water transitions on mixed maps; **croc-heavy** otherwise |
