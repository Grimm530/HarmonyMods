...
namespace PrivateMessagesHarmony
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[PrivateMessages] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[PrivateMessages] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[PrivateMessages] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[PrivateMessages] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[PrivateMessages] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[PrivateMessages] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[PrivateMessages] Permissions Service bind: " + ex.Message);
            }
        }
