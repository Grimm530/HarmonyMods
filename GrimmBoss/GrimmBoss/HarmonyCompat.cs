// Legacy -> Harmony compatibility shims for the ported GrimmBoss plugin.
...
namespace Harmony.Core
{
    ...
    public static class HarmonyModInterface
    {
        ...
        private static Type GetModRuntimeInterfaceType()
        {
            if (_oxideInterfaceResolved)
                return _oxideInterfaceType;
            _oxideInterfaceResolved = true;
            _oxideInterfaceType = Type.GetType("Harmony.Core.HarmonyModInterface, Harmony.Core");
            return _oxideInterfaceType;
        }

        /// <summary>
        /// Forwards select hooks to Harmony mods (Kits GiveKit). Otherwise null — no Harmony hook bus.
        /// </summary>
        public static object CallHook(string hook, params object[] args)
        {
            ...
            // Cache the lookup: HarmonyLoader logs every AssemblyResolve, and Harmony.Core is not installed.
            try
            {
                Type t = GetModRuntimeInterfaceType();
                if (t != null)
                {
    ...
    public class DataFileSystem
    {
        ...
        /// <summary>Harmony-compatible: returns full paths of *.json files under the given relative folder.</summary>
