...
| **Ready callbacks** | `RegisterReadyCallback(Action)` / `UnregisterReadyCallback(Action)` — consumers re-register after Permissions loads |
...
On load, Permissions auto-registers (lists in AdminMenu / `perm show`) from:
...
```text
harmony.reload Permissions
```

You should see: `[Permissions] Server admin bypass-all=False …` and `Invoking N ready callback(s)…`.

Consumer mods (AdminMenu, Kits, Backpacks, …) **auto-rebind** when `Permissions_Generation` changes. You do **not** need to `harmony.reload` those mods after reloading Permissions. Mods that register permissions subscribe a ready callback so their `RegisterPermission` / group grants run again on Permissions load/reload.

Load order at startup is filesystem-defined; lazy bind + ready callbacks are the failsafe when Permissions loads after (or is reloaded without) consumers.
...
```powershell
.\.cursor\HarmonyMods\Permissions\build.ps1
```

Load **Permissions** before consumers when possible. Consumers resolve the API lazily and **auto-rebind** after `harmony.reload Permissions` via `Permissions_Generation` + ready callbacks — you do not need to reload AdminMenu / Kits / etc. after Permissions.
