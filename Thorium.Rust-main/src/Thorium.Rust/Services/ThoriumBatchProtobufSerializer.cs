...
/// The schema is documented in docs/anticheat-send-data.md.
/// </summary>
internal static class ThoriumBatchProtobufSerializer
{
    public static byte[] Serialize(ThoriumBatch batch, DataHandlerPayload? caches = null)
    {
    ...
    private static void WriteThoriumBatch(ProtobufWireWriter w, ThoriumBatch batch, DataHandlerPayload? caches)
    {
        ...
        if (caches != null)
        {
            WriteOptionalBytes(w, 9, caches.PacketCacheBytes);
            WriteOptionalBytes(w, 10, caches.PvpCacheBytes);
            WriteOptionalBytes(w, 11, caches.JoinCacheBytes);
            WriteOptionalBytes(w, 12, caches.DamageCacheBytes);

            WriteOptionalInt64(w, 13, caches.TotalPackets);
            WriteOptionalInt64(w, 14, caches.TotalPvpPackets);
            WriteOptionalInt64(w, 15, caches.TotalJoinPackets);
            WriteOptionalInt64(w, 16, caches.TotalDamagePackets);

            WriteOptionalBytes(w, 17, caches.EntityCacheBytes);
            WriteOptionalInt64(w, 18, caches.TotalEntityPackets);
        }
    }
    ...
    private static void WritePlayerSnapshot(ProtobufWireWriter w, PlayerSnapshot s)
    {
        ...
        if (s.CombatData.IsAiming || s.CombatData.IsAttacking || s.CombatData.IsMounted ||
        ...
        WriteFloatIfNonZero(w, 29, s.InputButtons);
