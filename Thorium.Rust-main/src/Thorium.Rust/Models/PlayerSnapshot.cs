...
public class PlayerSnapshot
{
    ...
    public static PlayerSnapshot Create(Vector3 position, BasePlayer player, SnapshotTypeEnums type,
        CombatData combatData = default, Vector3? velocity = null, bool isGrounded = false, InputMessage? inputMessage = null)
    {
    ...
    public PlayerSnapshot ResetState()
    {
        // Reset fields that are only set by specific callers
        // Without this, pooled snapshots reused for events would
        // carry stale data from a previous PlayerTick belonging to a different player.
        EyesViewMode = false;
        ...
        LookDirZ = 0f;
        PoseType = 0;
        InheritedVelocityX = 0f;
        InheritedVelocityY = 0f;
        InheritedVelocityZ = 0f;
        Ducking = 0f;
        IsDead = false;
        OnLadder = false;
        AverageLatency = 0f;
