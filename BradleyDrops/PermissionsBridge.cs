 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to BradleyDropsHarmony,
 * log tag changed to [BradleyDrops].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace BradleyDropsHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[BradleyDrops] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[BradleyDrops] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[BradleyDrops] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[BradleyDrops] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[BradleyDrops] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[BradleyDrops] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[BradleyDrops] Permissions Service bind: " + ex.Message);
            }
        }
