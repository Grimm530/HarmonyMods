 ...
 * Adapted from HitMarkers/PermissionsBridge.cs — namespace changed to HitMarkersHarmony,
 * log tag changed to [HitMarkers].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace HitMarkersHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[HitMarkers] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[HitMarkers] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[HitMarkers] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[HitMarkers] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[HitMarkers] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[HitMarkers] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[HitMarkers] Permissions Service bind: " + ex.Message);
            }
        }
