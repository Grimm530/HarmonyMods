...
Harmony port of **ZombieHorde** 0.6.351. Spawns via **GrimmNPC** reflection (`SpawnNpc` + `NpcConfig`). No Grimm.ChaosNPC / Harmony.Core references.
...
|-------|--------|
| **Name** | ZombieHorde |
| **Source** | `.cursor/Harmony.Plugins.Cant-Use/ZombieHorde.cs` |
| **Type** | Harmony mod (`IHarmonyModHooks`) |
| **Entry** | `ZombieHorde.ZombieHordeHarmonyEntry` |
| **Config** | `HarmonyConfig/ZombieHorde.json` (migrates from `legacy/config/ZombieHorde.json`) |
...
**Prefer the space form** (0Permissions). Dotted `perm.grant` is Oxide’s alias and only works after legacy registration succeeds:
...
- `[ZombieHorde] Linked to legacy permission library (perm.grant).`
...
- `horde addloadout <kit> [kit…]` requires Harmony **Kits** (`Kits_Plugin` / `GetKitInfo`) or legacy Kits
...
| `RaidingZombies.cs` | original RaidingZombies 3.2.1 (C4/rockets / TC raid) |
