 ...
 * Local DTOs so AdminMenu does not reference PermissionsHarmony types.
 */
using System;
...
namespace StackManagerHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        private static MethodInfo _svcGetPermissions;
        private static MethodInfo _svcGetGroups;
        private static MethodInfo _svcGetUsersInGroup;
        private static MethodInfo _svcGetUserData;
        private static MethodInfo _svcGetGroupData;
        private static MethodInfo _svcRemoveGroup;
        private static MethodInfo _svcSetGroupParent;
        private static MethodInfo _svcGroupHas;
        private static MethodInfo _svcUserHas;
        private static MethodInfo _svcRegister;
        private static MethodInfo _svcExists;
        private static MethodInfo _svcGrantUser;
        private static MethodInfo _svcRevokeUser;
        private static MethodInfo _svcGrantGroup;
        private static MethodInfo _svcRevokeGroup;
        private static MethodInfo _svcAddUserGroup;
        private static MethodInfo _svcRemoveUserGroup;
        private static MethodInfo _svcUserHasGroup;
        ...
        /// <summary>Subscribe to Permissions load/reload so AdminMenu can re-register perms.</summary>
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[StackManager] RegisterReadyCallback: " + ex.Message);
            }

            // Permissions not loaded yet — stash in AppDomain list for later InvokeReadyCallbacks.
            try
            {
            ...
            {
                Debug.LogWarning("[StackManager] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
        ...
        private static void ClearBind()
        {
            ...
            _svcGetUserData = _svcGetGroupData = _svcRemoveGroup = _svcSetGroupParent = null;
            _svcGroupHas = _svcUserHas = _svcRegister = _svcExists = null;
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[StackManager] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[StackManager] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[StackManager] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[StackManager] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            try
            {
                ...
                _svcRemoveGroup = st.GetMethod("RemoveGroup", I, null, new[] { typeof(string) }, null);
                _svcSetGroupParent = st.GetMethod("SetGroupParent", I, null, new[] { typeof(string), typeof(string) }, null);
            ...
            {
                Debug.LogWarning("[StackManager] Permissions Service bind: " + ex.Message);
            }
        }
        ...
        public static string[] GetPermissions()
        {
            ...
            try
            {
                if (_service != null && _svcGetPermissions != null)
                    return ToStringArray(_svcGetPermissions.Invoke(_service, null));
            }
            catch { }
            return Array.Empty<string>();
        }
        ...
        public static string[] GetGroups()
        {
            ...
            try
            {
                if (_service != null && _svcGetGroups != null)
                    return ToStringArray(_svcGetGroups.Invoke(_service, null));
            }
            catch { }
            return Array.Empty<string>();
        }
        ...
        public static string[] GetUsersInGroup(string group)
        {
            ...
            try
            {
                if (_service != null && _svcGetUsersInGroup != null)
                    return ToStringArray(_svcGetUsersInGroup.Invoke(_service, new object[] { group }));
            }
            catch { }
            return Array.Empty<string>();
        }
        ...
        public static string[] GetUserGroups(string playerId)
        {
            ...
            try
            {
                if (_getUserGroups != null)
                    return ToStringArray(_getUserGroups.Invoke(null, new object[] { playerId }));
            }
            catch { }
            var data = GetUserData(playerId);
            if (data?.Groups == null || data.Groups.Count == 0)
                return Array.Empty<string>();
            var arr = new string[data.Groups.Count];
            data.Groups.CopyTo(arr);
            return arr;
        }
        ...
        public static bool AddUserGroup(string playerId, string group) =>
            InvokeBool(_addUserGroup, _svcAddUserGroup, playerId, group);

        public static bool RemoveUserGroup(string playerId, string group) =>
            InvokeBool(_removeUserGroup, _svcRemoveUserGroup, playerId, group);
        ...
        public static bool RemoveGroup(string group)
        {
            ...
            try
            {
                if (_service != null && _svcRemoveGroup != null &&
                    _svcRemoveGroup.Invoke(_service, new object[] { group }) is bool ok)
                    return ok;
            }
            catch { }
            return false;
        ...
        public static bool SetGroupParent(string group, string parent)
        {
            ...
            try
            {
                if (_service != null && _svcSetGroupParent != null &&
                    _svcSetGroupParent.Invoke(_service, new object[] { group, parent ?? "" }) is bool ok)
                    return ok;
            }
            catch { }
            return false;
        ...
        {
            var data = GetGroupData(group);
            if (data?.Perms == null || data.Perms.Count == 0)
                return Array.Empty<string>();
            var arr = new string[data.Perms.Count];
            data.Perms.CopyTo(arr);
            return arr;
        }

        public static bool GrantUserPermission(string playerId, string perm) =>
            InvokeBool(_grantUser, _svcGrantUser, playerId, perm);

        public static bool RevokeUserPermission(string playerId, string perm) =>
            InvokeBool(_revokeUser, _svcRevokeUser, playerId, perm);
            ...
            try
            {
                if (_service != null && _svcGetUserData != null)
                {
                    ...
                    return new UserData
                    {
                        ...
                        Groups = ReadStringHashSet(raw, "Groups")
                    };
                }
            }
            catch { }
        ...
        {
            if (groups == null || string.IsNullOrEmpty(perm)) return false;
            foreach (var g in groups)
            {
                if (GroupHasPermission(g, perm))
                    return true;
            }
            return false;
        }
