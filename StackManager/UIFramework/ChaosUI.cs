...
public class ChaosUI
{
	...
	public static void Show(BasePlayer player, BaseContainer baseContainer, bool recycle = true)
	{
		...
		// Clients only forward ConsoleGen commands; bridge stackmanager.callback through cui.endtest STACKMANAGER.
		arg = arg.Replace("\"command\":\"stackmanager.callback ", "\"command\":\"cui.endtest STACKMANAGER stackmanager.callback ");
		arg = arg.Replace("\"command\": \"stackmanager.callback ", "\"command\": \"cui.endtest STACKMANAGER stackmanager.callback ");
		// Also support stacksextended.callback if Title remains StacksExtended during port.
		arg = arg.Replace("\"command\":\"stacksextended.callback ", "\"command\":\"cui.endtest STACKMANAGER stacksextended.callback ");
		arg = arg.Replace("\"command\": \"stacksextended.callback ", "\"command\": \"cui.endtest STACKMANAGER stacksextended.callback ");
		SendInfo sendInfo = new SendInfo
		{
			connection = player.net.connection
		};
		CommunityEntity.ServerInstance.ClientRPC(RpcTarget.SendInfo("AddUI", sendInfo), arg);
		if (recycle)
		{
	...
	{
		List<List<BaseCuiComponent>> obj = Pool.Get<List<List<BaseCuiComponent>>>();
	...
	public static string ToJson(BaseContainer baseContainer)
	{
		...
		try
		{
			...
			using (StringWriter textWriter = new StringWriter(m_StringBuilder))
			{
				...
				foreach (BaseContainer child in rootContainer.Children)
				{
					child.Element.WriteJson(jsonWriter, null);
				}
	...
	public static void Destroy(BasePlayer player, string panel)
	{
		if ((bool)player && player.net != null)
		{
			...
			serverInstance.ClientRPC(RpcTarget.SendInfo("DestroyUI", sendInfo), panel);
		}
	}
	...
	public static void Destroy(BasePlayer player, BaseContainer baseContainer)
	{
		if ((bool)player && player.net != null)
		{
			...
			serverInstance.ClientRPC(RpcTarget.SendInfo("DestroyUI", sendInfo), baseContainer.RootContainer.Element.Name);
		}
	}
	...
	public static void SendUpdates(BasePlayer player, IEnumerable<UpdateComponent> updates)
	{
		...
		try
		{
			using (StringWriter textWriter = new StringWriter(m_StringBuilder))
			{
				using JsonWriter jsonWriter = new JsonTextWriter(textWriter);
				jsonWriter.WriteStartArray();
				foreach (UpdateComponent update in updates)
				{
