...
namespace InventoryCleaner
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[InventoryCleaner] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[InventoryCleaner] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[InventoryCleaner] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[InventoryCleaner] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[InventoryCleaner] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[InventoryCleaner] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[InventoryCleaner] Permissions Service bind: " + ex.Message);
            }
        }
