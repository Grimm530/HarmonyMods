 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to JetPackHarmony,
 * log tag changed to [JetPack].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace JetPackHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[JetPack] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[JetPack] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[JetPack] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[JetPack] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[JetPack] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[JetPack] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[JetPack] Permissions Service bind: " + ex.Message);
            }
        }
