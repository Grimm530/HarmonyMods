...
using UnityEngine;
using GrimmCuiHarmony;
using HarmonyPlugin = Harmony.Plugins.JetPack;
using GrimmCuiHarmony;
...
namespace JetPackHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("JetPack_Runner");
    ...
    public class JetPackMod : IHarmonyModHooks
    {
        public static JetPackMod Instance { get; private set; }
        ...
        public const string AppDomainApiKey = "JetPack_ApiType";
        public const string CuiMarker = "JETPACK";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[JetPack] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(JetPackMod)); }
            catch { }

            RegisterAttributedChatCommands();
            ChatSayBridge.Register("JetPack", OnChatCommand);
            ...
            Debug.Log("[JetPack] Harmony mod loaded. Config: HarmonyConfig/JetPack.json");
        }
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[JetPack] Permissions ready: " + ex.Message);
            }
        }
        ...
        public void OnUnloaded(OnHarmonyModUnloadedArgs args)
        {
            ...
            Instance = null;
            Debug.Log("[JetPack] Harmony mod unloaded.");
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[JetPack] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            catch (Exception ex) { Debug.LogWarning("[JetPack] ResolvePluginReferences: " + ex.Message); }
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[JetPack] Init: " + ex.Message); }
            try { plugin.CallLoaded(); }
            catch (Exception ex) { Debug.LogWarning("[JetPack] Loaded: " + ex.Message); }
            try { plugin.CallOnServerInitialized(); }
            catch (Exception ex) { Debug.LogError("[JetPack] OnServerInitialized: " + ex); }
            RefreshDynamicCommands();
            _initCoroutine = null;
            Debug.Log("[JetPack] Server initialized.");
        }
        ...
        private static void InvokeChatMethod(HarmonyPlugin plugin, string methodName, BasePlayer player, string command, string[] args, bool attributedOnly = false)
        {
            ...
            {
                Debug.LogWarning("[JetPack] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[JetPack] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[JetPack] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[JetPack] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            try
            ...
            {
                Debug.LogWarning("[JetPack] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                Debug.LogWarning("[JetPack] cui.endtest JETPACK: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[JetPack] " + methodName + ": " + (ex.InnerException ?? ex).Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[JetPack] cmd " + captured + ": " + ex.Message); }
                }
            };
