 ...
 * Adapted from ZoneManager/PermissionsBridge.cs — namespace changed to ZoneManagerHarmony,
 * log tag changed to [ZoneManager].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace ZoneManagerHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[ZoneManager] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[ZoneManager] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[ZoneManager] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[ZoneManager] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[ZoneManager] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[ZoneManager] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[ZoneManager] Permissions Service bind: " + ex.Message);
            }
        }
