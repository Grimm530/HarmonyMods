...
namespace Oxide.Plugins
{
    ...
    public partial class ZoneManager : RustPlugin
    {
        #region Fields
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo hitinfo)
        {
            ...
            // Check if Initiator is an NPC (even if not a BasePlayer)
            bool isNpcAttacker = hitinfo.Initiator != null && (hitinfo.Initiator is BaseNpc || hitinfo.Initiator is NPCPlayer || (hitinfo.Initiator is BasePlayer npcPlayer && IsNpc(npcPlayer)));
            ...
            {                
                ...
                if (attacker)
                {
                    ...
                    if (HasPlayerFlag(victim, ZoneFlags.PvpGod))
                    {
                        ...
                        // Allow NPCs to damage players with PvpGod
                        // For BaseNpc entities (like ScientistNPC), always allow damage
                        // For NPC players (BasePlayer.IsNpc), check the config
                        if (isNpcAttacker)
                            return null; // BaseNpc entities can always damage players
                        if (IsNpc(attacker) && Configuration.NPCHurtPvpGod)
                            return null; // NPC players need config enabled
                ...
                else if (isNpcAttacker)
                {
                    // NPC attacker (not a BasePlayer) attacking a player
                    if (IsNpc(victim))
                        return null;

                    if (HasPlayerFlag(victim, ZoneFlags.PvpGod))
                    {
                        // Always allow NPCs to damage players with PvpGod (NPCs are not players)
                        // The NPCHurtPvpGod config is for NPC players (BasePlayer.IsNpc), not BaseNpc entities
                        return null;
                    }
                }
                
                if (HasPlayerFlag(victim, ZoneFlags.PveGod) && !IsNpc(victim))
                    return true;
        ...
        private void InitializeZones()
        {
            if (zonesInitialized)
                return;
            if (storedData?.definitions == null)
                return;
