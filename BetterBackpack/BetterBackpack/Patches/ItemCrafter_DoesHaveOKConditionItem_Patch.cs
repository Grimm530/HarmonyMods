...
/// <summary>
/// When Retrieval is enabled, include backpack when checking OK-condition items for crafting.
/// </summary>
[HarmonyPatch(typeof(ItemCrafter), "DoesHaveOKConditionItem")]
internal class ItemCrafter_DoesHaveOKConditionItem_Patch
{
    [HarmonyPostfix]
    private static void Postfix(ItemCrafter __instance, int item, int iAmount, ref bool __result)
    {
        ...
        if (list != null)
        {
            for (var i = 0; i < list.Count; i++)
                have += list[i].GetOkConditionAmount(item, onlyUsableAmounts: true);
        }
        have += backpack.contents.GetOkConditionAmount(item, onlyUsableAmounts: true);
