...
{
    /// <summary>
    /// Harmony mod: registration API and patches for custom <see cref="ScientistNPC"/> instances (fixed skinID).
    ...
    public class GrimmNPC : IHarmonyModHooks
    {
        ...
        /// </summary>
        public static readonly ulong CUSTOM_NPC_SKIN_ID = 3793751435UL;
        private static bool IsLegacyCustomNpcSkin(ulong skin) => skin == 11162132011012UL;
        public static bool IsCustomNpcSkin(ulong skin) => skin == CUSTOM_NPC_SKIN_ID || IsLegacyCustomNpcSkin(skin);
        ...
        /// <summary>
        /// Short-lived spawn/init swim gate keyed by netId.
        /// While active, early spawn-time logic should avoid nav Stop/Pause/Resume transitions.
        /// </summary>
        private readonly Dictionary<ulong, float> _spawnSwimInitUntil = new Dictionary<ulong, float>(256);
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            if (ShouldLogLifecycle())
            {
                ...
                UnityEngine.Debug.Log($"[GrimmNPC] Config loaded: EnableDebugLogging={cfg.EnableDebugLogging}, EnableSwimmingDebug={cfg.EnableSwimmingDebug}");
                if (!cfg.EnableSwimmingDebug && cfg.EnableDebugLogging)
                    UnityEngine.Debug.Log("[GrimmNPC] EnableSwimmingDebug=false — set true in HarmonyConfig/GrimmNPC.json for [GrimmNPC SwimDebug] (CustomNpcData.CanSwim defaults true; set false to opt out).");
        ...
        {
            bool logLife = _config != null && (_config.DebugLifecycle || _config.EnableDebugLogging);
            SaveData();
            _npcs.Clear();
            _usedNpcUserIds.Clear();
            _pending.Clear();
            _spawnSwimInitUntil.Clear();
            Instance = null;
            if (logLife)
        ...
        {
            return entity != null && IsCustomNpcSkin(entity.skinID);
        }

        /// <summary>
        /// True when the scientist has a chase-relevant target in brain event memory.
        /// Checks <see cref="BaseAIBrain.Events.CurrentInputMemorySlot"/> first (any non-destroyed entity, same as vanilla nav intent),
        /// then any slot 0–7 for a real <see cref="BasePlayer"/> — BossMonster writes the player into Chase/Combat/Attack input slots
        /// while the brain can still be in Roam/Dismounted with a different current slot, which previously made GrimmNPC treat NPCs as idle.
        /// </summary>
        public static bool HasBrainEventMemoryCombatTarget(ScientistNPC npc)
        {
            var mem = npc?.Brain?.Events?.Memory?.Entity;
            if (mem == null) return false;
            int cur = npc.Brain.Events.CurrentInputMemorySlot;
            if (cur >= 0 && cur < 8)
            {
                BaseEntity e = mem.Get(cur);
                if (e != null && !e.IsDestroyed) return true;
            }
            const ulong steamIdFloor = 76561197960265728UL;
            for (int i = 0; i < 8; i++)
            {
                BaseEntity e = mem.Get(i);
                if (e == null || e.IsDestroyed) continue;
                if (e is BasePlayer p && !p.IsNpc && p.userID > steamIdFloor) return true;
            }
            return false;
        }
        ...
        /// <summary>Throttled navmesh line spam (legacy master or granular).</summary>
        public static bool ShouldLogDynamicNavMesh()
        {
            var c = GetConfig();
            return c != null && (c.EnableDebugLogging || c.DebugDynamicNavMesh);
        }

        public static bool ShouldLogPeriodicSnapshot()
        {
            var c = GetConfig();
            return c != null && (c.EnableDebugLogging || c.DebugPeriodicNpcSnapshot);
        }

        public static bool ShouldLogRoamInvestigate()
        {
            var c = GetConfig();
            return c != null && (c.EnableDebugLogging || c.DebugRoamInvestigate);
        }

        public static bool ShouldLogAssist()
        {
            var c = GetConfig();
            return c != null && (c.EnableDebugLogging || c.DebugAssist);
        }

        public static bool ShouldLogSpawnRegistration()
        {
            var c = GetConfig();
            return c != null && (c.EnableDebugLogging || c.EnableSpawnDebug);
        }

        public static bool ShouldLogLifecycle()
        {
            var c = GetConfig();
            return c != null && (c.DebugLifecycle || c.EnableDebugLogging);
        }

        public static bool ShouldLogImmediateTargeting()
        {
            var c = GetConfig();
            return c != null && (c.EnableTargetingDebug || c.EnableDebugLogging);
        }

        /// <summary>Throttled logs inside <see cref="Patches.TargetingPatches"/> (memory scoring, guard priority, raid goal).</summary>
        public static bool ShouldLogTargetingDiagnostics()
        {
            var c = GetConfig();
            return c != null && (c.EnableTargetingDebug || c.EnableDebugLogging);
        }

        public static bool ShouldLogCombatEnhancement()
        {
            var c = GetConfig();
            return c != null && (c.EnableCombatDebug || c.EnableDebugLogging);
        }

        public static bool ShouldLogCombatWeapons()
        {
            var c = GetConfig();
            return c != null && (c.EnableCombatDebug || c.EnableDebugLogging);
        }

        private static readonly Dictionary<string, float> _diagLastLogTime = new Dictionary<string, float>(256);

        private static bool ShouldLogDiagnostic(string key, float minIntervalSeconds)
        {
            float now = Time.realtimeSinceStartup;
            if (_diagLastLogTime.TryGetValue(key, out float last) && (now - last) < minIntervalSeconds)
                return false;
            _diagLastLogTime[key] = now;
            return true;
        }

        private static void LogCategoryFailure(
            string category,
            string code,
            string message,
            bool enabled,
        ...
        {
            if (!enabled) return;
            string key = $"{category}|{code}|{netId}";
            if (!ShouldLogDiagnostic(key, minIntervalSeconds)) return;
            UnityEngine.Debug.Log($"[GrimmNPC][{category}][{code}] {message}");
        }

        public static void LogNavMeshFailure(string code, string message, ulong netId = 0, float minIntervalSeconds = 2f)
        {
            var cfg = GetConfig();
            LogCategoryFailure("NavMesh", code, message, cfg.EnableNavMeshDebug, netId, minIntervalSeconds);
        }

        public static void LogSwimmingFailure(string code, string message, ulong netId = 0, float minIntervalSeconds = 2f)
        {
            var cfg = GetConfig();
            LogCategoryFailure("Swimming", code, message, cfg.EnableSwimmingCategoryDebug, netId, minIntervalSeconds);
        }

        public static void LogCombatFailure(string code, string message, ulong netId = 0, float minIntervalSeconds = 2f)
        {
            var cfg = GetConfig();
            LogCategoryFailure("Combat", code, message, cfg.EnableCombatDebug, netId, minIntervalSeconds);
        }

        public static void LogSpawnFailure(string code, string message, ulong netId = 0, float minIntervalSeconds = 2f)
        {
            var cfg = GetConfig();
            LogCategoryFailure("Spawn", code, message, cfg.EnableSpawnDebug, netId, minIntervalSeconds);
        }

        public static void LogTargetingFailure(string code, string message, ulong netId = 0, float minIntervalSeconds = 2f)
        {
            var cfg = GetConfig();
            LogCategoryFailure("Targeting", code, message, cfg.EnableTargetingDebug, netId, minIntervalSeconds);
        }
        
        /// <summary>
        /// Calls an Harmony hook via reflection (Harmony mods don't have direct Harmony.Core access).
        /// 
        /// PERFORMANCE OPTIMIZED: All reflection lookups are cached for maximum efficiency.
        /// - Harmony.Core assembly cached on first lookup
        /// - HarmonyModInterface.Mods instance cached
        ...
        public static bool CallOxideHook(string hookName, params object[] args)
        {
            ...
            catch (Exception ex)
            {
                if (GetConfig().EnableDebugLogging)
                    UnityEngine.Debug.LogWarning($"[GrimmNPC] CallOxideHook({hookName}) failed: {ex.Message}");
            }
        ...
        private static void InitializeOxideCache()
        {
            ...
            try
            {
                // Search all loaded assemblies for Harmony.Core (only once)
                System.Reflection.Assembly oxideCoreAssembly = null;
                foreach (var assembly in System.AppDomain.CurrentDomain.GetAssemblies())
                {
                    try
                    {
                        if (assembly.GetName().Name == "Harmony.Core" || 
                            assembly.FullName.Contains("Harmony.Core"))
                        {
                    ...
                    {
                        // Assembly enumeration may throw for dynamic assemblies; skip.
                    }
                }
                ...
                var interfaceType = oxideCoreAssembly.GetType("Harmony.Core.HarmonyModInterface");
                ...
                var modRuntimeProperty = interfaceType.GetProperty("Mods", 
                    System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
                if (modRuntimeProperty == null)
                {
                    _oxideAvailable = false;
                    return;
                }

                _oxideInstance = modRuntimeProperty.GetValue(null);
            ...
            catch (Exception ex)
            {
                _oxideAvailable = false;
                if (GetConfig().EnableDebugLogging)
                    UnityEngine.Debug.LogWarning($"[GrimmNPC] Oxide cache init failed: {ex.Message}");
            }
        }
        ...
        public static void RegisterNpc(ulong netId, CustomNpcData npcData)
        {
            if (Instance != null)
            {
                ...
                if (ShouldLogSpawnRegistration() && npcData != null)
                {
                    if (alreadyExists)
                    {
        ...
        public static void UnregisterNpc(ulong netId)
        {
            if (Instance != null && Instance._npcs.TryGetValue(netId, out var npcData))
            {
                ...
                Instance._spawnSwimInitUntil.Remove(netId);
            }
        }
        ...
        /// <summary>
        /// Marks a short spawn/init grace window for swimming custom NPCs.
        /// </summary>
        public static void MarkSpawnSwimInitWindow(ulong netId, float durationSeconds = 2f)
        {
            if (Instance == null || netId == 0)
                return;
            if (durationSeconds <= 0f)
                durationSeconds = 2f;
            Instance._spawnSwimInitUntil[netId] = Time.realtimeSinceStartup + durationSeconds;
        }

        /// <summary>
        /// Returns true while an NPC is in spawn/init swim grace window.
        /// </summary>
        public static bool IsInSpawnSwimInitWindow(ulong netId)
        {
            if (Instance == null || netId == 0)
                return false;
            if (!Instance._spawnSwimInitUntil.TryGetValue(netId, out float until))
                return false;
        ...
        internal static CustomNpcData ConsumePending(BaseEntity entity)
        {
            ...
            int instanceId = entity.GetInstanceID();
            var config = GetConfig();
            bool debugSpawnReg = ShouldLogSpawnRegistration();
    ...
    public class NpcConfig
    {
        ...
        // Debug Configuration
        /// <summary>
        /// Legacy master switch: when true, behaves like all granular NPC logs below are on (dynamic navmesh, periodic snapshot, roam/investigate, assist, spawn registration via <see cref="EnableSpawnDebug"/> OR, targeting via <see cref="EnableTargetingDebug"/>, combat via <see cref="EnableCombatDebug"/>).
        /// For a quiet server set this false and enable only the specific flags you need.
        /// </summary>
        public bool EnableDebugLogging { get; set; } = false;
        /// <summary>Log mod load/unload and config path messages.</summary>
        public bool DebugLifecycle { get; set; } = false;
        /// <summary>Log dynamic monument/terrain navmesh updates (very chatty per NPC).</summary>
        public bool DebugDynamicNavMesh { get; set; } = false;
        /// <summary>Log throttled per-NPC state snapshot (destination, roam, target).</summary>
        public bool DebugPeriodicNpcSnapshot { get; set; } = false;
        /// <summary>Log investigation / lost-target movement.</summary>
        public bool DebugRoamInvestigate { get; set; } = false;
        /// <summary>Log assist callouts and alerted allies.</summary>
        public bool DebugAssist { get; set; } = false;
        public bool EnableNavMeshDebug { get; set; } = false;
        public bool EnableSwimmingCategoryDebug { get; set; } = false;
        public bool EnableCombatDebug { get; set; } = false;
        public bool EnableSpawnDebug { get; set; } = false;
        public bool EnableTargetingDebug { get; set; } = false;
        /// <summary>
        /// When true, logs throttled swimming diagnostics (modelState.waterLevel vs world water probes) for custom NPCs in <see cref="Patches.BaseNavigator_IsSwimming_Patch"/>.
        /// </summary>
        public bool EnableSwimmingDebug { get; set; } = false;
        public bool EnableNavMeshValidation { get; set; } = false;

        /// <summary>
        /// When true (default), <see cref="Patches.BaseNavigator_SetDestination_Patch"/> nudges destinations onto the navmesh with a short path check (NpcSpawn-style).
        /// </summary>
        public bool EnableNavDestinationSanitize { get; set; } = true;

        /// <summary>
        /// Max radius (meters) for <c>NavMesh.SamplePosition</c> when sanitizing <see cref="BaseNavigator.SetDestination"/> goals.
        /// Matches NpcSpawn melee/raid checks (~6m): avoids snapping to distant mesh across walls or onto another island.
        /// Capped against the game&apos;s per-call <c>navmeshSampleDistance</c> (whichever is smaller).
        /// </summary>
        public float NavDestinationSanitizeSampleMaxMeters { get; set; } = 6f;

        /// <summary>
        /// When true (default), idle movement (no brain memory target) caps each destination leg from the current position,
        /// matching <c>Rust.Ai.Gen2.State_Patrol</c> (~4–6 m donut) instead of sprinting across the full <see cref="CustomNpcData.RoamRange"/> disk.
        /// Long paths are still allowed when the NPC is outside roam (return-home) or has a chase/combat target.
        /// </summary>
        public bool EnableIdlePatrolLegCap { get; set; } = true;
    ...
    public class CustomNpcData
    {
        ...
        // Optional movement gate (set by plugins; avoids per-SetDestination reflection for integrations like boss packs)
        /// <summary>
        /// When true, <c>BaseNavigator.SetDestination</c> is blocked for this NPC if no real players are within <see cref="FreezeMovementPlayerCheckRadius"/>.
        /// </summary>
        public bool FreezeMovementUnlessPlayersNearby { get; set; }

        /// <summary>Radius (meters) for <see cref="FreezeMovementUnlessPlayersNearby"/>.</summary>
        public float FreezeMovementPlayerCheckRadius { get; set; } = 150f;
        ...
        /// <summary>Default true: custom NPCs use Grimm swim logic when in water. Set false to keep land-only behavior.</summary>
        ...
        // --- Healing (Grimm think + syringe; tuned like NpcSpawn cadence) ---
        /// <summary>Minimum seconds between syringe uses per NPC.</summary>
        public float SyringeCooldownSeconds { get; set; } = 4f;
        /// <summary>While chasing/combat with a brain target, only start a heal when health fraction is at or below this (0–1).</summary>
        public float SyringeCombatMaxHealthFraction { get; set; } = 0.5f;
        /// <summary>Healing scale for syringe amount (1 = default +15 HP).</summary>
        public float HealingScale { get; set; } = 1f;

        // --- Wooden cover barricade (NpcSpawn PlaceBarricadeCover-style; requires belt item barricade.wood.cover) ---
        /// <summary>When true and belt has cover + syringe, may deploy wood double cover under pressure.</summary>
        public bool EnableBarricadeCover { get; set; } = true;
        /// <summary>Deploy only when health fraction is at or below this (e.g. 0.35 = 35% HP).</summary>
        public float BarricadeMaxHealthFraction { get; set; } = 0.35f;
        /// <summary>Require target at least this far (m) so the cover is meaningful.</summary>
        public float BarricadeMinTargetDistance { get; set; } = 12f;
        /// <summary>Minimum seconds between barricade attempts per NPC.</summary>
        public float BarricadeCooldownSeconds { get; set; } = 45f;
        /// <summary>Belt shortname for deployable wooden cover (matches NpcSpawn).</summary>
        public string BarricadeCoverBeltShortname { get; set; } = "barricade.wood.cover";
    }
    ...
    public static class Raid
    {
        ...
        private const int MaxRaidGoalStructureScan = 5000;
        private static readonly RaycastHit[] RaycastHits = new RaycastHit[32];
        private static readonly List<BuildingBlock> FoundationRemovalBuffer = new List<BuildingBlock>(64);
        ...
        public static void TickRaid(ScientistNPC npc)
        {
            ...
            if (data.RaidGoalActive && (brain.Events == null || brain.Events.Memory == null))
            {
                ...
                if (data.RaidGoalPosition != Vector3.zero)
                {
                    ...
                    if (Time.time >= state.NextRaidTime)
                    {
                        ...
                        if (foundTarget != null)
                        {
                            DispatchRaidAttack(npc, foundTarget, settings);
                        }
                    }
                }
                return;
            ...
            if (trackedRaidTarget != null)
            {
                ...
                {
                    state.NextRaidTime = Time.time + 0.5f;
                    DispatchRaidAttack(npc, trackedRaidTarget, settings);
                }
                return;
            }
            ...
            if (targetPlayer != null)
            {
                ...
                DispatchRaidAttack(npc, blockingTarget, settings);
                return;
            }
        ...
        private static BaseCombatEntity FindStructuresNearRaidGoal(ScientistNPC npc, Vector3 raidGoalPosition)
        {
            ...
            {
                if (++scanned > MaxRaidGoalStructureScan)
                    break;
        ...
        private static void UpdateRaidTargets(ScientistNPC npc, CustomNpcData data)
        {
            ...
            {
                FoundationRemovalBuffer.Clear();
                foreach (var foundation in data.Foundations)
                {
                    if (foundation == null || foundation.IsDestroyed || foundation.Health() <= 0f)
                        FoundationRemovalBuffer.Add(foundation);
                }
                for (int i = 0; i < FoundationRemovalBuffer.Count; i++)
                    data.Foundations.Remove(FoundationRemovalBuffer[i]);
            }
        ...
        /// <summary>
        /// Helper method to attack raid targets (calls StartExplosion or HandleRaidAttack).
        /// </summary>
        private static void DispatchRaidAttack(ScientistNPC npc, BaseCombatEntity target, RaidSettings settings)
        {
            ...
            if (!StartExplosion(npc, target))
            {
                ...
                SpecialWeaponsHandler.HandleRaidAttack(npc, target, settings);
            }
        }
        ...
        private static bool IsTeam(ulong playerId, ulong targetId)
        {
            ...
            // Note: Friends and Clans plugin integration would require Harmony mod system
            // Harmony mods don't have direct access to Harmony mods
