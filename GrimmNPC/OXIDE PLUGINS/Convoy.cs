...
{
    [Info("Convoy", "Adem | Updated by Grimm530", "2.9.11")]
    internal class Convoy : RustPlugin
    {
        ...
        private static class PveModeManager
        {
            ...
            public static void CreatePveModeZone(Vector3 position, BasePlayer externalOwner)
            {
                ...
                _ins.PveMode.Call("EventAddPveMode", _ins.Name, config, position, _ins._eventController.EventConfig.ZoneRadius, crates, npc, bradley, helicopters, turrets, _pveModeOwners, playerOwner);
            }
        ...
        private static class NpcSpawnManager
        {
            ...
            private static ScientistNPC SpawnScientistNpc(NpcConfig npcConfig, Vector3 position, float healthFraction, bool isStationary, bool isPassive)
            {
                ...
                {
                    _ins.PrintError("SpawnScientistNpc: NpcSpawn plugin reference is null! Cannot spawn NPCs.");
                    return null;
                }

                if (!_ins.plugins.Exists("NpcSpawn"))
                {
                    _ins.PrintError("SpawnScientistNpc: NpcSpawn plugin is not loaded! Cannot spawn NPCs.");
                    return null;
                }
                ...
                {
                    _ins.PrintError($"SpawnScientistNpc: Failed to create config object for preset '{npcConfig.PresetName}'!");
                    return null;
                }

                object spawnResult = _ins.NpcSpawn?.Call("SpawnNpc", position, baseNpcConfigObj);
                if (spawnResult == null)
                {
                    _ins.PrintError($"SpawnScientistNpc: NpcSpawn.Call('SpawnNpc') returned null for preset '{npcConfig.PresetName}' at position {position}.");
                    return null;
                }
                ...
                {
                    _ins.PrintError($"SpawnScientistNpc: NpcSpawn.Call('SpawnNpc') returned unexpected type '{spawnResult.GetType().Name}' for preset '{npcConfig.PresetName}'.");
                    return null;
                }

                if (!scientistNpc.IsExists())
                {
                    _ins.PrintError($"SpawnScientistNpc: NPC spawned but IsExists() returned false for preset '{npcConfig.PresetName}'.");
                    return null;
                }
            ...
            private static object GetBaseNpcConfig(NpcConfig config, float healthFraction, bool isStationary, bool isPassive)
            {
                ...
                {
                    _ins.PrintError("GetBaseNpcConfig: NpcSpawn plugin type is null!");
                    return null;
                }
                ...
                {
                    _ins.PrintError("GetBaseNpcConfig: Failed to find NpcSpawn.NpcConfig type. Make sure NpcSpawn plugin is loaded.");
                    return null;
                }
                ...
                {
                    _ins.PrintError("GetBaseNpcConfig: Failed to find NpcSpawn.NpcWear or NpcBelt types.");
                    return null;
                }

                // Convert WearItems from Convoy.NpcWear to NpcSpawn.NpcWear
                ...
                // Convert BeltItems from Convoy.NpcBelt to NpcSpawn.NpcBelt
                // Always create HashSet (even if empty) to ensure UpdateInventory() has a valid collection
                // Note: isPassive only affects AI behavior (states), not inventory - passive NPCs should still have weapons
                ...
                if (npcBeltType != null)
                {
                    ...
                    if (config.BeltItems != null && config.BeltItems.Count > 0)
                    {
                        ...
                        foreach (var item in config.BeltItems)
                        {
                            ...
                            {
                                _ins.PrintWarning($"GetBaseNpcConfig: Skipping belt item for '{config.PresetName}' - ShortName is null or empty");
                                skippedCount++;
                                continue;
                            }
                        ...
                        if (addedCount == 0 && config.BeltItems.Count > 0)
                        {
                            _ins.PrintWarning($"GetBaseNpcConfig: No belt items were added for '{config.PresetName}' (skipped {skippedCount} out of {config.BeltItems.Count} items)");
                        }
                    }
                }
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "RoamRange", isStationary ? 0f : config.RoamRange);
                SetProperty(npcConfigType, npcSpawnConfig, "ChaseRange", isStationary ? 0f : config.ChaseRange);
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "Speed", isStationary ? 0f : config.Speed);
