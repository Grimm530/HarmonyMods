...
namespace TeleportGUI
{
    public partial class TeleportGUIMod : IHarmonyModHooks
    {
        ...
        private Dictionary<string, TeleportGUIData.WarpPoint> _warpData;
        ...
        private static void SyncUsage(TeleportGUIData.UserData.Usage usage, int usesToday, double cooldownUntil)
        {
            if (usage == null) return;
            usage.UsesToday = usesToday;
        ...
        /// <summary>
        /// GrimmCUI endtest entry. Supports:
        /// - TELEPORTGUI teleportgui.cui &lt;action&gt; … (hand-rolled TeleportGUIUI)
        /// - TELEPORTGUI teleportgui.callback &lt;id&gt; … (Chaos UI CommandCallbackHandler)
        /// </summary>
        public void HandleCuiCallback(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                HandleChaosCallback(args, a);
                return;
            }

            // Hand-rolled: TELEPORTGUI teleportgui.cui mode.teleport …
            if (second.Equals("teleportgui.cui", StringComparison.OrdinalIgnoreCase))
            {
                if (!GrimmCui.InvokeRegisteredConsoleCommand("teleportgui.cui", args, 2))
                {
                    string cmd = a.Length > 2 ? (a.GetValue(2)?.ToString() ?? "") : "";
                    var rest = new string[Math.Max(0, a.Length - 3)];
                    for (int i = 3; i < a.Length; i++) rest[i - 3] = a.GetValue(i)?.ToString() ?? "";
                    HandleCuiCommand(player, cmd, rest);
                }
                return;
            }

            // Legacy: TELEPORTGUI <action> …
            HandleCuiEndtest(args, a);
        }

        private void HandleChaosCallback(ConsoleSystem.Arg args, Array a)
        {
            // Populated when Chaos TeleportGUI.UI.cs is compiled and SetupUIComponents runs.
            if (_chaosCallbackHandler == null) return;
            ...
            if (a.Length >= 2)
            {
                    ...
                    second.StartsWith("teleportgui.callback", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        var rest = second.Substring("teleportgui.callback".Length).Trim();
            ...
            try
            {
                ...
                _chaosCallbackHandler.HandleCallback(new ConsoleSystem.Arg(opt, sb.ToString()));
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogWarning("[TeleportGUI] cui.endtest TELEPORTGUI callback: " + ex);
            }
        }

        public void HandleCuiEndtest(ConsoleSystem.Arg args, System.Array a)
        {
            if (a == null || a.Length < 2) return;
            var player = args?.Connection?.player as BasePlayer ?? args?.Player();
            ...
            if (second.Equals("teleportgui.cui", StringComparison.OrdinalIgnoreCase))
                cmdIndex = 2;
            if (a.Length <= cmdIndex) return;

            string cmd = a.GetValue(cmdIndex)?.ToString() ?? string.Empty;
            var rest = new string[Math.Max(0, a.Length - cmdIndex - 1)];
            for (int i = cmdIndex + 1; i < a.Length; i++) rest[i - cmdIndex - 1] = a.GetValue(i)?.ToString() ?? string.Empty;
            HandleCuiCommand(player, cmd, rest);
        }

        // Set by Chaos UI SetupUIComponents when TeleportGUI.UI.cs is enabled.
        private Ext.Chaos.UIFramework.CommandCallbackHandler _chaosCallbackHandler;
        private void HandleCuiCommand(BasePlayer player, string cmd, string[] args)
        {
            ...
            if (cmd == "addwarp")
            {
                ...
                _showingModal.Remove(player.userID);
                if (_data.WarpPoints == null) _data.WarpPoints = new Dictionary<string, TeleportGUIData.WarpPoint>();
                _data.WarpPoints[name] = TeleportGUIData.WarpPoint.FromVector3(pos);
                _warpData = _data.WarpPoints;
                SaveData();
        ...
        private void CmdTP(BasePlayer player, string[] args)
        {
            ...
            if (delay > 0)
            {
                ...
                ServerMgr.Instance?.StartCoroutine(DelayedTeleport(player, dest, delay, () =>
                {
                    ...
                    SyncUsage(user.TPUsage, user.TPUsesToday, user.TPCooldownUntil);
                }, "teleport"));
            }
            else
            {
        ...
        private void CmdHome(BasePlayer player, string subCmd, string[] args)
        {
            ...
            if (isSet)
            {
                ...
                user.Homes[name] = new TeleportGUIData.UserData.HomePoint { Position = player.transform.position };
            ...
            if (delay > 0)
            {
                ...
                ServerMgr.Instance?.StartCoroutine(DelayedTeleport(player, dest, delay, () =>
                {
                    ...
                    SyncUsage(user.HomeUsage, user.HomeUsesToday, user.HomeCooldownUntil);
                }, "home"));
            }
            else
            {
        ...
        private void CmdWarp(BasePlayer player, string[] args)
        {
            ...
            if (delay > 0)
            {
                ...
                ServerMgr.Instance?.StartCoroutine(DelayedTeleport(player, dest, delay, () =>
                {
                    ...
                    SyncUsage(user.WarpUsage, user.WarpUsesToday, user.WarpCooldownUntil);
                }, "warp"));
            }
            else
            {
        ...
        internal bool HasPerm(BasePlayer player, string perm)
        {
            if (player == null) return false;
            if ((_config?.AdminsBypass ?? true) && (player.IsAdmin || player.IsDeveloper)) return true;
            return PermissionsBridge.UserHasPermission(player, perm);
        }

        internal bool HasVipPermission(BasePlayer player, string vipPerm)
        {
            return string.IsNullOrEmpty(vipPerm) || HasPerm(player, vipPerm);
        }

        internal static string EnsureWarpPermission(string permission)
        {
            if (string.IsNullOrEmpty(permission)) return string.Empty;
            return permission.StartsWith("teleportgui.", StringComparison.OrdinalIgnoreCase)
                ? permission
                : "teleportgui.warps." + permission.Trim().ToLowerInvariant();
        }
        ...
        public void OnServerSave() => SaveData();
    }
}
