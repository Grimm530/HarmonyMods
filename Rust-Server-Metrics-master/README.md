...
## This working copy (`!RustServer`)

This tree under `.cursor/HarmonyMods/Rust-Server-Metrics-master` is a **fork** layered on [RustyMoose/Rust.ServerMetrics](https://github.com/RustyMoose/Rust.ServerMetrics) with local build tooling, Grafana/Influx launchers under `Ps1&Batch/`, and extra Harmony/metrics behavior. **Pulling upstream updates, remotes, conflict hotspots, and a step-by-step merge playbook** are documented in **`RSM-Instructional.md`** → section **Fork maintenance and upstream updates**. Quick diff vs public `main`: `.\scripts\Compare-ToUpstream.ps1`. For **Run as administrator** shortcuts, use repo-root **`Launch-Grafana-UAC.vbs`** / **`Launch-InfluxDB-UAC.vbs`** (most reliable with `&` / `!` in paths), or **`Start-Grafana-AsAdmin.bat`** / **`Start-InfluxDB-AsAdmin.bat`**. If you build a `.lnk` with `powershell.exe -File ...`, **quote the full path** after `-File` or the `&` in `Ps1&Batch` will break the command.
