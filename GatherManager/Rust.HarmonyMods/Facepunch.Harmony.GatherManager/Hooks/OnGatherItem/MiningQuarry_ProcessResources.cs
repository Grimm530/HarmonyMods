...
namespace Facepunch.Harmony.GatherManager
{
    ...
    internal class MiningQuarry_ProcessResources
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator, MethodBase originalMethod)
        {
            ...
            var hookMethod = typeof(MiningQuarry_ProcessResources).GetMethod("Hook", BindingFlags.Public | BindingFlags.Static);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    if (method.Name == "OnQuarryItem" && method.DeclaringType?.Name == "Azure")
                    {
        ...
        public static void Hook(MiningQuarry quarry, Item item)
        {
            try
            {
                if (GatherManagerMod.Instance == null || item == null) return;
                GatherManagerMod.Instance.ApplyQuarryModifier(quarry, item);
            }
            catch (Exception ex)
            {
