...
using GrimmCuiHarmony;
using HarmonyPlugin = Harmony.Plugins.VirtualQuarries;
using GrimmCuiHarmony;
...
namespace VirtualQuarriesHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("VirtualQuarries_Runner");
    ...
    public class VirtualQuarriesMod : IHarmonyModHooks
    {
        public static VirtualQuarriesMod Instance { get; private set; }
        public static HarmonyPlugin Plugin => HarmonyPlugin.GetModInstance();

        public const string AppDomainApiKey = "VirtualQuarries_ApiType";
        public const string CuiMarker = "VIRTUALQUARRIES";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[VirtualQuarries] FAIL: construct/config: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(VirtualQuarriesMod)); }
            catch { }
            ...
            Debug.Log("[VirtualQuarries] Harmony mod loaded v2.6.0. Config: HarmonyConfig/VirtualQuarries.json. Data: HarmonyData/VirtualQuarries/");
        }

        private void OnPermissionsReady()
        {
            // Init/LoadMessages need ItemManager; WaitForServerThenInit owns that.
        }
        ...
        public void OnUnloaded(OnHarmonyModUnloadedArgs args)
        {
            ...
            Instance = null;
            Debug.Log("[VirtualQuarries] Harmony mod unloaded.");
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[VirtualQuarries] Init: " + ex.Message); }
            ...
            catch (Exception ex) { Debug.LogError("[VirtualQuarries] OnServerInitialized: " + ex); }

            RefreshDynamicCommands();
            _initCoroutine = null;
            Debug.Log("[VirtualQuarries] Server initialized.");
        }
        ...
        internal static void InvokeCommandMethod(HarmonyPlugin plugin, string methodName, BasePlayer player, string command, string[] args)
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] InvokeCommandMethod " + methodName + ": " + (ex.InnerException?.Message ?? ex.Message));
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[VirtualQuarries] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            try
            ...
            {
                Debug.LogWarning("[VirtualQuarries] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] cui.endtest VIRTUALQUARRIES: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] " + methodName + ": " + (ex.InnerException?.Message ?? ex.Message));
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[VirtualQuarries] cmd " + captured + ": " + ex.Message); }
                }
            };
