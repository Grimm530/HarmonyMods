 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to VirtualQuarriesHarmony,
 * log tag changed to [VirtualQuarries].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace VirtualQuarriesHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[VirtualQuarries] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[VirtualQuarries] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[VirtualQuarries] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[VirtualQuarries] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[VirtualQuarries] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[VirtualQuarries] Permissions Service bind: " + ex.Message);
            }
        }
