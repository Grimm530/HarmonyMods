...
namespace AutoCodeLockHarmony
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[AutoCodeLock] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[AutoCodeLock] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[AutoCodeLock] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[AutoCodeLock] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[AutoCodeLock] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[AutoCodeLock] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[AutoCodeLock] Permissions Service bind: " + ex.Message);
            }
        }
