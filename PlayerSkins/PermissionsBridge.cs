...
namespace PlayerSkinsHarmony
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[PlayerSkins] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[PlayerSkins] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[PlayerSkins] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[PlayerSkins] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[PlayerSkins] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[PlayerSkins] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[PlayerSkins] Permissions Service bind: " + ex.Message);
            }
        }
