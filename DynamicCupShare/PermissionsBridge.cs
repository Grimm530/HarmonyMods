...
namespace DynamicCupShareHarmony
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[DynamicCupShare] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[DynamicCupShare] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[DynamicCupShare] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[DynamicCupShare] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[DynamicCupShare] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[DynamicCupShare] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[DynamicCupShare] Permissions Service bind: " + ex.Message);
            }
        }
