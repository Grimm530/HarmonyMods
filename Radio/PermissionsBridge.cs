 ...
 * Adapted from Radio/PermissionsBridge.cs — namespace changed to RadioHarmony,
 * log tag changed to [Radio].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace RadioHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[Radio] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[Radio] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[Radio] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[Radio] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[Radio] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[Radio] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[Radio] Permissions Service bind: " + ex.Message);
            }
        }
