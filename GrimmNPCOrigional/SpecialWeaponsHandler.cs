...
namespace GrimmNPC
{
    ...
    public static class SpecialWeaponsHandler
    {
        ...
        internal class WeaponState
        {
            ...
            public float RocketReloadUntil;
            public float RocketLastAttemptTime; // Track last attempt to prevent rapid retries
            ...
            public float NextFlameThrowerFireTime;
        ...
        /// </summary>
        public static bool TryHandleCombatAttack(ScientistNPC npc, BaseEntity target)
        {
            ...
            {
                UnityEngine.Debug.Log($"[GrimmNPC Combat] NPC {npc.net?.ID.Value ?? 0} TryHandleCombatAttack: " +
        ...
        private static bool ExecuteCombatWeapon(ScientistNPC npc, BaseEntity target, WeaponChoice choice, float distance)
        {
            ...
            switch (choice.Kind)
            {
                case WeaponKind.RocketLauncher:
                    handled = TryFireRocketLauncher(npc, target as BaseCombatEntity, distance);
                    break;
                case WeaponKind.GrenadeLauncher:
                    handled = TryFireGrenadeLauncher(npc, target as BaseCombatEntity, distance);
                    break;
                case WeaponKind.FlameThrower:
                    handled = TryFireFlameThrower(npc, target as BaseCombatEntity, distance);
                    break;
                case WeaponKind.BaseProjectile:
                    ...
                    {
                        handled = TryFireBow(npc, target as BaseCombatEntity, distance, 50f, shortName);
                    }
                    else
                    {
                ...
                case WeaponKind.ThrownExplosive:
                    // F1 grenade throwing (like BotReSpawn.TryThrow)
                    handled = TryThrowF1Grenade(npc, target);
                    break;
                case WeaponKind.Melee:
                    handled = TryUseMeleeWeapon(npc, target as BaseCombatEntity, 25f);
                    break;
            }
            ...
            if (!handled)
            {
                ...
                if (standardWeapon != null)
                {
                    ...
                    bool fallbackResult = TryFireBaseProjectile(npc, target as BaseCombatEntity, distance, 50f);
                    if (Time.frameCount % 120 == 0)
                ...
                else if (distance <= 4f)
                {
                    ...
                    return TryUseMeleeWeapon(npc, target as BaseCombatEntity, 25f);
                }
                else
                {
                    if (Time.frameCount % 120 == 0)
        ...
        /// <summary>
        /// Throws F1 grenade at target (like BotReSpawn.TryThrow).
        /// </summary>
        private static bool TryThrowF1Grenade(ScientistNPC npc, BaseEntity target)
        {
            if (npc == null || target == null) return false;
        ...
        public static bool TryHandleRaidAttack(ScientistNPC npc, BaseCombatEntity target, RaidSettings settings)
        {
        ...
        private static bool ExecuteRaidWeapon(ScientistNPC npc, BaseCombatEntity target, RaidSettings settings, WeaponChoice choice, float distance)
        {
            ...
            {
                case WeaponKind.RocketLauncher:
                    handled = TryFireRocketLauncher(npc, target, distance);
                    break;
                case WeaponKind.GrenadeLauncher:
                    handled = TryFireGrenadeLauncher(npc, target, distance);
                    break;
                case WeaponKind.FlameThrower:
                    handled = TryFireFlameThrower(npc, target, distance);
                    break;
                case WeaponKind.BaseProjectile:
                    handled = TryFireBaseProjectile(npc, target, distance, settings.FallbackDamageRanged);
                    break;
                case WeaponKind.ThrownExplosive:
                    handled = TryUseThrownExplosive(npc, target);
                    break;
                case WeaponKind.Melee:
                    handled = TryUseMeleeWeapon(npc, target, settings.FallbackDamageMelee);
                    break;
                default:
                    handled = false;
                    break;
            ...
            if (HasStandardProjectile(npc))
                return TryFireBaseProjectile(npc, target, distance, settings.FallbackDamageRanged);

            return TryUseMeleeWeapon(npc, target, settings.FallbackDamageMelee);
        }

        private static bool TryFireRocketLauncher(ScientistNPC npc, BaseCombatEntity target, float distance)
        {
            if (npc == null || target == null) return false;
        ...
        private static bool TryFireGrenadeLauncher(ScientistNPC npc, BaseCombatEntity target, float distance)
        {
            if (npc == null || target == null) return false;
            // Only check max range, no minimum
        ...
        /// </summary>
        private static bool TryFireFlameThrower(ScientistNPC npc, BaseCombatEntity target, float distance)
        {
            if (npc == null || target == null) return false;
            ...
            if (distance > effectiveRange)
            {
                ...
                // User wants: move in while firing, trying to close the gap quickly
                if (brain != null && brain.Navigator != null)
                {
                    Vector3 targetPos = target.transform.position;
        ...
        private static bool TryFireBaseProjectile(ScientistNPC npc, BaseCombatEntity target, float distance, float fallbackDamage)
        {
            if (npc == null || target == null) return false;
            ...
            string shortName = activeItem?.info?.shortname ?? string.Empty;
            if (IsBowWeapon(shortName))
                return TryFireBow(npc, target, distance, fallbackDamage, shortName);
        ...
        private static bool TryFireBow(ScientistNPC npc, BaseCombatEntity target, float distance, float fallbackDamage, string shortName)
        {
            var state = GetState(npc.net?.ID.Value ?? 0);
            if (state == null) return false;
                ...
                case WeaponState.SatchelCyclePhase.Throwing:
                    ...
                    if (state.SatchelsThrownThisCycle < SatchelsPerCycle)
                    {
                        ...
                        if (distance >= SatchelMinRange && distance <= SatchelMaxRange && hasWallInRange)
                        {
                            ...
                            return false; // Let TryUseThrownExplosive execute
                        }
        ...
        /// </summary>
        private static bool TryUseMeleeWeapon(ScientistNPC npc, BaseCombatEntity target, float fallbackDamage)
        {
            if (npc == null || target == null) return false;
        ...
        /// </summary>
        private static bool TryUseThrownExplosive(ScientistNPC npc, BaseCombatEntity target)
        {
            if (npc == null || target == null) return false;
