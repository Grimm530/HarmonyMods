...
namespace CombatClassesHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("CombatClasses_Runner");
    ...
    public class CombatClassesMod : IHarmonyModHooks
    {
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[CombatClasses] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(CombatClassesMod)); }
            catch { }

            foreach (var cmd in new[] { "class", "gearbox", "givexp", "movecombat", "moveclasshud", "resetcombat" })
            {
                _chatCommandNames.Add(cmd);
                RegisterChatAliasConsole(cmd);
            }

            ChatSayBridge.Register("CombatClasses", OnChatCommand);
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] Permissions ready re-register: " + ex.Message);
            }
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[CombatClasses] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[CombatClasses] Init: " + ex.Message); }

            try { plugin.CallOnServerInitialized(); }
            catch (Exception ex) { Debug.LogError("[CombatClasses] OnServerInitialized: " + ex); }

            RefreshDynamicCommands();

            _initCoroutine = null;
            Debug.Log("[CombatClasses] Server initialized.");
        }
        ...
        private static void InvokeChatMethod(OxidePlugin plugin, string methodName, BasePlayer player, string command, string[] args)
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[CombatClasses] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            ...
            {
                Debug.LogWarning("[CombatClasses] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            try
            {
                ...
                Debug.LogWarning("[CombatClasses] cui.endtest CC: command not registered: " + cmdName);
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[CombatClasses] cui.endtest CC: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[CombatClasses] cmd " + captured + ": " + ex.Message); }
                }
            };
