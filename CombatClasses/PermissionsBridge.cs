 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to CombatClassesHarmony,
 * log tag changed to [CombatClasses].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace CombatClassesHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[CombatClasses] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[CombatClasses] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[CombatClasses] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[CombatClasses] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[CombatClasses] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[CombatClasses] Permissions Service bind: " + ex.Message);
            }
        }
