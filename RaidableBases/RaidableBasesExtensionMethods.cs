...
namespace RaidableBases.RaidableBasesExtensionMethods
{
    public static class ExtensionMethods
    {
        public class DisposableBuilder : IDisposable, Pool.IPooled
        {
            ...
            public void LeavePool() { _builder = Pool.Get<StringBuilder>(); _builder.Clear(); }
            ...
            public DisposableBuilder Append(char value) { _builder.Append(value); return this; }
            public DisposableBuilder Append(int value) { _builder.Append(value); return this; }
            ...
            public DisposableBuilder EnsureCapacity(int capacity) { _builder.EnsureCapacity(capacity); return this; }
            public override string ToString() => _builder.ToString();
            public int Length { get => _builder.Length; set => _builder.Length = value; }
        }
        public class DisposableSet<T> : IDisposable, Pool.IPooled
        {
            private PooledHashSet<T> _set = Pool.Get<PooledHashSet<T>>();
            public static DisposableSet<T> Get() => Pool.Get<DisposableSet<T>>();
            public static DisposableSet<T> Get(IEnumerable<T> obj)
            {
                DisposableSet<T> set = Get();
                set._set.UnionWith(obj);
                return set;
            }
            public static DisposableSet<T> Get(T item)
            {
                DisposableSet<T> set = Get();
                set._set.Add(item);
                return set;
            }
            public void UnionWith(IEnumerable<T> items) => _set.UnionWith(items);
            public void EnterPool() { _set.Clear(); Pool.Free(ref _set); }
            public void LeavePool() { _set = Pool.Get<PooledHashSet<T>>(); _set.Clear(); }
            public void Dispose() { DisposableSet<T> obj = this; Pool.Free(ref obj); }
        }
        ...
        public static bool Any(this IList<string> a, string src) { if (a == null || a.Count == 0 || string.IsNullOrWhiteSpace(src)) return false; for (int i = 0; i < a.Count; i++) { if (src.Contains(a[i])) { return true; } } return false; }
        ...
        public static StringView[] Skip(this StringView[] a, int b) => a.Length == 0 || b >= a.Length ? Array.Empty<StringView>() : a[b..];
        public static string[] Skip(this string[] a, int b) => a.Length == 0 || b >= a.Length ? Array.Empty<string>() : a[b..];
        ...
        public static bool IsHuman(this BasePlayer a) { if (a == null) return false; try { return ((ulong)a.userID).IsSteamId(); } catch { return a.UserIDString.IsSteamId(); } }
        ...
        public static ulong userid(this BasePlayer player) => player.userID.Get();
        ...
        public static bool TryAddIfAbsent<TKey, TValue>(this IDictionary<TKey, TValue> dict, TKey key, TValue value) { if (dict == null || dict.ContainsKey(key)) return false; dict[key] = value; return true; }
        ...
        public static T GetSecureRandom<T>(this List<T> list) { if (list == null || list.Count == 0) { return default(T); } if (list.Count == 1) return list[0]; return list[System.Security.Cryptography.RandomNumberGenerator.GetInt32(0, list.Count)]; }
    }
}
