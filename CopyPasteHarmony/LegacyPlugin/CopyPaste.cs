...
Fixed error with "oldID" by skipping duplicates of the same key, applies to copypaste files saved with older versions of Fortify
Updated parsing for itemid, dataInt and dataFloat
Removed static scope from fields
*/

namespace Harmony.Plugins
{
    [Info("Copy Paste", "misticos/nivex/Grimm530", "4.2.6")]
    ...
    public class CopyPaste : CovalencePlugin
    {
        ...
        //
        // Tracker IDs are:
        // - Generated when copying if the source entity doesn't already have one.
        // - Persisted in the copypaste JSON as "trackerid" alongside "prefabname"/"pos"/"rot".
        // - Re-attached to entities when pasting (stored in this dictionary and exposed via API).
        ...
        private object TryCopyFromSteamId(ulong userId, string filename, string[] args, Action callback = null)
        {
            ...
            return TryCopy(hit.point, entity.GetNetworkRotation().eulerAngles, filename,
                DegreeToRadian(player.GetNetworkRotation().eulerAngles.y), args, new RustIPlayer(player), callback);
        }
            ...
            Action<BaseEntity> callbackSpawned = null)
        {
            ...
            return TryPaste(hit.point, filename, new RustIPlayer(player),
        ...
        private void CopyLoop(CopyData copyData)
        {
            ...
            */
            ...
            else
            {
                ...
                var datafile = HarmonyModInterface.Mods.DataFileSystem.GetFile(path);
                ...
                HarmonyModInterface.Mods.DataFileSystem.SaveDatafile(path);
            ...
            CopyData copyData)
        {
            ...
            {
                // Compact GUID without dashes, good enough as a globally unique tracker.
            ...
            // Save scale if entity has been scaled
            if (entity.networkEntityScale && entity.transform.localScale != Vector3.one)
            {
            ...
            {
                if (CopyPasteReflectionHelpers.TryGetMemberValue(orientableLight, "pitchAmount", out var pitchRaw) && pitchRaw != null)
                    data.Add("pitchAmount", Convert.ToSingle(pitchRaw));

                if (CopyPasteReflectionHelpers.TryGetMemberValue(orientableLight, "yawAmount", out var yawRaw) && yawRaw != null)
                    data.Add("yawAmount", Convert.ToSingle(yawRaw));
            }
            ...
            {
                if (CopyPasteReflectionHelpers.TryGetRidableHorseCurrentBreedIndex(ridableHorse2, out int breedIndex))
                    data.Add("currentBreedIndex", breedIndex);

                if (ridableHorse2.IsTowing &&
                    CopyPasteReflectionHelpers.TryGetRidableHorseTowingEntityId(ridableHorse2, out var towingEntityId))
                    data.Add("towingEntityId", towingEntityId);
            }
            ...
            {
                var materials = CopyPasteReflectionHelpers.GetIntArrayMember(constructableEntity, "currentMaterials");
                if (materials != null && materials.Length > 0)
                {
                    data.Add("currentMaterials", materials);
            ...
            if (ioEntity.IsValid() && !ioEntity.IsDestroyed)
            {
                ...
                if (audioVisual != null)
                {
                if (CopyPasteReflectionHelpers.TryGetIntMember(audioVisual, "currentColour", out var colour))
                    ioData.Add("colour", colour);
                if (CopyPasteReflectionHelpers.TryGetIntMember(audioVisual, "currentVolumeSensitivity", out var volumeSensitivity))
                    ioData.Add("volumeSensitivity", volumeSensitivity);
                if (CopyPasteReflectionHelpers.TryGetIntMember(audioVisual, "currentSpeed", out var speed))
                    ioData.Add("speed", speed);
                if (CopyPasteReflectionHelpers.TryGetIntMember(audioVisual, "currentGradient", out var gradient))
                    ioData.Add("gradient", gradient);

                var connectedUid = CopyPasteReflectionHelpers.TryGetAudioVisualConnectedToUid(audioVisual);
                if (connectedUid.HasValue && connectedUid.Value != 0)
                    ioData.Add("connectedTo", connectedUid.Value);
                }
        ...
        private IEnumerator PasteLoop(PasteData pasteData)
        {
            ...
            foreach (var entity in pasteData.StabilityEntities)
            {
                ...
                foreach (var adapter in pasteData.industrialStorageAdaptors)
                {
                    ...
                    {
                        using var ents = Facepunch.Pool.Get<PooledList<BaseEntity>>();
        ...
        private void PasteEntity(Dictionary<string, object> data, PasteData pasteData, BaseEntity parent = null)
        {
            ...
            // Restore scale if it was saved
            if (data.ContainsKey("scale"))
            {
                var scaleData = data["scale"] as Dictionary<string, object>;
                if (scaleData != null)
                {
            ...
            // TrackerId: if this entity data contains a trackerid, attach it to the spawned entity so that
            // external plugins can query it via CopyPaste's API. This must be done AFTER spawning when entity.net is guaranteed to exist.
            ...
            if (entity != null && IsStringLightsType(entity.GetType()))
            {
                ...
                if (data.ContainsKey("animationStyle"))
                {
                    ...
                    // Try set enum/field smartly; if enum type changed, try parse underlying int
                    if (!SetFieldOrPropertyValue(entity, "animationStyle", anim))
                    {
                        ...
                        // try set by converting to target enum if available
            ...
            var chandelier = entity as Chandelier;
            if (chandelier != null && data.ContainsKey("chandelierLength"))
            {
            ...
            if (orientableLight != null)
            {
                float? pitch = data.ContainsKey("pitchAmount") ? Convert.ToSingle(data["pitchAmount"]) : null;
                float? yaw = data.ContainsKey("yawAmount") ? Convert.ToSingle(data["yawAmount"]) : null;
                CopyPasteReflectionHelpers.TrySetOrientableLightPitchYaw(orientableLight, pitch, yaw);
            }
                    ...
                    pasteData.FinalProcessingActions.Add(() =>
                    {
                            ...
                            result.TryGetValue("entity", out var newEntityObj))
                        {
                                ...
                                newEntity is ITowing iTowing)
                            {
                                ...
                                CopyPasteReflectionHelpers.TryAttachRidableHorseTowing(ridableHorse2, newEntity, iTowing);
                            }
                        }
                    });
                }
            ...
            if (constructableEntity != null)
            {
                if (data.TryGetValue("currentMaterials", out var currentMaterialsObj))
                {
                    ...
                    {
                        CopyPasteReflectionHelpers.SetIntArrayMember(constructableEntity, "currentMaterials", currentMaterials);
                    }
                }
                ...
                CopyPasteReflectionHelpers.TryInvoke(constructableEntity, "UpdateState");
            }
            ...
            if (mixingTable != null && mixingTable.IsOn())
            {
                ...
                var recipe = RecipeDictionary.GetMatchingRecipeAndQuantity(mixingTable.Recipes, orderedContainerItems, out var quantity);
                CopyPasteReflectionHelpers.SetMemberValue(mixingTable, "currentRecipe", recipe);
                mixingTable.currentQuantity = quantity;
                if (recipe == null || !itemsAreContiguous)
                {
                    mixingTable.StopMixing();
                    return;
                }
                mixingTable.RemainingMixTime = recipe.MixingDuration * mixingTable.currentQuantity;
                mixingTable.TotalMixTime = mixingTable.RemainingMixTime;
                if (mixingTable.RemainingMixTime == 0.0)
                {
                    mixingTable.ProduceItem(recipe, mixingTable.currentQuantity);
                }
                else
                {
            ...
            if (entity is FarmableAnimal farmableAnimal)
            {
                if (data.TryGetValue("hunger", out var hunger))
                    CopyPasteReflectionHelpers.SetMemberValue(farmableAnimal, "AnimalHunger", Convert.ToSingle(hunger));
                if (data.TryGetValue("thirst", out var thirst))
                    CopyPasteReflectionHelpers.SetMemberValue(farmableAnimal, "AnimalThirst", Convert.ToSingle(thirst));
                if (data.TryGetValue("love", out var love))
                    CopyPasteReflectionHelpers.SetMemberValue(farmableAnimal, "AnimalLove", Convert.ToSingle(love));
                if (data.TryGetValue("sunlight", out var sunlight))
                    CopyPasteReflectionHelpers.SetMemberValue(farmableAnimal, "AnimalSunlight", Convert.ToSingle(sunlight));
                if (data.TryGetValue("animalName", out var animalName))
                    CopyPasteReflectionHelpers.SetMemberValue(farmableAnimal, "AnimalName", (string)animalName);
            ...
            if (entity is ChickenCoop chickenCoop)
            {
                    ...
                    data.TryGetValue("timeUntilHatches", out var timeUntilHatchesObj))
                {
                    ...
                    if (timeUntilHatches != null && timeUntilHatches.Count > 0)
                    {
                        ...
                        // Some Rust versions don't expose CheckEggHatchState publicly; use Unity string-based scheduling instead.
                        if (!chickenCoop.IsInvoking("CheckEggHatchState"))
                            chickenCoop.InvokeRepeating("CheckEggHatchState", 10f, 10f);
        ...
        private void ProgressIOEntity(Dictionary<string, object> ioData, PasteData pasteData)
        {
            ...
            if (audioVisual != null)
            {
                if (ioData.TryGetValue("colour", out object audioObj))
                    CopyPasteReflectionHelpers.TrySetAudioVisualEnumProperty<AudioVisualisationEntity.LightColour>(audioVisual, "currentColour",
                        Convert.ToInt32(audioObj));
                if (ioData.TryGetValue("volumeSensitivity", out audioObj))
                    CopyPasteReflectionHelpers.TrySetAudioVisualEnumProperty<AudioVisualisationEntity.VolumeSensitivity>(audioVisual, "currentVolumeSensitivity",
                        Convert.ToInt32(audioObj));
                if (ioData.TryGetValue("speed", out audioObj))
                    CopyPasteReflectionHelpers.TrySetAudioVisualEnumProperty<AudioVisualisationEntity.Speed>(audioVisual, "currentSpeed",
                        Convert.ToInt32(audioObj));
                if (ioData.TryGetValue("gradient", out audioObj))
                    CopyPasteReflectionHelpers.SetMemberValue(audioVisual, "currentGradient", Convert.ToInt32(audioObj));
                ...
                if (ioData.TryGetValue("connectedTo", out audioObj))
                {
                    ...
                    if (oldId != 0 && pasteData.EntityLookup.TryGetValue(oldId, out var newConnectedTo) && newConnectedTo.TryGetValue("newId", out audioObj))
                        CopyPasteReflectionHelpers.TrySetAudioVisualConnectedToUid(audioVisual, Convert.ToUInt64(audioObj));
                }
            }
        ...
        private void PopulateInventory(PasteData pasteData, Dictionary<string,object> data, BaseEntity entity, ItemContainer inventory)
        {
            ...
            foreach (var itemDef in items)
            {
                ...
                var itemid = item.TryGetValue("id", out getObj) && int.TryParse(getObj?.ToString(), out var iInt) ? iInt : 0;
                ...
                var dataInt = item.TryGetValue("dataInt", out getObj) && int.TryParse(getObj?.ToString(), out var dInt) ? dInt : 0;
                var dataFloat = item.TryGetValue("dataFloat", out getObj) && float.TryParse(getObj?.ToString(), out var dFloat) ? dFloat : 0f;
                ...
                if (i != null)
                {
                    if (i.hasCondition && !i.info.HasComponent<ItemModFoodSpoiling>()) // ItemModFoodSpoiling handles condition using dataFloat, setting condition here will break the item
                    {
            ...
            Action<BaseEntity> callbackSpawned = null)
        {
            ...
            var data = HarmonyModInterface.Mods.DataFileSystem.GetDatafile(path);
        ...
        private void CmdList(IPlayer player, string command, string[] args)
        {
            ...
            var files = HarmonyModInterface.Mods.DataFileSystem.GetFiles(_subDirectory);
