...
namespace SubmersiblePump
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[SubmersiblePump] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[SubmersiblePump] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[SubmersiblePump] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[SubmersiblePump] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[SubmersiblePump] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[SubmersiblePump] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[SubmersiblePump] Permissions Service bind: " + ex.Message);
            }
        }
