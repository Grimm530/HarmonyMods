...
#pragma warning restore CS0649, CS0169
        ...
        {
            if (container == null || !container.IsExists() || _eventController == null)
                return;
            if (container.ShortPrefabName != "bradley_crate")
                return;
            float burnTime = _ins._config.MainConfig.BradleyCrateBurnTime;
            if (burnTime <= 0f)
                return;
            if (Vector3.Distance(_eventController.GetEventPosition(), container.transform.position) > _eventController.EventConfig.ZoneRadius)
                return;
            LockedByEntCrate lockedCrate = container as LockedByEntCrate;
            if (lockedCrate == null)
                return;
            container.Invoke(() => ExtinguishBradleyCrate(lockedCrate), burnTime);
        }

        private static void ExtinguishBradleyCrate(LockedByEntCrate crate)
        {
        ...
        /// <summary>
        /// Prevents the Travelling Vendor (and any other caller) from destroying Armored Train event wagons.
        /// When the vendor drives into a stopped train at a road/track intersection, its BuildingCheck
        /// would otherwise Kill(DestroyMode.Gib) any TrainCar in range, ending the event.
        /// </summary>
        private object OnEntityKill(TrainCar trainCar)
        {
            if (trainCar == null || trainCar.net == null)
                return null;

            if (!EventLauncher.IsEventActive() || _eventController == null)
                return null;

            if (_eventController.IsTrainWagon(trainCar.net.ID.Value))
                return true; // Block kill - this is an Armored Train event wagon
        ...
        private void OnCorpsePopulate(ScientistNPC scientistNpc, NPCPlayerCorpse corpse)
        {
            ...
            if (npcConfig == null || !npcConfig.DeleteCorpse)
                return;
        ...
        private void ConsoleSaveCustomWagonCommand(ConsoleSystem.Arg arg)
        {
            ...
            string customizationPresetName = arg.Args[0].ToString();
            string wagonShortPrefabName = arg.Args[1].ToString();
        ...
        #region Methods
        /// <summary>
        /// Locate a PNG under HarmonyData / HarmonyImages (plugin folder first, then shared Images).
        /// Returns absolute path or null if not found.
        /// </summary>
        private static string ResolveDataImagePath(string imageName)
        {
            ...
            string[] candidates =
            {
                ...
                Path.Combine(root, "HarmonyImages", "ArmoredTrain", fileName),
                Path.Combine(root, "HarmonyImages", fileName),
            };
            ...
            UnityEngine.Debug.LogWarning("[ArmoredTrain] Image not found: " + fileName + " (searched under " + dataRoot + " and " + root + ")");
            return null;
        }

        /// <summary>Replacement for obsolete BaseEntity.SetFlag — SetFlagLocal + network flags update.</summary>
        private static void ApplyEntityFlag(BaseEntity entity, BaseEntity.Flags flag, bool value)
        {
            if (entity == null || entity.IsDestroyed) return;
            using (entity.StartSetFlags(BaseEntity.FlagsUpdateMode.SendNetworkUpdate_Flags))
                entity.SetFlagLocal(flag, value);
        }
        ...
        private void UpdateConfig()
        {
            if (_config.VersionConfig != Version)
            {
                ...
                {
                    if (_config.VersionConfig.Patch <= 2)
                    {
                        ...
                        try
                        {
                            ...
                            if (File.Exists(configPath))
                            {
                                ...
                                JObject customizationConfig = root["Customization Config"] as JObject;
                                if (customizationConfig != null)
                                {
                                    JToken profileNameToken = customizationConfig["Profile Name"];
                                    if (profileNameToken != null)
                                    {
                                        string profileName = profileNameToken.ToString();
                                        if (profileName == "Halloween")
                                        {
                                            _config.CustomizationConfig.IsHalloween = true;
                                            _config.CustomizationConfig.IsChristmas = false;
                                        }
                                        else if (profileName == "NewYear")
                                        {
                                            _config.CustomizationConfig.IsHalloween = false;
                                            _config.CustomizationConfig.IsChristmas = true;
                                        }
                                    }
                                }
                            }
                        ...
                        {
                            PrintWarning($"Failed to migrate ProfileName: {ex.Message}");
                        }
                    }

                    if (_config.VersionConfig.Patch <= 3)
                    {
                        // Version migration code removed
                    }
                }
        ...
        private static class EventLauncher
        {
            ...
            public static void AutoStartEvent()
            {
                ...
                {
                    UnityEngine.Debug.Log("[ArmoredTrain] Auto-event disabled in Main Setting.");
                    return;
                }

                if (ServerMgr.Instance == null)
                {
                    UnityEngine.Debug.LogWarning("[ArmoredTrain] AutoStartEvent called before ServerMgr ready; skipping.");
                    return;
                }
                ...
                int min = Math.Max(1, _ins._config.MainConfig.MinTimeBetweenEvents);
                int max = Math.Max(min, _ins._config.MainConfig.MaxTimeBetweenEvents);
                _autoEventCoroutine = ServerMgr.Instance.StartCoroutine(AutoEventCoroutine());
                UnityEngine.Debug.Log("[ArmoredTrain] Auto-event timer armed. Next start in " + min + "-" + max + " sec (after ServerMgr ready).");
            }

            private static IEnumerator AutoEventCoroutine()
            {
                // Use float Range so min==max still waits that many seconds (Unity int Range max is exclusive).
                int min = Math.Max(1, _ins._config.MainConfig.MinTimeBetweenEvents);
                int max = Math.Max(min, _ins._config.MainConfig.MaxTimeBetweenEvents);
                float wait = UnityEngine.Random.Range((float)min, (float)max);
                UnityEngine.Debug.Log("[ArmoredTrain] Auto-event: waiting " + (int)wait + " sec until next start.");
                yield return CoroutineEx.waitForSeconds(wait);
                yield return CoroutineEx.waitForSeconds(5f);
                UnityEngine.Debug.Log("[ArmoredTrain] Auto-event: starting train.");
        ...
        private class EventController : FacepunchBehaviour
        {
            ...
            private readonly HashSet<StorageContainer> _containers = new HashSet<StorageContainer>();
            ...
            {
                return _ins._config.MainConfig.IsAggressive || _aggressiveTime > 0 || _stopTime > 0 || _driver == null || !_driver.IsExists() || _driver.IsDead();
            }
            ...
            {
                return _driver != null && _driver.IsExists();
            }
            ...
            public Vector3 GetEventPosition()
            {
                ...
                return count > 0 ? resultPositon / count : Vector3.zero;
            }
            ...
            {
                if (_trainEngine == null || _lastWagon == null) return;
                if (_trainEngine.coupling != null) _trainEngine.coupling.Uncouple(true);
                if (_lastWagon.coupling != null) _lastWagon.coupling.Uncouple(false);
            }
            ...
            private IEnumerator EntitiesSpawnCoroutine()
            {
                ...
                foreach (WagonData wagonData in _wagonDatas)
                {
                    ...
                    {
                        BradleyConfig bradleyConfig = _ins._config.BradleysConfigs.FirstOrDefault(x => x.PresetName == bradleyData.Key);
            ...
            private void SpawnCrate(CrateConfig crateConfig, LocationConfig locationConfig, BaseEntity trainCar)
            {
                ...
                {
                    ApplyEntityFlag(crateEntity, BaseEntity.Flags.Reserved8, false);
                }
            }
            ...
            {
                if (_driver == null || !_driver.IsExists())
                {
            ...
            {
                foreach (StorageContainer storageContainer in _containers)
                {
                    if (storageContainer.IsExists() && storageContainer is HackableLockedCrate or SupplyDrop)
                    {
            ...
            private void KillTrain()
            {
                ...
                if (_turrets != null)
                {
                    foreach (AutoTurret autoTurret in _turrets)
                    {
                        if (autoTurret != null && autoTurret.IsExists())
                        {
                ...
                if (_npcDatas != null)
                {
                    foreach (NpcData npcData in _npcDatas)
                    {
                        if (npcData?.ScientistNpc != null && npcData.ScientistNpc.IsExists())
                            npcData.ScientistNpc.Kill();
                    }
                }
        ...
        private class WagonCustomizer : FacepunchBehaviour
        {
            ...
            {
                ApplyEntityFlag(entity, BaseEntity.Flags.Busy, true);
                ApplyEntityFlag(entity, BaseEntity.Flags.Locked, true);
            ...
            private void UpdateCommonEntities(BaseEntity entity)
            {
                if (entity.ShortPrefabName == "skulltrophy.deployed")
                    ApplyEntityFlag(entity, BaseEntity.Flags.Reserved1, true);
            ...
            private void UpdateLightEntity(BaseEntity entity)
            {
                if (_ins._config.CustomizationConfig.IsLightOnlyAtNight)
                    ...
                    ApplyEntityFlag(entity, BaseEntity.Flags.On, true);
            }

            private static void DecorateChristmasTree(BaseEntity christmasTree)
            {
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved1, true);
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved2, true);
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved3, true);
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved4, true);
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved5, true);
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved6, true);
                ApplyEntityFlag(christmasTree, BaseEntity.Flags.Reserved7, true);
            }
            ...
            private void TurnLight(bool enable)
            {
                ...
                foreach (BaseEntity entity in _lightEntities)
                    if (entity.IsExists())
                        ApplyEntityFlag(entity, BaseEntity.Flags.On, enable);
            }
            ...
            private class ItemThrower
            {
                ...
                private static LootItemConfig GetItemForThrowing()
                {
                    int counter = 0;

                    while (counter < 100)
                    {
                        LootItemConfig itemConfig = _ins._config.CustomizationConfig.GiftCannonSetting.Items.GetRandom();

                        if (Random.Range(0.0f, 100.0f) <= itemConfig.Chance)
                            return itemConfig;
                        counter++;
                    }
                ...
                private static Item CreateItemByItemConfig(LootItemConfig itemConfig)
                {
            ...
            private static class SignPainter
            {
                ...
                {
                    string filePath = ResolveDataImagePath(imageName);
                    if (filePath == null)
                    {
                        _ins.PrintWarning(imageName + ".png was not found under HarmonyData/ArmoredTrain/Images/ (or HarmonyData/Images/, HarmonyImages/).");
                        yield break;
                    }
                    ...
                    {
                        _ins.PrintWarning("Failed to load neon sign image " + imageName + ": " + unityWebRequest.error);
                        yield break;
                    }

                    Texture2D tex = DownloadHandlerTexture.GetContent(unityWebRequest);
                    if (tex == null || neonSign == null || !neonSign.IsExists())
                    {
        ...
        private class EventHeli : FacepunchBehaviour
        {
            ...
            {
                return _eventHeli != null && _eventHeli._patrolHelicopter.IsExists();
            }
        ...
        private class TurretOptimizer : FacepunchBehaviour
        {
            ...
            public void OptimizedTargetTick()
            {
                ...
                {
                    _autoTurret.nextVisCheck = Time.realtimeSinceStartup + Random.Range(0.2f, 0.3f);
                ...
                if (Time.time >= _autoTurret.nextShotTime && _autoTurret.targetVisible && Mathf.Abs(_autoTurret.AngleToTarget(_autoTurret.target, _autoTurret.currentAmmoGravity != 0f)) < _autoTurret.GetMaxAngleForEngagement())
                {
                    if ((bool)attachedWeapon)
                    {
                        if (attachedWeapon.primaryMagazine.contents > 0)
                        {
                            _autoTurret.FireAttachedGun(_autoTurret.AimOffset(_autoTurret.target), _autoTurret.aimCone, _autoTurret.PeacekeeperMode() ? _autoTurret.target : null);
                ...
                if (_autoTurret.target != null && (!targetPlayer.IsRealPlayer() || _autoTurret.target.IsDead() || Time.realtimeSinceStartup - _autoTurret.lastTargetSeenTime > 3f || Vector3.Distance(_autoTurret.transform.position, _autoTurret.target.transform.position) > _autoTurret.sightRange || (_autoTurret.PeacekeeperMode() && !_autoTurret.IsEntityHostile(_autoTurret.target))))
        ...
        private class EventMapMarker : FacepunchBehaviour
        {
            ...
            private void UpdateVendingMarker(Vector3 position)
            {
                ...
                ApplyEntityFlag(_vendingMarker, BaseEntity.Flags.Busy, pveModeEventOwner == null);
                _vendingMarker.SendNetworkUpdate();
            }
        ...
        private static class NpcSpawnManager
        {
            ...
            private static ScientistNPC SpawnScientistNpc(NpcConfig npcConfig, Vector3 position, float healthFraction, bool isStationary, bool isPassive)
            {
                ...
                try
                {
                    if (npcConfig == null)
                    {
                        NotifyManager.PrintError(null, "NpcConfig is null!");
                        return null;
                    }

                    if (_ins.NpcSpawn == null)
                    {
                        NotifyManager.PrintError(null, "NpcSpawn plugin reference is null! Cannot spawn NPCs.");
                        return null;
                    }

                    object baseNpcConfigObj = GetBaseNpcConfig(npcConfig, healthFraction, isStationary, isPassive);
                    if (baseNpcConfigObj == null)
                    {
                        NotifyManager.PrintError(null, $"Failed to create config object for NPC '{npcConfig.DisplayName}'");
                        return null;
                    }

                    // Invoke SpawnNpc directly (same as Convoy) so Oxide.Call() dispatch/overload doesn't lose JObject type
                    object spawnResult = null;
                    try
                    {
                        Type npcSpawnType = _ins.NpcSpawn.GetType();
                        var method = npcSpawnType.GetMethod("SpawnNpc", BindingFlags.Public | BindingFlags.Instance, null, new Type[] { typeof(Vector3), typeof(object) }, null);
                        if (method != null)
                            spawnResult = method.Invoke(_ins.NpcSpawn, new object[] { position, baseNpcConfigObj });
                        if (spawnResult == null && method == null)
                            spawnResult = _ins.NpcSpawn.Call("SpawnNpc", position, baseNpcConfigObj);
                    }
                    catch (Exception ex)
                    {
                        _ins.PrintWarning($"SpawnScientistNpc: Exception calling NpcSpawn.SpawnNpc for '{npcConfig.PresetName}': {ex.Message}");
                        return null;
                    }

                    if (spawnResult == null)
                    {
                        _ins.PrintWarning($"NpcSpawn.Call('SpawnNpc') returned null for '{(npcConfig?.DisplayName ?? "NPC")}' at position {position}");
                        return null;
                    }

                    ScientistNPC scientistNpc = spawnResult as ScientistNPC;
                    if (scientistNpc == null)
                    {
                        NotifyManager.PrintError(null, $"NpcSpawn.Call('SpawnNpc') returned unexpected type '{spawnResult.GetType().Name}' for '{npcConfig.DisplayName}'");
                        return null;
                    }

                    if (!scientistNpc.IsExists())
                    {
                        NotifyManager.PrintError(null, $"NPC spawned but IsExists() returned false for '{npcConfig.DisplayName}'");
                        return null;
                    }
                ...
                {
                    _ins?.PrintWarning($"SpawnScientistNpc: {ex.Message}");
                    return null;
                }
            }
            ...
            private static object GetBaseNpcConfig(NpcConfig config, float healthFraction, bool isStationary, bool isPassive)
            {
                ...
                if (wearItems == null || wearItems.Count == 0)
                {
                    ...
                    {
                        foreach (var x in config.WearItems)
                            wearItems.Add(new JObject { ["ShortName"] = x.ShortName, ["SkinID"] = x.SkinID });
                    }
                }

                JArray beltItemsArray = new JArray();
                if (!isPassive && config.BeltItems != null && config.BeltItems.Count > 0)
                {
                    foreach (var x in config.BeltItems)
                        beltItemsArray.Add(new JObject
                        {
                            ["ShortName"] = x.ShortName,
                            ["Amount"] = x.Amount,
                            ["SkinID"] = x.SkinID,
                            ["mods"] = x.Mods != null ? new JArray(x.Mods) : new JArray(),
                            ["Ammo"] = x.Ammo ?? string.Empty
                        });
                }

                JArray statesArray;
                bool hasRaidWeapons = false;
                if (config.BeltItems != null)
                {
                    foreach (var b in config.BeltItems)
                    {
                        if (b.ShortName == "rocket.launcher" || b.ShortName == "explosive.timed")
                ...
                if (isPassive && isStationary)
                    ...
                    statesArray = new JArray("IdleState", "CombatStationaryState");
                else if (hasRaidWeapons)
                    statesArray = new JArray("RaidState", "RoamState", "ChaseState", "CombatState");
                else
                    statesArray = new JArray("RoamState", "ChaseState", "CombatState");

                return new JObject
                {
                    ["Name"] = config.DisplayName ?? string.Empty,
                    ["WearItems"] = wearItems,
                    ["BeltItems"] = beltItemsArray,
                    ["Kit"] = config.Kit ?? string.Empty,
                    ...
                    ["MeleeDamageScale"] = config.MeleeDamageScale
                };
            }
        ...
        private static class NotifyManager
        {
            ...
            public static void SendMessageToPlayer(BasePlayer player, string langKey, params object[] args)
            {
                ...
                if (_ins._config.NotifyConfig.GameTipConfig.IsEnabled)
                    player.SendConsoleCommand("gametip.showtoast_translated", _ins._config.NotifyConfig.GameTipConfig.Style, "armoredtrain.tip", ClearColorAndSize(playerMessage), false, System.Array.Empty<string>());
        ...
        private static class GuiManager
        {
            ...
            {
                if (!TryLoadPngIntoFileStorage(TabImageInfo.ImageName, out string imageId))
                {
            ...
            private static IEnumerator LoadIconsCoroutine()
            {
                ...
                {
                    if (!TryLoadPngIntoFileStorage(imageInfo.ImageName, out string imageId))
                    {
            ...
            /// <summary>
            /// Resolve GUI/sign PNGs under HarmonyData (preferred) then HarmonyImages.
            /// Does not throw on missing files — returns false so callers can soft-fail.
            /// </summary>
            private static bool TryLoadPngIntoFileStorage(string imageName, out string imageId)
            {
                imageId = null;
                string filePath = ResolveDataImagePath(imageName);
                if (filePath == null)
                ...
                try
                {
                    ...
                    if (bytes == null || bytes.Length == 0)
                        return false;
                    if (CommunityEntity.ServerInstance == null || CommunityEntity.ServerInstance.net == null)
                        return false;
                    uint storedId = FileStorage.server.Store(bytes, FileStorage.Type.png, CommunityEntity.ServerInstance.net.ID);
                    imageId = storedId.ToString();
                    return true;
                }
                catch (Exception ex)
                {
                    UnityEngine.Debug.LogWarning("[ArmoredTrain] Failed reading GUI image '" + imageName + "': " + ex.Message);
                    return false;
                }
            }

            private static void OnImageSaveFailed(string missingName = null)
            {
                string hint = string.IsNullOrEmpty(missingName) ? "Tab_Adem.png / Clock_Adem.png / Crates_Adem.png / Soldiers_Adem.png" : (missingName + ".png");
                UnityEngine.Debug.LogWarning("[ArmoredTrain] GUI image missing: " + hint + ". Place PNGs in HarmonyData/ArmoredTrain/Images/ (or HarmonyData/Images/). Countdown GUI disabled; event continues.");
                _isLoadingImageFailed = true;
            }

            public static void CreateGui(BasePlayer player, params string[] args)
            {
                if (!_ins._config.GUIConfig.IsEnable || _isLoadingImageFailed)
                    return;
                if (string.IsNullOrEmpty(TabImageInfo.ImageId))
                    return;
        ...
        private class MainConfig
        {
            ...
            [JsonProperty(En ? "Time until Bradley crates stop burning [sec] (0 = game default)" : "Время до прекращения горения ящиков Bradley [сек] (0 = по умолчанию)")] 
            public float BradleyCrateBurnTime { get; set; }
        ...
        private class NpcConfig
        {
            ...
            [JsonProperty("DisplaySashTargetsOnly")] public bool DisplaySashTargetsOnly { get; set; }
            [JsonProperty("IgnoreSafeZonePlayers")] public bool IgnoreSafeZonePlayers { get; set; } = true;
            [JsonProperty("IgnoreSleepingPlayers")] public bool IgnoreSleepingPlayers { get; set; }
            [JsonProperty("IgnoreWoundedPlayers")] public bool IgnoreWoundedPlayers { get; set; }
            [JsonProperty("Underwear")] public uint Underwear { get; set; }
            [JsonProperty("InstantDeathIfHitHead")] public bool InstantDeathIfHitHead { get; set; }
            [JsonProperty("DestroyTrapsOnDeath")] public bool DestroyTrapsOnDeath { get; set; }
            [JsonProperty("HeadDamageScale")] public float HeadDamageScale { get; set; } = 1f;
            [JsonProperty("BodyDamageScale")] public float BodyDamageScale { get; set; } = 1f;
            [JsonProperty("LegDamageScale")] public float LegDamageScale { get; set; } = 1f;
            [JsonProperty("MeleeDamageScale")] public float MeleeDamageScale { get; set; }
        }
        ...
        {
            [JsonProperty(En ? "Type of target" : "Тип цели")] 
            public string Type { get; set; }
            
            [JsonProperty(En ? "Damage Multiplier" : "Множитель урона")] 
        ...
        private class PluginConfig
        {
            ...
            public HashSet<BradleyConfig> BradleysConfigs { get; set; }
            ...
            public static PluginConfig DefaultConfig()
            {
                ...
                {
                    VersionConfig = new VersionNumber(1, 9, 2),
                    Prefix = "[ArmoredTrain]",
