...
Harmony port of Legacy **PveMode 1.2.9**. Named **`0PveMode`** so HarmonyLoader / filesystem order places it before **ArmoredTrain** / **Convoy** (same idea as **0Permissions**).
...
`plugins.Exists("PveMode")` / `plugins.Exists("0PveMode")` both resolve (TruePVE HarmonyCompat alias).
