...
using UnityEngine;
using GrimmCuiHarmony;
using HarmonyPlugin = Harmony.Plugins.RocketGuidanceSystem;
using GrimmCuiHarmony;
...
namespace RocketGuidanceSystemHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("RocketGuidanceSystem_Runner");
    ...
    public class RocketGuidanceSystemMod : IHarmonyModHooks
    {
        public static RocketGuidanceSystemMod Instance { get; private set; }
        ...
        public const string AppDomainApiKey = "RocketGuidanceSystem_ApiType";
        public const string CuiMarker = "RGS";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[RocketGuidanceSystem] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(RocketGuidanceSystemMod)); }
            catch { }

            RegisterAttributedChatCommands();
            ChatSayBridge.Register("RocketGuidanceSystem", OnChatCommand);
            ...
            Debug.Log("[RocketGuidanceSystem] Harmony mod loaded. Config: HarmonyConfig/RocketGuidanceSystem.json");
        }
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] Permissions ready: " + ex.Message);
            }
        }
        ...
        public void OnUnloaded(OnHarmonyModUnloadedArgs args)
        {
            ...
            Instance = null;
            Debug.Log("[RocketGuidanceSystem] Harmony mod unloaded.");
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[RocketGuidanceSystem] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            catch (Exception ex) { Debug.LogWarning("[RocketGuidanceSystem] ResolvePluginReferences: " + ex.Message); }
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[RocketGuidanceSystem] Init: " + ex.Message); }
            try { plugin.CallLoaded(); }
            catch (Exception ex) { Debug.LogWarning("[RocketGuidanceSystem] Loaded: " + ex.Message); }
            try { plugin.CallOnServerInitialized(); }
            catch (Exception ex) { Debug.LogError("[RocketGuidanceSystem] OnServerInitialized: " + ex); }
            RefreshDynamicCommands();
            _initCoroutine = null;
            Debug.Log("[RocketGuidanceSystem] Server initialized.");
        }
        ...
        private static void InvokeChatMethod(HarmonyPlugin plugin, string methodName, BasePlayer player, string command, string[] args, bool attributedOnly = false)
        {
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[RocketGuidanceSystem] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            try
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] cui.endtest RGS: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[RocketGuidanceSystem] " + methodName + ": " + (ex.InnerException ?? ex).Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[RocketGuidanceSystem] cmd " + captured + ": " + ex.Message); }
                }
            };
