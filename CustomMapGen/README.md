...
- **Jungle monument relocation**: Jungle-bound monuments (`jungle_ziggurat`, `jungle_ruins_*`) stay in the **Jungle** biome when relocated for terrain-blend clearance — the search pushes them to a nearby blank Jungle spot instead of dumping them mid-map. Tiny monuments (water wells, etc.) use a 30m radius for clearance checks (not the outpost 120m radius). If a large non-jungle monument (e.g. Airfield) cannot find a blank slot, it may **take** a jungle monument’s roomy clearing and push that jungle monument nearby within Jungle (not a 1:1 swap onto the airfield’s old non-jungle position).
