 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to KillFeedHarmony,
 * log tag changed to [KillFeed].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace KillFeedHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[KillFeed] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[KillFeed] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[KillFeed] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[KillFeed] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[KillFeed] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[KillFeed] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[KillFeed] Permissions Service bind: " + ex.Message);
            }
        }
