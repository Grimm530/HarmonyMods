// HarmonyCompat.cs  --  Legacy -> Harmony shims for the KillFeed port.
...
// KillFeed plugin compiles without legacy plugin host runtime.
...
namespace Harmony.Core.Configuration
{
    ...
    public class DynamicConfigFile : IEnumerable<KeyValuePair<string, object>>
    {
        ...
        public void Save()
        {
            ...
            {
                Debug.LogWarning("[KillFeed] DynamicConfigFile.Save: " + ex.Message);
            }
        }
...
namespace Harmony.Core
{
    ...
    public static class HarmonyModInterface
    {
        ...
        private static object TryAppDomainBool(string key, object arg0, object arg1, string logLabel)
        {
            ...
            {
                Debug.LogWarning("[KillFeed] " + logLabel + " bridge: " + ex.Message);
            }
            return null;
        }
        ...
        private static IEnumerator NextTickCoroutine(Action action)
        {
            yield return null;
            try { action?.Invoke(); }
            catch (Exception ex) { Debug.LogWarning("[KillFeed] NextTick: " + ex.Message); }
        }
    }
    ...
    public class HarmonyModRuntime
    {
        ...
        public void LogInfo(string message) => Debug.Log("[KillFeed] " + message);
        // Format overload: caller (TruePVE.Puts) already supplies its own "[KillFeed]" tag.
        public void LogInfo(string format, params object[] args)
            => Debug.Log(SafeFormat(format, args));
        public void LogWarning(string message) => Debug.LogWarning("[KillFeed] " + message);
        public void LogWarning(string format, params object[] args)
            => Debug.LogWarning(SafeFormat(format, args));
        public void LogError(string message)   => Debug.LogError("[KillFeed] " + message);
        public void LogError(string format, params object[] args)
            => Debug.LogError(SafeFormat(format, args));
        public void LogDebug(string message) => Debug.Log("[KillFeed] " + message);
        ...
        public void UnloadPlugin(string name)
            => Debug.LogWarning("[KillFeed] UnloadPlugin('" + name + "') ignored under Harmony.");
        public void ReloadPlugin(string name)
            => Debug.LogWarning("[KillFeed] ReloadPlugin('" + name + "') ignored under Harmony.");
    ...
    public class DataFileSystem
    {
        ...
        public void WriteObject<T>(string name, T obj, bool sync = false)
        {
            ...
            catch (Exception ex) { Debug.LogWarning("[KillFeed] WriteObject: " + ex.Message); }
        }
...
namespace Harmony.Core.Libraries
{
    public class Lang
    {
        ...
        public string GetLanguage(string userId = null) => "en";
        public string[] GetLanguages(object plugin = null) => new[] { "en", "de", "ru" };
    }
...
namespace Harmony.Core.Libraries.Covalence
{
    ...
    public class BasePlayerWrapper : IPlayer
    {
        ...
        public bool HasPermission(string perm)
        {
            ...
            return KillFeedHarmony.PermissionsBridge.UserHasPermission(_p.UserIDString, perm);
        }
    }
    ...
    public class RustConsolePlayer : IPlayer
    {
        ...
        public void Reply(string msg) => Debug.Log("[KillFeed] " + msg);
...
namespace Harmony.Core.Plugins
{
    ...
    public sealed class PluginBridgeApi : Plugin
    {
        ...
        public override object Call(string method, params object[] args)
        {
            ...
            {
                Debug.LogWarning("[KillFeed] Plugin.Call " + Name + "." + method + ": " + (ex.InnerException?.Message ?? ex.Message));
            }
            return null;
        }
...
namespace Harmony.Plugins
{
    ...
    public class ServerShim
    {
        public void Command(string cmd, params object[] args)
        {
            ...
            catch (Exception ex) { Debug.LogWarning("[KillFeed] Server.Command: " + ex.Message); }
        }
    ...
    /// <summary>
    /// Harmony permission.* surface backed by KillFeedHarmony.PermissionsBridge
    ...
    public class PermissionLib : Harmony.Core.Libraries.Permission
    {
        public void RegisterPermission(string perm, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.RegisterPermission(perm);

        public bool PermissionExists(string perm)
            => KillFeedHarmony.PermissionsBridge.PermissionExists(perm);
        ...
        public bool UserHasPermission(string userId, string perm)
            => KillFeedHarmony.PermissionsBridge.UserHasPermission(userId, perm);

        public void GrantUserPermission(string userId, string perm, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.GrantUserPermission(userId, perm);

        public void RevokeUserPermission(string userId, string perm, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.RevokeUserPermission(userId, perm);

        public string[] GetGroupPermissions(string group)
            => KillFeedHarmony.PermissionsBridge.GetGroupPermissions(group);

        public string[] GetUsersInGroup(string group)
            => KillFeedHarmony.PermissionsBridge.GetUsersInGroup(group);

        public string[] GetGroups()
            => KillFeedHarmony.PermissionsBridge.GetGroups();

        public bool GroupExists(string group)
            => KillFeedHarmony.PermissionsBridge.GroupExists(group);

        public bool CreateGroup(string name, string title, int rank)
            => KillFeedHarmony.PermissionsBridge.CreateGroup(name, title, rank);

        public bool GroupHasPermission(string group, string perm)
            => KillFeedHarmony.PermissionsBridge.GroupHasPermission(group, perm);

        public bool GrantGroupPermission(string group, string perm, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.GrantGroupPermission(group, perm);

        public bool RevokeGroupPermission(string group, string perm, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.RevokeGroupPermission(group, perm);

        public bool UserHasGroup(string userId, string group)
            => KillFeedHarmony.PermissionsBridge.UserHasGroup(userId, group);

        public void AddUserGroup(string userId, string group, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.AddUserGroup(userId, group);

        public void RemoveUserGroup(string userId, string group, object plugin = null)
            => KillFeedHarmony.PermissionsBridge.RemoveUserGroup(userId, group);

        public string[] GetUserGroups(string userId)
            => KillFeedHarmony.PermissionsBridge.GetUserGroups(userId);
    }
    ...
    public class TimerLib
    {
        ...
        private IEnumerator RunOnce(float secs, Timer t, Action cb)
        {
            ...
            try { cb(); } catch (Exception ex) { Debug.LogWarning("[KillFeed] Timer: " + ex.Message); }
            t.Destroy();
    ...
    public abstract partial class RustPlugin
    {
        ...
        // Logging
        private string Tag => "[" + (Title ?? Name ?? "KillFeed") + "]";
        ...
        public void LogToFile(string filename, string text, object plugin, bool timestamp, bool extraFlag)
        {
            try
            {
                ...
                var dir   = Path.Combine(root, "HarmonyData", Name ?? "KillFeed", "logs");
                if (!System.IO.Directory.Exists(dir)) System.IO.Directory.CreateDirectory(dir);
                var path  = Path.Combine(dir, (filename ?? Name ?? "KillFeed") + ".txt");
        ...
        public void NextTick(Action action)
        {
            ...
            KillFeedHarmony.ModRunner.Enqueue(action);
        }
    ...
    public class WebRequests
    {
        ...
        public void Enqueue(string url, string body, Action<int, string> callback,
                            object owner,
                            Harmony.Core.Libraries.RequestMethod method,
        ...
        private static IEnumerator Send(string url, string body, Action<int, string> callback,
            Harmony.Core.Libraries.RequestMethod method, Dictionary<string, string> headers)
        {
            using var req = new UnityEngine.Networking.UnityWebRequest(url,
                method == Harmony.Core.Libraries.RequestMethod.GET ? "GET" : "POST");
            ...
            else if (method != Harmony.Core.Libraries.RequestMethod.GET)
