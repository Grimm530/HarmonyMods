...
{
    /// <summary>
    /// Minimal compat API stand-ins so NpcSpawn logic can run as a Harmony mod without Harmony.Core references.
    /// </summary>
    public static class HarmonyCompat
    {
        ...
        /// <summary>Replaces HarmonyModInterface.Mods.DataDirectory for NpcSpawn folder layout under HarmonyConfig.</summary>
        ...
        public static T ReadConfig<T>() where T : class, new()
        {
            try
            {
                ...
                if (File.Exists(oxideCfg))
                {
                    ...
                    {
                        WriteConfig(cfg);
                        Debug.Log("[GrimmNPC] Migrated config from legacy/config/NpcSpawn.json");
                        return cfg;
                    }
                }
            }
        ...
        public static object CallHook(string hook, params object[] args)
        {
            ...
            // reach them even when Harmony.Core.HarmonyModInterface is a no-op or belongs to another assembly.
            try
            {
            ...
            try
            {
                if (_callHook == null)
                {
                    ...
                    {
                        Type t = asm.GetType("Harmony.Core.HarmonyModInterface");
                        if (t == null) continue;
        ...
        /// <summary>
        /// Harmony ModReference stand-in. For "Kits", also binds the Harmony Kits mod via
