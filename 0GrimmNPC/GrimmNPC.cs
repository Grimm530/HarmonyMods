    ...
    public class GrimmNPC : IHarmonyModHooks
    {
        ...
        public class CustomScientistNpc : ScientistNPC
        {
            ...
            public new BaseEntity GetBestTarget()
            {
                ...
                float lostRange = Brain.TargetLostRange > 0f ? Brain.TargetLostRange : Brain.SenseRange * 2f;
                float now = Time.realtimeSinceStartup;
                float memoryDuration = Brain.MemoryDuration > 0f ? Brain.MemoryDuration : 5f;
                float listenRange = Brain.ListenRange;
                ...
                foreach (SimpleAIMemory.SeenInfo info in memoryAll)
                {
                    ...
                    // Do NOT require CanSeeTarget to keep a memory target.
                    // Requiring LOS here cleared damage-aggro every Think tick until the player
                    // wandered into a perfect raycast — felt like a multi-second reaction delay.
                    // Chase uses live entity position; CombatState/CombatStationary still gate fire on LOS.
                    float dist = Vector3.Distance(entity.transform.position, transform.position);
                    if (dist > lostRange) continue;
                    if (entity != CurrentTarget
                        && !CanSeeTarget(entity)
                        && (now - info.Timestamp) > memoryDuration
                        && dist > listenRange)
                        continue;
            ...
            public new class CombatState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (_npc.CurrentWeapon is BaseProjectile)
                    {
                        ...
                        {
                            // Close enough - strafe/duck on timer only (fire every think below)
                        ...
                        // Fire every think while in combat — weapon AttackEntity rate-limits ShotTest.
                        // Previously fire was gated on the 1-3s strafe timer, which felt extremely slow.
            ...
            public new class CombatStationaryState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (_npc.CurrentWeapon is BaseProjectile)
                    {
                        ...
                        // Fire every think — do not wait on strafe timer (mounted convoy / stationary).
                        if (_npc.CurrentWeapon is BaseLauncher) _npc.FireGrenadeLauncher();
                        else
                        {
                            _npc.IsAttackingBaseProjectile = true;
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            UnityEngine.Debug.Log("[GrimmNPC] 0GrimmNPC loaded (NpcSpawn Harmony port 3.3.04). Config: HarmonyConfig/GrimmNPC.json.");
        }
        ...
        internal object OnEntityTakeDamage(BaseCombatEntity victim, HitInfo info)
        {
            ...
            if (IsCustomScientist(victim))
            {
                ...
                if (attacker is BasePlayer playerAttacker && playerAttacker.IsPlayer())
                {
                    ...
                    // Immediate damage-aggro: do not wait for senses/LOS or assist timers
                    victimNpc.SetKnown(playerAttacker);
                    victimNpc.CurrentTarget = playerAttacker;
                    ...
                    for (int i = 0; i < nearbyCount; i++)
                    {
                        ...
                        // Close allies react immediately; far ones get a short delay (was up to 5s).
                ...
                if (attacker is BasePlayer)
                {
                    ...
                    if (attackerBp.userID.IsSteamId())
                    {
                        ...
                        victimNpc.CurrentTarget = attackerBp;
