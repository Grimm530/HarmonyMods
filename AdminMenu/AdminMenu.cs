...
{
	[Info("AdminMenu", "k1lly0u", "2.1.13")]
    public class AdminMenu : AdminMenuPluginBase
    {
	    ...
	    [Permission] private const string USE_PERMISSION = "adminmenu.use";
	    [Permission] private const string PERM_PERMISSION = "adminmenu.permissions";
	    [Permission] private const string GROUP_PERMISSION = "adminmenu.groups";
	    [Permission] private const string CONVAR_PERMISSION = "adminmenu.convars";
	    [Permission] private const string PLUGIN_PERMISSION = "adminmenu.plugins";

	    [Permission] private const string GIVE_PERMISSION = "adminmenu.give";
	    [Permission] private const string GIVE_SELF_PERMISSION = "adminmenu.give.selfonly";
	    [Permission] private const string PLAYER_PERMISSION = "adminmenu.players";

	    [Permission] private const string PLAYER_KICKBAN_PERMISSION = "adminmenu.players.kickban";
	    [Permission] private const string PLAYER_MUTE_PERMISSION = "adminmenu.players.mute";
	    [Permission] private const string PLAYER_BLUERPRINTS_PERMISSION = "adminmenu.players.blueprints";
	    [Permission] private const string PLAYER_HURT_PERMISSION = "adminmenu.players.hurt";
	    [Permission] private const string PLAYER_HEAL_PERMISSION = "adminmenu.players.heal";
	    [Permission] private const string PLAYER_KILL_PERMISSION = "adminmenu.players.kill";
	    [Permission] private const string PLAYER_STRIP_PERMISSION = "adminmenu.players.strip";
	    [Permission] private const string PLAYER_TELEPORT_PERMISSION = "adminmenu.players.teleport";
	    ...
	    {
		    m_RecentPlayers = new DataFile<RecentPlayers>("recent_players");
		    m_RecentPlayers.Data ??= new RecentPlayers();
		    ...
		    AdminMenuImages.TryLoad();
		    m_MagnifyImage = AdminMenuImages.MagnifyCrc;
		    m_LogoImage = AdminMenuImages.LogoCrc;
	    ...
	    /// <summary>
	    /// Register adminmenu.* and ensure the Permissions "admin" group has them.
	    /// Safe to call again after Permissions reload (ready callback).
	    /// </summary>
	    public void RegisterPermissionsWithPermissionsMod()
	    {
		    string[] builtInPerms =
		    {
			    USE_PERMISSION, PERM_PERMISSION, GROUP_PERMISSION, CONVAR_PERMISSION, PLUGIN_PERMISSION,
			    GIVE_PERMISSION, GIVE_SELF_PERMISSION, PLAYER_PERMISSION,
			    PLAYER_KICKBAN_PERMISSION, PLAYER_MUTE_PERMISSION, PLAYER_BLUERPRINTS_PERMISSION,
			    PLAYER_HURT_PERMISSION, PLAYER_HEAL_PERMISSION, PLAYER_KILL_PERMISSION,
			    PLAYER_STRIP_PERMISSION, PLAYER_TELEPORT_PERMISSION
		    };
	    ...
	    public void OnPlayerConnected(BasePlayer player) => m_RecentPlayers.Data.OnPlayerConnected(player);
	    ...
	    public void OnServerSave() => m_RecentPlayers?.Save();
	    ...
	    public void CmdAdmin(BasePlayer player, string command, string[] args)
	    {
		    ...
		    // Permissions only — no IsAdmin bypass. Put staff in the Permissions "admin" group
		    // (AdminMenu grants all adminmenu.* to that group on load).
		    if (!player.HasPermission(USE_PERMISSION))
		    {
			    player.ChatMessage(GetString("Error.NoPermission", player));
			    return;
		    }
...
#region Functions
	    private void UpdatePermissionList()
	    {
		    ...
		    plugin.AddRange(plugins.GetPlugins());
	    ...
	    private bool UserHasPermissionNoGroup(string playerId, string perm)
	    {
		    ...
		    UserData userData = permission.GetUserData(playerId);
	    ...
	    protected override void RegisterMessages()
	    {
		    ...
		    for (int i = 0; i < menuTypes.Length; i++)
		    {
			    ...
			    messages[$"Category.{menuType}"] = menuType.ToString();
		    }
		    ...
		    for (int i = 0; i < permissionTypes.Length; i++)
		    {
			    ...
			    messages[$"Permissions.{(int)index}"] = index.ToString();
		    }
		    ...
		    for (int i = 0; i < commandTypes.Length; i++)
		    {
			    ...
			    messages[$"Commands.{(int)index}"] = index.ToString();
		    }
		    messages[$"Commands.{(int)CommandSubType.PlayerInfo}"] = "Player Info";

		    GroupSubType[] groupTypes = (GroupSubType[])Enum.GetValues(typeof(GroupSubType));
		    for (int i = 0; i < groupTypes.Length; i++)
		    {
			    GroupSubType index = groupTypes[i];
			    messages[$"Groups.{(int)index}"] = index.ToString();
		    }
		    messages[$"Groups.{(int)GroupSubType.UserGroups}"] = "User Groups";
		    messages[$"Groups.{(int)GroupSubType.GroupUsers}"] = "Group Users";
		    
		    ItemCategory[] itemCategories = (ItemCategory[])Enum.GetValues(typeof(ItemCategory));
		    for (int i = 0; i < itemCategories.Length; i++)
		    {
			    ItemCategory index = itemCategories[i];
			    messages[$"Give.{(int)index}"] = index.ToString();
		    }
        ...
        public void CreateAdminMenu(BasePlayer player)
        {
        ...
        private void CreateGiveMenu(UIUser uiUser, BaseContainer parent)
        {
	        ...
	        if (uiUser.CommandTarget1 == null)
	        {
		        ...
		        {
			        uiUser.CommandTarget1 = uiUser.Player.ToIPlayer();
			        goto RESTART;
		        }
					        ...
					        .WithChildren(panel =>
					        {
						        ...
						        BaseContainer.Create(panel, UIAnchor.BottomStretch, new Offset(8f, 64f, -8f, 112f))
							        .WithChildren(reason =>
							        {
								        ...
								        ImageContainer.Create(reason, UIAnchor.HorizontalCenterStretch, new Offset(64f, -16f, 0f, 16f))
									        .WithStyle(m_RustSearchFilterStyle)
									        .WithChildren(input =>
									        {
										        InputFieldContainer.Create(input, UIAnchor.FullStretch, new Offset(8f, 0f, -8f, 0f))
        ...
        private void SetupPlayerActions()
        {
	        ...
	        if (Configuration?.PlayerInfoCommands != null)
	        {
							...
							.WithChildren(foreground =>
							{
								...
								BaseContainer.Create(foreground, UIAnchor.BottomStretch, new Offset(8f, 154f, -8f, 202f))
									.WithChildren(name =>
									{
										...
										ImageContainer.Create(name, UIAnchor.HorizontalCenterStretch, new Offset(134f, -16f, 0f, 16f))
											.WithStyle(m_RustSearchFilterStyle)
											.WithChildren(input =>
											{
												InputFieldContainer.Create(input, UIAnchor.FullStretch, new Offset(8f, 0f, -8f, 0f))
								...
								BaseContainer.Create(foreground, UIAnchor.BottomStretch, new Offset(8f, 109f, -8f, 157f))
									.WithChildren(title =>
									{
										...
										ImageContainer.Create(title, UIAnchor.HorizontalCenterStretch, new Offset(134f, -16f, 0f, 16f))
											.WithStyle(m_RustSearchFilterStyle)
											.WithChildren(input =>
											{
												InputFieldContainer.Create(input, UIAnchor.FullStretch, new Offset(8f, 0f, -8f, 0f))
								...
								BaseContainer.Create(foreground, UIAnchor.BottomStretch, new Offset(8f, 64f, -8f, 112f))
									.WithChildren(rank =>
									{
										...
										ImageContainer.Create(rank, UIAnchor.HorizontalCenterStretch, new Offset(134f, -16f, 0f, 16f))
											.WithStyle(m_RustSearchFilterStyle)
											.WithChildren(input =>
											{
												InputFieldContainer.Create(input, UIAnchor.FullStretch, new Offset(8f, 0f, -8f, 0f))
        ...
        {
	        // Refresh in case Permissions / RaidableBases registered more since server init
	        UpdatePermissionList();
	        CreateCharacterFilter(uiUser, parent);
        ...
        private void LogToDiscord(BasePlayer player, string message)
        {
	        ...
	        DiscordEmbed embed = DiscordEmbed.Create()
		        .WithColor(DiscordColor.Blurple)//8471
        ...
        {
	        Configuration = LoadConfigObject<ConfigData>() ?? GenerateDefaultConfiguration();

	        if (Configuration.ChatCommands == null)
		        Configuration.ChatCommands = GenerateDefaultConfiguration().ChatCommands;
	        if (Configuration.ConsoleCommands == null)
		        Configuration.ConsoleCommands = GenerateDefaultConfiguration().ConsoleCommands;
	        if (Configuration.PlayerInfoCommands == null)
		        Configuration.PlayerInfoCommands = GenerateDefaultConfiguration().PlayerInfoCommands;
	        if (Configuration.LogWebhook == null)
	        ...
	        if (Configuration.Version == default)
		        Configuration.Version = new VersionNumber(2, 1, 13);
        ...
        public class ConfigData
        {
            ...
            public VersionNumber Version { get; set; } = new VersionNumber(2, 1, 13);
        ...
        ConfigData GenerateDefaultConfiguration()
        {
        ...
        private class RecentPlayers
        {
	        ...
	        public void OnPlayerConnected(BasePlayer player)
	        {
		        ...
		        if (player.ToIPlayer() != null) 
			        _cachedPlayers[player.UserIDString] = player.ToIPlayer();
	        }
	        ...
	        public void InitRecentPlayersCache(CovalenceLib covalence)
	        {
	        ...
	        public void PurgeCollection(int days)
	        {
		        ...
		        for (int i = m_RecentPlayers.Count - 1; i >= 0; i--)
		        {
			        ...
			        if (currentTime - kvp.Value > expireTime)
				        m_RecentPlayers.Remove(kvp.Key);
		        }
	        }
        }
        #endregion 
