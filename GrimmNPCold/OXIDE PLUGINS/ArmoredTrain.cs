...
namespace Oxide.Plugins
{
    ...
    public class ArmoredTrain : RustPlugin
    {
        ...
        [PluginReference] private Plugin NpcSpawn, PveMode, GUIAnnouncements, Notify, DiscordMessages, DynamicPVP, Economics, ServerRewards, IQEconomic, TrainHomes, AlphaLoot;
        ...
        private static class NpcSpawnManager
        {
            ...
            private static ScientistNPC SpawnScientistNpc(NpcConfig npcConfig, Vector3 position, float healthFraction, bool isStationary, bool isPassive)
            {
                ...
                object spawnResult = _ins.NpcSpawn?.Call("SpawnNpc", position, baseNpcConfigObj, isPassive);
                if (spawnResult == null)
                {
                    NotifyManager.PrintError(null, $"NpcSpawn.Call('SpawnNpc') returned null for '{npcConfig.DisplayName}' at position {position}");
                    return null;
                }
            ...
            private static object GetBaseNpcConfig(NpcConfig config, float healthFraction, bool isStationary, bool isPassive)
            {
                ...
                HashSet<string> states = isPassive ? new HashSet<string>() : isStationary ? new HashSet<string> { "IdleState", "CombatStationaryState" } : config.BeltItems != null && config.BeltItems.Any(x => x.ShortName == "rocket.launcher" || x.ShortName == "explosive.timed") ? new HashSet<string> { "RaidState", "RoamState", "ChaseState", "CombatState" } : new HashSet<string> { "RoamState", "ChaseState", "CombatState" };
                ...
                {
                    NotifyManager.PrintError(null, "GetBaseNpcConfig: NpcSpawn plugin type is null!");
                    return null;
                }

                var npcConfigType = npcSpawnType.GetNestedType("NpcConfig", BindingFlags.Public);
                if (npcConfigType == null)
                {
                    NotifyManager.PrintError(null, "GetBaseNpcConfig: Failed to find NpcSpawn.NpcConfig type. Make sure NpcSpawn plugin is loaded.");
                    return null;
                }
                ...
                var npcWearType = npcSpawnType.GetNestedType("NpcWear", BindingFlags.Public);
                var npcBeltType = npcSpawnType.GetNestedType("NpcBelt", BindingFlags.Public);

                if (npcWearType == null || npcBeltType == null)
                {
                    NotifyManager.PrintError(null, "GetBaseNpcConfig: Failed to find NpcSpawn.NpcWear or NpcBelt types.");
                    return null;
                }

                // Check for custom wear items from WagonCustomizer
                JArray customWearItems = WagonCustomizer.GetCustomizeNpcWearSet();
                bool useCustomWear = customWearItems != null && customWearItems.Count > 0;

                // Convert WearItems from ArmoredTrain.NpcWear to NpcSpawn.NpcWear
                ...
                if (npcWearType != null)
                {
                    ...
                    if (useCustomWear)
                    {
                        // Use custom wear items from WagonCustomizer
                        var addMethod = hashSetType.GetMethod("Add");
                        foreach (JToken item in customWearItems)
                        {
                            if (item == null) continue;
                            string shortName = item["ShortName"]?.ToString();
                            if (string.IsNullOrWhiteSpace(shortName)) continue;
                            
                            ulong skinId = 0;
                            if (item["SkinID"] != null && ulong.TryParse(item["SkinID"].ToString(), out ulong parsedSkinId))
                                skinId = parsedSkinId;
                            
                            var npcWear = Activator.CreateInstance(npcWearType);
                            // NpcSpawn uses fields, not properties
                            SetField(npcWearType, npcWear, "ShortName", shortName);
                            SetField(npcWearType, npcWear, "SkinID", skinId);
                            addMethod.Invoke(convertedWearItems, new[] { npcWear });
                        }
                    }
                    else if (config.WearItems != null && config.WearItems.Count > 0)
                    {
                        // Use config wear items
                        var addMethod = hashSetType.GetMethod("Add");
                ...
                // Convert BeltItems from ArmoredTrain.NpcBelt to NpcSpawn.NpcBelt
                // Always create HashSet (even if empty) to ensure UpdateInventory() has a valid collection
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "Name", config.DisplayName ?? string.Empty);
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "Health", config.Health * healthFraction);
                SetProperty(npcConfigType, npcSpawnConfig, "RoamRange", isStationary ? 0 : config.RoamRange);
                SetProperty(npcConfigType, npcSpawnConfig, "ChaseRange", isStationary ? 0 : config.ChaseRange);
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "CheckVisionCone", true);
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "TurretDamageScale", config.TurretDamageScale);
                ...
                SetProperty(npcConfigType, npcSpawnConfig, "CanRunAwayWater", false);
                SetProperty(npcConfigType, npcSpawnConfig, "CanSleep", false);
                SetProperty(npcConfigType, npcSpawnConfig, "SleepDistance", 100f);
                SetProperty(npcConfigType, npcSpawnConfig, "Speed", isStationary ? 0 : config.Speed);
            ...
            private static void SetProperty(Type type, object obj, string propertyName, object value)
            {
                ...
                if (property != null && property.CanWrite)
                {
                    ...
                    {
                        NotifyManager.PrintError(null, $"SetProperty: Failed to set {propertyName} on {type.Name}: {ex.Message}");
                    }
                }
            }
            ...
            private static void SetField(Type type, object obj, string fieldName, object value)
            {
                ...
                if (field != null)
                {
                    ...
                    {
                        NotifyManager.PrintError(null, $"SetField: Failed to set {fieldName} on {type.Name}: {ex.Message}");
                    }
                }
            }
