...
namespace GrimmNPC.Patches
{
    ...
    public class HumanNPC_GetBestTarget_Patch
    {
        static bool Prefix(HumanNPC __instance, ref BaseEntity __result)
        {
            ...
            __result = FilterScarecrow(GetCustomBestTarget(npc, config), config);
            return false;
        }

        private static BaseEntity FilterScarecrow(BaseEntity entity, NpcConfig config)
        {
            if (entity == null || config == null || !config.PreventScarecrowTargeting)
                return entity;
            return entity is ScarecrowNPC ? null : entity;
        }
        ...
        private static BaseEntity GetCustomBestTarget(ScientistNPC npc, NpcConfig config)
        {
            ...
            if (config == null)
                config = _cachedTargetingConfig;
            
            var brain = npc.Brain;
            BaseEntity bestTarget = null;
            float bestScore = float.MinValue;
            bool debugLogging = GrimmNPC.ShouldLogTargetingDiagnostics();
            ...
            if (npcData.IsGuardNpc && npcData.GuardTarget != null && !npcData.GuardTarget.IsDestroyed)
            {
                ...
                {
                    if (debugLogging && Time.frameCount % 480 == 0)
                    {
