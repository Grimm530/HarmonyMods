    ...
    public class ScientistNPC_ServerInit_Patch
    {
        ...
        static void Postfix(ScientistNPC __instance)
        {
            ...
            try
            {
                ...
                {
                    ulong netId = __instance.net?.ID.Value ?? 0;
                    GrimmNPC.LogSpawnFailure(
                        "NavmeshNotReady",
                        $"netId={netId} pos={__instance.transform.position}",
                        netId);
                    if (GrimmNPC.ShouldLogSpawnRegistration())
                    {
                ...
                // Spawn/init swim guard: if NPC spawns already swimming, gate early nav transitions.
                ulong spawnNetId = __instance.net?.ID.Value ?? 0;
                if (spawnNetId != 0)
                {
                    var npcData = GrimmNPC.GetNpcData(spawnNetId);
                    if (npcData != null && npcData.CanSwim && __instance.IsSwimming())
                    {
                        GrimmNPC.MarkSpawnSwimInitWindow(spawnNetId);
                    }
                }
            }
            catch (Exception ex)
        ...
        private static void ApplyCustomConfig(ScientistNPC npc)
        {
            ...
            bool debugLogging = GrimmNPC.ShouldLogSpawnRegistration();
                ...
                if (npcData.AreaMask > 0)
                {
                    ...
                    if (agentProperty != null)
                    {
                        ...
                        if (navAgent != null)
                        {
                            ...
                            // Use properties from the *runtime* agent type. Cached PropertyInfo from
                            // Type.GetType("…NavMeshAgent…") can be a different Type identity than
                            // navigator.Agent (TargetException: Object does not match target type).
                            ...
                            var updateRotationProp = agentType.GetProperty("updateRotation");
                            var updatePositionProp = agentType.GetProperty("updatePosition");
                            ...
                            {
                                if (debugLogging)
                                    UnityEngine.Debug.LogWarning($"[GrimmNPC Spawn] NavMeshAgent property set failed: {agentEx.Message}");
                            }
    ...
    public class BaseAIBrain_InitializeAI_Patch
    {
        static void Postfix(BaseAIBrain __instance)
        {
            ...
            try
            {
                ...
                ulong netId = baseEntity.net?.ID.Value ?? 0;
                if (netId != 0)
                {
                    var npcData = GrimmNPC.GetNpcData(netId);
                    if (npcData != null && npcData.CanSwim && navigator.IsSwimming())
                    {
                        GrimmNPC.MarkSpawnSwimInitWindow(netId);
                        return;
                    }
                }
