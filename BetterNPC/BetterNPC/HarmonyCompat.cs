// Legacy -> Harmony compatibility shims for the ported BetterNPC plugin.
// These provide the small Oxide API surface the plugin uses (RustPlugin base, Plugin
// references, HarmonyModInterface.Mods data/config, VersionNumber, lang, command attributes) so the
// original plugin body compiles almost verbatim. Harmony-only runtime is required.
using System;
...
namespace Harmony.Core
{
    ...
    /// <summary>Minimal replacement for Oxide's HarmonyModInterface.Mods + HarmonyModInterface.CallHook.</summary>
    public static class HarmonyModInterface
    {
        ...
        /// (OnNpcSpawnInitialized / OnNpcSpawnPresetRename) are dispatched via AppDomain
        /// Harmony_CallHookList from 0GrimmNPC, not this method.
        /// </summary>
    ...
    public class HarmonyModRuntime
    {
        ...
        /// <summary>
        /// compat-style data root. Plugin paths like BetterNpc/ resolve under HarmonyData/.
        /// </summary>
        ...
        /// <summary>Soft no-op: never unload the Harmony mod on missing optional deps.</summary>
        public void UnloadPlugin(string name)
        {
            Debug.LogWarning("[BetterNPC] UnloadPlugin('" + name + "') requested but ignored (Harmony port keeps running / soft-fail).");
        }

        public object CallHook(string hook, params object[] args) => HarmonyModInterface.CallHook(hook, args);
    ...
    public class DataFileSystem
    {
        ...
        public T ReadObject<T>(string name)
        {
            ...
            {
                Debug.LogWarning("[BetterNPC] DataFileSystem.ReadObject('" + name + "') failed: " + ex.Message);
                return default;
            }
        }
        ...
        public void WriteObject<T>(string name, T obj, bool sync = false)
        {
            ...
            {
                Debug.LogWarning("[BetterNPC] DataFileSystem.WriteObject('" + name + "') failed: " + ex.Message);
            }
        }
        ...
        public string[] GetFiles(string folder)
        {
            try
            {
                string dir = Path.Combine(_baseDir, (folder ?? string.Empty).Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar).TrimEnd(Path.DirectorySeparatorChar));
                if (!Directory.Exists(dir)) return Array.Empty<string>();
                // Oxide DataFileSystem.GetFiles returns paths relative to data dir WITHOUT .json extension, matching ReadObject keys
                string[] files = Directory.GetFiles(dir, "*.json");
                var result = new string[files.Length];
                for (int i = 0; i < files.Length; i++)
                {
                    string full = files[i];
                    string rel = full.Substring(_baseDir.Length).TrimStart(Path.DirectorySeparatorChar, '/', '\\');
                    if (rel.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
                        rel = rel.Substring(0, rel.Length - 5);
                    result[i] = rel.Replace('\\', '/');
                }
                return result;
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[BetterNPC] DataFileSystem.GetFiles failed: " + ex.Message);
...
namespace Harmony.Core.Plugins
{
    ...
    /// <summary>Stand-in for an Harmony mod reference. Base does nothing; subclasses can override Call.</summary>
    public class Plugin
    {
    ...
    /// <summary>
    /// Bridges compat-style Plugin.Call to the shared 0PveMode Harmony mod via AppDomain
    ...
    public sealed class PveModePluginBridge : Plugin
    {
        ...
        public override object Call(string hook, params object[] args)
        {
            ...
            {
                Debug.LogWarning("[BetterNPC] PveMode.Call " + hook + " failed: " + (ex.InnerException?.Message ?? ex.Message));
                return null;
            }
        }
    ...
    public class ModManager
    {
        public bool Exists(string name)
        {
            ...
            if (string.Equals(name, "NpcSpawn", StringComparison.OrdinalIgnoreCase) ||
                ...
                return global::BetterNpc.BetterNpcGrimmNpc.Available;
    ...
    public sealed class EconomicsPluginBridge : Plugin
    {
        ...
        public override object Call(string hook, params object[] args)
        {
            ...
            {
                Debug.LogWarning("[BetterNPC] Economics.Call " + hook + " failed: " + (ex.InnerException?.Message ?? ex.Message));
                return null;
            }
        }
    }
...
namespace Harmony.Plugins
{
    ...
    /// <summary>Config file wrapper matching the compat DynamicConfigFile surface used by the plugin.</summary>
    public class DynamicConfigFile
    {
        ...
        public void WriteObject<T>(T obj, bool sync = false)
        {
            ...
            {
                Debug.LogWarning("[BetterNPC] Config write failed: " + ex.Message);
            }
        }
    }
    ...
    public class ServerShim
    {
        public void Command(string command, params object[] args)
        {
            ...
            catch (Exception ex) { Debug.LogWarning("[BetterNPC] Server.Command failed: " + ex.Message); }
        }
    ...
    public abstract class RustPlugin
    {
        ...
        public readonly TimerLibrary timer = new TimerLibrary();
        ...
        {
            string baseDir = Harmony.Core.HarmonyModRuntime.ResolveServerRoot();
            string[] candidates =
            {
        ...
        // Invoked by BetterNpcMod to drive the compat-style lifecycle.
        public void HarmonyLoadConfig() => LoadConfig();
        public void HarmonyLoadDefaultMessages() => LoadDefaultMessages();

        // ----- logging / chat -----
        public void Puts(string format, params object[] args) => Debug.Log("[" + (Title ?? "BetterNPC") + "] " + Fmt(format, args));
        public void PrintWarning(string format, params object[] args) => Debug.LogWarning("[" + (Title ?? "BetterNPC") + "] " + Fmt(format, args));
        public void PrintError(string format, params object[] args) => Debug.LogError("[" + (Title ?? "BetterNPC") + "] " + Fmt(format, args));
        ...
        public void NextTick(Action action)
        {
            ...
            global::BetterNpc.ModRunner.Enqueue(action);
        }

        // ----- Harmony hook (un)subscription becomes a no-op; patches always dispatch and early-out. -----
    ...
    public class PermissionLibrary
    {
        public void RegisterPermission(string perm, object plugin = null)
        {
            ...
            {
                Debug.LogWarning("[BetterNPC] RegisterPermission failed: " + ex.Message);
            }
        }
    ...
    {
        public Timer In(float seconds, Action action)
        {
            var t = new Timer();
            if (action == null) return t;
            if (global::BetterNpc.ModRunner.Instance != null)
                global::BetterNpc.ModRunner.Instance.StartCoroutine(Delay(seconds, action, t));
            return t;
        }

        private static System.Collections.IEnumerator Delay(float seconds, Action action, Timer timer)
        {
            if (seconds > 0f) yield return new WaitForSeconds(seconds);
            if (timer.Destroyed) yield break;
            try { action(); } catch (Exception ex) { Debug.LogWarning("[BetterNPC] timer.In callback failed: " + ex.Message); }
        }
    }
    ...
    /// <summary>Oxide cmd.AddChatCommand — registers on BetterNpcMod after it is loaded.</summary>
    public class CommandLibrary
    {
        public void AddChatCommand(string name, object plugin, Action<BasePlayer, string, string[]> callback)
        {
            ...
            global::BetterNpc.BetterNpcMod.Instance?.RegisterChatCommand(name, callback);
        }
    }
}
