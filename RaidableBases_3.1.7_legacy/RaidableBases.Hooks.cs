...
namespace RaidableBases
{
    public partial class RaidableBases
    {
        ...
        private void OnLoseCondition(Item item, ref float amount)
        {
            ...
            if (player == null || !((ulong)player.userID).IsSteamId() || player.HasPermission("raidablebases.durabilitybypass"))
            {
                return;
            }
        ...
        private void OnEntitySpawned(FireBall fire)
        {
            ...
            else if (config.Settings.Management.PreventFireFromSpreading && fire.ShortPrefabName == "flamethrower_fireball" && fire.creatorEntity is BasePlayer player && !((ulong)player.userID).IsSteamId())
            {
                fire.DelayedSafeKill();
            }
        ...
        private void OnLootEntityEnd(BasePlayer player, StorageContainer container)
        {
            ...
            if (((ulong)player.userID).IsSteamId())
            {
        ...
        private bool IsSentryTargetingNpc(BasePlayer player, BaseEntity entity) => entity is NPCAutoTurret && player.skinID != RB_SKIN_ID && !((ulong)player.userID).IsSteamId();
        ...
        private DamageResult HandleTrueDamage(RaidableBase raid, HitInfo info, BaseEntity weapon, BasePlayer victim)
        {
            ...
            if (turret != null)
            {
                var (min, max) = ((ulong)victim.userID).IsSteamId() ? (raid.Options.AutoTurret.Min, raid.Options.AutoTurret.Max) : (raid.Options.AutoTurret.NpcMin, raid.Options.AutoTurret.NpcMax);
        ...
        private DamageResult HandleNpcAttacker(RaidableBase raid, BasePlayer victim, BasePlayer attacker, HitInfo info, DamageType damageType)
        {
            ...
            {
                return ((ulong)victim.userID).IsSteamId() ? DamageResult.Blocked : DamageResult.None;
            }
