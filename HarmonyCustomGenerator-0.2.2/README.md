...
> **Train tracks and tunnel entrances:** There is no special logic to connect above-ground train tracks to below-ground tunnel entrances. Rail ring and tunnel-entrance placement are separate procedural steps; tracks may not physically meet `train_tunnel_volume_entrance` triggers. If tracks don’t connect, try another seed/size or leave "Remove tunnel entrances" off so both rails and entrances are generated.
