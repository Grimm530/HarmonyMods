...
namespace CustomGenerator.Utility {
    ...
    static class MapImage
    {
        ...
        private static void CheckResources() {
            if (!Directory.Exists("maps/images")) Directory.CreateDirectory("maps/images");
            if (!Directory.Exists("maps/images/resources")) Directory.CreateDirectory("maps/images/resources");

            string path = "maps/images/resources";
            foreach (var resource in RequirementResources) {
                ...
                using (var client = new WebClient()) {
                    ...
                    try {
                        ...
                        Logging.Error($"DEPS - Error whilst downloading: {ex.Message} \nTry moving file from the `Resources` repository folder to the `maps/images/resources/`");
                    }
                }
            }
        }
        public static void RenderMap(float scale = 0.5f, int oceanMargin = 500) {
            ...
            string fullPath = Path.GetFullPath(Path.Combine(Environment.CurrentDirectory, $"maps/images/{mapName}.png"));
            File.WriteAllBytes(fullPath, array);
            Logging.Info("Generated Map image: maps/images/");
    ...
    public static class MapImageRender {
        private static readonly string PermanentMarkerFont = "maps/images/resources/PermanentMarker.ttf";
        private static readonly string DinProFontBold = "maps/images/resources/dinprobold.otf";
        ...
        public static byte[] Render(out int imageWidth, out int imageHeight, out Color background, float scale = 0.5f, bool lossy = true, bool transparent = false, int oceanMargin = 500) {
            ...
            float GetShoreDist(float x, float y) => TerrainTexturing.Instance.GetMainlandCoarseVectorToShore(x, y).shoreDist;
