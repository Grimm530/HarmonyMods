// Legacy -> Harmony compatibility shims for the ported CustomHelicopterTiers plugin.
...
namespace Harmony.Core
{
    ...
    public static class HarmonyModInterface
    {
        ...
        /// <summary>
        /// No Harmony hook bus in Harmony. Returns null (no plugin cancels/handles). Core logic must
        /// never depend on this; it is only used by the plugin for optional cross-plugin notifications.
        /// </summary>
    ...
    public class HarmonyModRuntime
    {
        ...
        /// <summary>
        /// compat-style data root. Plugin paths like CustomHelicopterTiers/Images resolve under HarmonyData/.
        /// </summary>
        ...
        {
            Debug.LogWarning("[CHT] UnloadPlugin('" + name + "') requested but ignored (Harmony port keeps running / soft-fail).");
        }
    ...
    public class DataFileSystem
    {
        ...
        public string[] GetFiles(string folder) { string dir = Path.Combine(_baseDir, folder ?? string.Empty); return Directory.Exists(dir) ? Directory.GetFiles(dir, "*.json", SearchOption.TopDirectoryOnly) : Array.Empty<string>(); }
        public void DeleteDataFile(string name) { string path = PathFor(name); if (File.Exists(path)) File.Delete(path); }
        ...
        public T ReadObject<T>(string name)
        {
            ...
            {
                Debug.LogWarning("[CHT] DataFileSystem.ReadObject('" + name + "') failed: " + ex.Message);
                return default;
            }
        }
        ...
        public void WriteObject<T>(string name, T obj, bool sync = false)
        {
            ...
            {
                Debug.LogWarning("[CHT] DataFileSystem.WriteObject('" + name + "') failed: " + ex.Message);
            }
        }
    }
}
...
namespace Harmony.Core.Plugins
{
    ...
    public sealed class PveModePluginBridge : Plugin
    {
        ...
        public override object Call(string hook, params object[] args)
        {
            ...
            {
                Debug.LogWarning("[CHT] PveMode.Call " + hook + " failed: " + (ex.InnerException?.Message ?? ex.Message));
                return null;
            }
        }
...
namespace Harmony.Plugins
{
    ...
    public class DynamicConfigFile
    {
        ...
        public void WriteObject<T>(T obj, bool sync = false)
        {
            ...
            {
                Debug.LogWarning("[CHT] Config write failed: " + ex.Message);
            }
        }
    }
    ...
    public class ServerShim
    {
        public void Command(string command, params object[] args)
        {
            ...
            catch (Exception ex) { Debug.LogWarning("[CHT] Server.Command failed: " + ex.Message); }
        }
    ...
    {
        public void AddChatCommand(string command, object plugin, string callback) { }
    }    public abstract class RustPlugin
    {
        ...
        public readonly PermissionShim permission = new PermissionShim();
        public readonly CommandShim cmd = new CommandShim();
        ...
        private static string ResolveConfigPath(string name)
        {
            ...
            string[] candidates =
            {
                Path.Combine(baseDir, "HarmonyConfig", "CHT.json"),
                Path.Combine(baseDir, "HarmonyConfig", name + ".json"),
                Path.Combine(baseDir, "oxide", "config", "CHT.json"),
            ...
            foreach (string c in candidates)
                if (File.Exists(c))
                    return c;
            return candidates[0]; // default write target: HarmonyConfig/CHT.json
        }
        ...
        // Invoked by CHTMod to drive the compat-style lifecycle.
        public void HarmonyLoadConfig() => LoadConfig();
        public void HarmonyLoadDefaultMessages() => LoadDefaultMessages();

        // ----- logging / chat -----
        public void Puts(string format, params object[] args) => Debug.Log("[" + (Title ?? "CHT") + "] " + Fmt(format, args));
        public void PrintWarning(string format, params object[] args) => Debug.LogWarning("[" + (Title ?? "CHT") + "] " + Fmt(format, args));
        public void PrintError(string format, params object[] args) => Debug.LogError("[" + (Title ?? "CHT") + "] " + Fmt(format, args));
        ...
        public void SendReply(BasePlayer player, string message) => PrintToChat(player, message);
        ...
        public void NextTick(Action action)
        {
            ...
            global::CHT.ModRunner.Enqueue(action);
        }
