 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to UpLiftedHarmony,
 * log tag changed to [UpLifted].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace UpLiftedHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[UpLifted] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[UpLifted] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[UpLifted] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[UpLifted] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[UpLifted] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[UpLifted] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[UpLifted] Permissions Service bind: " + ex.Message);
            }
        }
