...
namespace UpLiftedHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("UpLifted_Runner");
    ...
    public class UpLiftedMod : IHarmonyModHooks
    {
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[UpLifted] FAIL: construct/config: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(UpLiftedMod)); }
            catch { }
            ...
            Debug.Log("[UpLifted] Harmony mod loaded . Config: HarmonyConfig/UpLifted.json. Data: HarmonyData/UpLifted/");
        }

        private void OnPermissionsReady()
        {
            try { Plugin?.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[UpLifted] Permissions ready: " + ex.Message); }
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[UpLifted] Init: " + ex.Message); }
            ...
            catch (Exception ex) { Debug.LogError("[UpLifted] OnServerInitialized: " + ex); }

            RefreshDynamicCommands();
            _initCoroutine = null;
            Debug.Log("[UpLifted] Server initialized.");
        }
        ...
        internal static void InvokeCommandMethod(OxidePlugin plugin, string methodName, BasePlayer player, string command, string[] args)
        {
            ...
            {
                Debug.LogWarning("[UpLifted] InvokeCommandMethod " + methodName + ": " + (ex.InnerException?.Message ?? ex.Message));
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[UpLifted] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[UpLifted] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[UpLifted] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            try
            ...
            {
                Debug.LogWarning("[UpLifted] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                Debug.LogWarning("[UpLifted] cui.endtest UPLIFTED: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[UpLifted] " + methodName + ": " + (ex.InnerException?.Message ?? ex.Message));
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[UpLifted] cmd " + captured + ": " + ex.Message); }
                }
            };
