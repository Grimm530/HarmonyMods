...
namespace QuestHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("Quest_Runner");
    ...
    public class QuestMod : IHarmonyModHooks
    {
        ...
        public const string AppDomainApiKey = "Quest_ApiType";
        public const string CuiMarker = "QUEST";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[Quest] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(QuestMod)); }
            catch { }

            foreach (var cmd in new[] { "quest", "qlist" })
            {
                _chatCommandNames.Add(cmd);
                RegisterChatAliasConsole(cmd);
            }

            ChatSayBridge.Register("Quest", OnChatCommand);
            ...
            Debug.Log("[Quest] Harmony mod loaded. Chat: /quest /qlist. Config: HarmonyConfig/Quest.json.");
        }
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[Quest] Permissions ready re-register: " + ex.Message);
            }
        }
        ...
        private void BindBetterChat()
        {
            try
            {
                ...
                mi?.Invoke(null, new object[] { "Quest", _betterChatTitle });
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[Quest] BetterChat bind: " + ex.Message);
            }
        }
        ...
        private void UnbindBetterChat()
        {
            try
            {
                ...
                mi?.Invoke(null, new object[] { "Quest" });
            }
            catch { }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[Quest] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            _initCoroutine = null;
            Debug.Log("[Quest] Server initialized.");
        }
        ...
        public object Call(string method, params object[] args)
        {
            ...
            {
                Debug.LogWarning("[Quest] Call(" + method + "): " + ex.Message);
                return null;
            }
        }
        ...
        private static void InvokeChatMethod(OxidePlugin plugin, string methodName, BasePlayer player, string command, string[] args)
        {
            ...
            {
                Debug.LogWarning("[Quest] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[Quest] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedChatCommands()
        {
            ...
            {
                Debug.LogWarning("[Quest] RegisterAttributedChatCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[Quest] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[Quest] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            ...
            {
                Debug.LogWarning("[Quest] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            try
            {
                ...
                Debug.LogWarning("[Quest] cui.endtest QUEST: command not registered: " + cmdName);
            }
            catch (Exception ex)
            {
                Debug.LogWarning("[Quest] cui.endtest QUEST: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[Quest] " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[Quest] cmd " + captured + ": " + ex.Message); }
                }
            };
