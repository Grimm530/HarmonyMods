 ...
 * Adapted from Quest/PermissionsBridge.cs — namespace changed to CombatClassesHarmony,
 * log tag changed to [Quest].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace QuestHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[Quest] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[Quest] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[Quest] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[Quest] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[Quest] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[Quest] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[Quest] Permissions Service bind: " + ex.Message);
            }
        }
        ...
        public static string[] GetPermissionUsers(string perm)
        {
            ...
            try
            {
                var mi = _service?.GetType().GetMethod("GetUsersWithPermission", BindingFlags.Public | BindingFlags.Instance);
                if (mi != null)
                    return ToStringArray(mi.Invoke(_service, new object[] { perm }));
            }
            catch { }
            return Array.Empty<string>();
        }
