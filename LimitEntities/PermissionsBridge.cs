...
namespace LimitEntities
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[LimitEntities] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[LimitEntities] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
        ...
        private static void ClearBind()
        {
            _permType = null;
            _service = null;
            _userHas = _register = _exists = null;
            _svcUserHas = _svcRegister = _svcExists = null;
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[LimitEntities] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[LimitEntities] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[LimitEntities] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[LimitEntities] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[LimitEntities] Permissions Service bind: " + ex.Message);
            }
        }
