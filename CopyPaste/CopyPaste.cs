...
namespace CopyPasteHarmony
{
    ...
    public partial class CopyPaste : CopyPasteBase
    {
        ...
        // Deferred: StringPool needs FileSystem, which is not ready during early Harmony OnLoaded.
        ...
        internal bool _pasteReady;
        private readonly List<PasteData> _pendingPastes = new();
        private float _steamWaitStarted = -1f;
        private float _nextSteamStatusLogAt = 30f;
        private const float SteamDefinitionsTimeoutSeconds = 90f;
        ...
        private void EnsurePrefabIds()
        {
            if (_floorFramePrefabId != 0 && _floorTriangleFramePrefabId != 0)
                return;
            _floorFramePrefabId = StringPool.Get("assets/prefabs/building core/floor.frame/floor.frame.prefab");
            _floorTriangleFramePrefabId = StringPool.Get("assets/prefabs/building core/floor.triangle.frame/floor.triangle.frame.prefab");
        }
        ...
        // TrackerId support -----------------------------------------------------------------------------------------
        // Maps spawned entities (by net ID) to a stable tracker ID that is also stored in the copypaste JSON.
        // External plugins (Dungeon, Portals, NPC/Boss trackers, etc.) can use the public API methods at the
        // bottom of this file to read the tracker ID for any entity that originated from a CopyPaste paste.
        private readonly Dictionary<ulong, string> _trackerIdsByNetId = new();
        ...
        private static bool IsStringLightsType(Type type)
        {
            while (type != null)
            {
                var name = type.Name;
                if (name == "AdvancedChristmasLights" || name == "ChristmasLights" || name == "StringLights")
                    return true;
                type = type.BaseType;
            }
            return false;
        ...
        private static object GetFieldOrPropertyValue(object obj, string memberName)
        {
            if (obj == null)
                return null;
        ...
        private static bool SetFieldOrPropertyValue(object obj, string memberName, object value)
        {
            if (obj == null)
                return false;
            var type = obj.GetType();
        ...
        private void LoadVariables()
        {
            ...
            if (_config == null || _config.Copy == null || _config.Paste == null)
            {
                LoadDefaultConfig();
            ...
            _config.Copy ??= new ConfigData.CopyOptions();
            _config.Paste ??= new ConfigData.PasteOptions();
        ...
        private void ProcessItemDefinitions()
        {
            try
            {
                ...
                if (_steamWaitStarted < 0f)
                    _steamWaitStarted = Time.realtimeSinceStartup;

                float waited = Time.realtimeSinceStartup - _steamWaitStarted;
                bool itemsReady = ItemManager.itemList != null && ItemManager.itemList.Count > 0;

                // Pastes only need ItemManager. Steam inventory defs are optional (paid-skin filtering).
                // Waiting on Steam was blocking raid base spawns for minutes after the grid finished,
                // and ServerMgr.Invoke retries often die during dedicated boot so the wait never timed out.
                if (!itemsReady)
                {
                    if (waited >= _nextSteamStatusLogAt)
                    {
                        PrintWarning($"Still waiting {Mathf.RoundToInt(waited)}s for ItemManager ({_pendingPastes.Count} paste(s) queued)...");
                        _nextSteamStatusLogAt += 30f;
                    }
                    ScheduleProcessItemDefinitions(1f);
                    return;
                }

                var steamDefs = GetSteamInventoryDefinitions();
                bool steamReady = steamDefs != null && steamDefs.Length > 0;
                if (!steamReady && waited < SteamDefinitionsTimeoutSeconds)
                {
                    // Items are ready — enable paste now. Keep polling Steam in the background for skin lists.
                    BuildItemMapsAndFinishPasteReady(steamDefs: null, noteMissingSteam: false);
                    ScheduleEnrichPaidSkins(3f);
                    return;
                }

                if (!steamReady)
                    PrintWarning($"Steam inventory definitions unavailable after {Mathf.RoundToInt(waited)}s; paste already enabled without full paid-skin detection.");

                BuildItemMapsAndFinishPasteReady(steamDefs, noteMissingSteam: !steamReady);
            }
            catch (Exception ex)
            {
                PrintError($"ProcessItemDefinitions failed: {ex}");
                float waited = _steamWaitStarted < 0f ? 0f : Time.realtimeSinceStartup - _steamWaitStarted;
                bool itemsReady = ItemManager.itemList != null && ItemManager.itemList.Count > 0;
                if (itemsReady)
                {
                    PrintWarning("Enabling paste after ProcessItemDefinitions error (ItemManager is ready).");
                    BuildItemMapsAndFinishPasteReady(steamDefs: null, noteMissingSteam: true);
                }
                else if (waited < SteamDefinitionsTimeoutSeconds + 60f)
                    ScheduleProcessItemDefinitions(1f);
                else
                {
                    PrintWarning("Giving up on definition init after repeated errors; enabling paste.");
                    FinishPasteReady();
                }
            }
        }

        private void BuildItemMapsAndFinishPasteReady(Steamworks.InventoryDef[] steamDefs, bool noteMissingSteam)
        {
            if (_pasteReady)
                return;

            if (steamDefs != null && steamDefs.Length > 0)
            {
                const string WORKSHOP_ID = "workshopid";
                foreach (var inventoryDef in steamDefs)
                {
                    if (inventoryDef == null) continue;
            ...
            try
            {
                ...
                if (skins != null)
                {
                    for (int i = 0; i < skins.Length; i++)
                        _paidSkinIds.Add((ulong)skins[i].id);
                }
            }
            catch
            {
                // ItemSkinDirectory can throw while skins.asset is still mounting.
            }
            ...
            if (noteMissingSteam && steamDefs == null)
                PrintWarning("Paste enabled without Steam inventory definitions (paid-skin filter may be incomplete).");

            FinishPasteReady();
        }

        private void ScheduleEnrichPaidSkins(float delay)
        {
            void Enrich()
            {
                try
                {
                    ...
                    {
                        float waited = _steamWaitStarted < 0f ? 0f : Time.realtimeSinceStartup - _steamWaitStarted;
                        if (waited < SteamDefinitionsTimeoutSeconds)
                            ScheduleEnrichPaidSkins(5f);
                        return;
                    }

                    const string WORKSHOP_ID = "workshopid";
                    int before = _paidSkinIds.Count;
                    foreach (var inventoryDef in steamDefs)
                    {
                        if (inventoryDef == null) continue;
                        if (ulong.TryParse(inventoryDef.GetProperty(WORKSHOP_ID), out ulong skinId))
                    ...
                    if (_paidSkinIds.Count > before)
                        Puts($"Paid-skin list enriched from Steam (+{_paidSkinIds.Count - before}; total {_paidSkinIds.Count}).");
                }
                catch (Exception ex)
                {
                    PrintWarning($"EnrichPaidSkins failed: {ex.Message}");
                }
            }

            if (ServerMgr.Instance != null)
                ServerMgr.Instance.Invoke(Enrich, delay);
            else if (timer != null)
                timer.In(delay, Enrich);
        }

        /// <summary>Resolve Steam inventory defs via direct API, with reflection fallback (same approach as AlphaLoot).</summary>
        private static Steamworks.InventoryDef[] GetSteamInventoryDefinitions()
        {
            try
            {
            ...
            try
            {
                ...
                {
                    var type = asm.GetType("Steamworks.SteamInventory");
                    if (type == null) continue;
                    var prop = type.GetProperty("Definitions", BindingFlags.Static | BindingFlags.Public);
                    if (prop?.GetValue(null, null) is Steamworks.InventoryDef[] reflected && reflected.Length > 0)
                        return reflected;
                    if (prop?.GetValue(null, null) is Array arr && arr.Length > 0)
                    {
                        var list = new List<Steamworks.InventoryDef>(arr.Length);
                        foreach (var item in arr)
                        {
                            if (item is Steamworks.InventoryDef def)
                                list.Add(def);
                        }
                        if (list.Count > 0)
                            return list.ToArray();
        ...
        private void ScheduleProcessItemDefinitions(float delay)
        {
            // Prefer a DontDestroyOnLoad waiter — ServerMgr.Invoke retries often get wiped during dedicated boot.
            try
            {
                var go = new GameObject("CopyPaste_DefinitionWait");
                UnityEngine.Object.DontDestroyOnLoad(go);
                go.AddComponent<DefinitionWaitBehaviour>().Begin(this, Mathf.Max(0.25f, delay));
                return;
            }
            catch
            {
                // Fall through to Invoke/timer.
            }

            if (ServerMgr.Instance != null)
                ServerMgr.Instance.Invoke(ProcessItemDefinitions, delay);
            else if (timer != null)
                timer.In(delay, ProcessItemDefinitions);
        }

        private sealed class DefinitionWaitBehaviour : MonoBehaviour
        {
            private CopyPaste _plugin;
            private float _delay;

            public void Begin(CopyPaste plugin, float delay)
            {
                _plugin = plugin;
                _delay = delay;
                StartCoroutine(Wait());
            ...
            private System.Collections.IEnumerator Wait()
            {
                ...
                var plugin = _plugin;
                Destroy(gameObject);
                plugin?.ProcessItemDefinitions();
            }
        }

        private void FinishPasteReady()
        {
            if (_pasteReady)
                return;
            ...
            for (int i = 0; i < _pendingPastes.Count; i++)
            {
                ...
                float delay = i * 0.1f;
                if (ServerMgr.Instance != null)
                    ServerMgr.Instance.Invoke(() => PasteLoop(pasteData), delay);
                else
                    timer.Once(delay, () => PasteLoop(pasteData));
            }
        ...
        public object TryCopyFromSteamId(ulong userId, string filename, string[] args, Action callback = null)
        {
            ...
            return TryCopy(hit.point, entity.GetNetworkRotation().eulerAngles, filename,
                DegreeToRadian(player.GetNetworkRotation().eulerAngles.y), args, player.ToIPlayer(), callback);
        }
            ...
            Action<BaseEntity> callbackSpawned = null)
        {
            ...
            return TryPaste(hit.point, filename, player.ToIPlayer(),
                DegreeToRadian(player.GetNetworkRotation().eulerAngles.y),
                args, callback: callback, callbackSpawned: callbackSpawned).Item1;
        }

        public object TryPasteFromVector3(Vector3 pos, float rotationCorrection, string filename, string[] args,
        ...
        private void RemoveEntity(BaseEntity entity)
        {
            ...
            if (io != null)
            {
                try { io.ClearConnections(); } catch { } // edge cases where this can still throw
            }
        ...
        private void UndoLoop(HashSet<BaseEntity> entities, IPlayer player, int count = 0, string filename = null)
        {
            ...
            if (entities.Count > 0)
                timer.Once(_config.UndoBatchWait, () => UndoLoop(entities, player, entities.Count, filename));
            else if (player != null)
            {
                player.Reply(filename == null ? Lang("UNDO_SUCCESS", player.Id) : Lang("UNDO_SUCCESS", player.Id) + ": " + filename);
            ...
            CopyData copyData)
        {
            ...
            // TrackerId: try to reuse an existing tracker for this entity if it was originally pasted by CopyPaste.
            // If none exists, generate a new one. This ID will be stored in the JSON and re-attached on paste.
            string trackerId = null;
            var netId = entity.net?.ID.Value ?? 0;
            if (netId != 0 && _trackerIdsByNetId.TryGetValue(netId, out var existingId))
            {
                trackerId = existingId;
            }
            else
            {
                trackerId = Guid.NewGuid().ToString("N");
                if (netId != 0)
                    _trackerIdsByNetId[netId] = trackerId;
            }
            ...
            // Handle StringLights variants (AdvancedChristmasLights -> ChristmasLights -> StringLights)
            if (entity != null && IsStringLightsType(entity.GetType()))
            {
                var pointsObj = GetFieldOrPropertyValue(entity, "points");
                ...
                if (pointsObj is IEnumerable enumerable)
                {
                    ...
                    {
                        var normalObj = GetFieldOrPropertyValue(point, "normal");
                        var pointObj = GetFieldOrPropertyValue(point, "point");
                        var slackObj = GetFieldOrPropertyValue(point, "slack");
                        Vector3 normal = normalObj is Vector3 v1 ? v1 : Vector3.zero;
                        Vector3 pos = pointObj is Vector3 v2 ? v2 : Vector3.zero;
                        float slack = 0f;
                        if (slackObj != null)
                            float.TryParse(slackObj.ToString(), out slack);

                        lightsPointsList.Add(new Dictionary<string, object>
                        {
                            { "normal", normal },
                            { "point", NormalizePosition(copyData.SourcePos, pos, copyData.RotCor) },
                            { "slack", slack },
                        });
                    }
                }
                data.Add("points", lightsPointsList);
                var anim = GetFieldOrPropertyValue(entity, "animationStyle");
                if (anim != null)
                    data.Add("animationStyle", anim);
            }
        ...
        public object FindBestHeight(ICollection<Dictionary<string, object>> entities, Vector3 startPos)
        {
            var maxHeight = 0f;
        ...
        public PasteData Paste(ICollection<Dictionary<string, object>> entities, Dictionary<string, object> protocol,
            ...
            bool checkPlaced, bool enableSaving = true, bool? dlc = null, int? skinsMode = null)
        {
            ...
            // Parse VersionNumber (JSON ints are often boxed as Int64 — do not use (int) cast)
            var version = protocol.ContainsKey("version") ? protocol["version"] as Dictionary<string, object> : null;
            
            VersionNumber vNumber = default;
            if (version != null)
                vNumber = new VersionNumber(
                    Convert.ToInt32(version["Major"]),
                    Convert.ToInt32(version["Minor"]),
                    Convert.ToInt32(version["Patch"]));
            ...
            if (!_pasteReady)
            {
                // Safety net: never block raid/admin pastes on Steam once ItemManager exists.
                if (ItemManager.itemList != null && ItemManager.itemList.Count > 0)
                    BuildItemMapsAndFinishPasteReady(GetSteamInventoryDefinitions(), noteMissingSteam: false);

                if (!_pasteReady)
                {
                    PrintWarning($"Pasting '{filename}' has been queued: waiting for ItemManager");
            ...
            timer.Once(_config.PasteBatchWait, () => PasteLoop(pasteData));
        ...
        private void PasteLoop(PasteData pasteData)
        {
            if (pasteData.Cancelled)
            {
                UndoLoop(new HashSet<BaseEntity>(pasteData.PastedEntities), pasteData.Player,
                    pasteData.PastedEntities.Count, pasteData.Filename);
        ...
        private void PasteEntity(Dictionary<string, object> data, PasteData pasteData, BaseEntity parent = null)
        {
            ...
            // TrackerId: attach the saved tracker to the spawned entity after entity.net is available.
            if (data.ContainsKey("trackerid"))
            {
                try
                {
                    var trackerIdObj = data["trackerid"];
                    var trackerIdStr = trackerIdObj != null ? trackerIdObj.ToString() : null;
                    if (!string.IsNullOrEmpty(trackerIdStr) && entity.net != null)
                        _trackerIdsByNetId[entity.net.ID.Value] = trackerIdStr;
                }
                catch (Exception)
                {
                    // Defensive: never fail a paste due to tracker ID issues.
                }
            }
            ...
            if (entity != null && IsStringLightsType(entity.GetType()))
            {
                if (data.ContainsKey("points"))
                {
                    ...
                    {
                        var pointsCollection = GetFieldOrPropertyValue(entity, "points") as System.Collections.IList;
                        if (pointsCollection != null)
                        {
                            foreach (Dictionary<string, object> pointEntry in points)
                            {
                                ...
                                var pointEntryType = pointsCollection.GetType().GetGenericArguments().FirstOrDefault() ??
                                                     pointsCollection.GetType().GetElementType();
                                object newPoint = pointEntryType != null ? Activator.CreateInstance(pointEntryType) : new object();
                                SetFieldOrPropertyValue(newPoint, "normal", new Vector3(Convert.ToSingle(normal["x"]), Convert.ToSingle(normal["y"]), Convert.ToSingle(normal["z"])));
                                SetFieldOrPropertyValue(newPoint, "point", adjustedPoint);
                                SetFieldOrPropertyValue(newPoint, "slack", slack);
                ...
                if (data.ContainsKey("animationStyle"))
                {
                    ...
                    if (!SetFieldOrPropertyValue(entity, "animationStyle", anim))
                    {
                        ...
                        if (member is FieldInfo fi && fi.FieldType.IsEnum)
                            fi.SetValue(entity, Enum.ToObject(fi.FieldType, enumValue));
                        else if (member is PropertyInfo pi && pi.PropertyType.IsEnum && pi.CanWrite)
                            pi.SetValue(entity, Enum.ToObject(pi.PropertyType, enumValue), null);
                    }
                }
            }
            ...
            if (ioEntity.IsValid() && !ioEntity.IsDestroyed)
            {
                ...
                if (ioData.TryGetValue("oldID", out oldIdObject))
                {
                    ...
                    if (!pasteData.EntityLookup.ContainsKey(oldId)) // some old Fortify saves will throw without this guard
        ...
        #region API - TrackerId

        /// <summary>
        /// API_GetTrackerId(BaseEntity entity)
        /// Returns the TrackerId string for an entity that was pasted by CopyPaste, or null if none is known.
        /// </summary>
        /// <param name="entity">The entity to query.</param>
        /// <returns>string trackerId or null</returns>
        private object API_GetTrackerId(BaseEntity entity)
        {
            if (entity == null || entity.net == null)
                return null;

            if (_trackerIdsByNetId.TryGetValue(entity.net.ID.Value, out var id))
                return id;
        ...
        public HashSet<Dictionary<string, object>> PreLoadData(List<object> entities, Vector3 startPos,
        ...
        internal void CmdCopy(IPlayer player, string command, string[] args)
        {
        ...
        internal void CmdPaste(IPlayer player, string command, string[] args)
        {
        ...
        internal void CmdList(IPlayer player, string command, string[] args)
        {
        ...
        internal void CmdPasteBack(IPlayer player, string command, string[] args)
        {
        ...
        internal void CmdUndo(IPlayer player, string command, string[] args)
        {
            ...
            LastPaste lastPaste = _lastPastes[player.Id].Pop();
            var entities = new HashSet<BaseEntity>(lastPaste.Entities);

            UndoLoop(entities, player, filename: lastPaste.Filename);
        }
