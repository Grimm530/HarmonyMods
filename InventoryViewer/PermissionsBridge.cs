...
namespace InventoryViewer
{
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[InventoryViewer] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[InventoryViewer] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[InventoryViewer] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[InventoryViewer] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[InventoryViewer] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[InventoryViewer] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[InventoryViewer] Permissions Service bind: " + ex.Message);
            }
        }
