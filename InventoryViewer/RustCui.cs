...
namespace Game.Rust.Cui
{
    ...
    public static class CuiHelper
    {
        ...
        public static bool AddUi(BasePlayer player, string json)
        {
            if (player?.net != null)
            {
                // Clients only forward ConsoleGen commands. Rewrite compat-style WipeSchedule UI commands
                // callbacks to cui.endtest so WipeScheduleHarmony.Patches.Cui_Endtest_Patch can route them.
        ...
        /// <summary>
        /// WipeSchedule uses command.wipe.schedule on CUI buttons. Under Harmony those never leave the client.
        /// Bridge through cui.endtest with WIPESCHEDULE marker.
        /// </summary>
        private static string RewriteHarmonyButtonCommands(string json)
        {
            if (string.IsNullOrEmpty(json) || json.IndexOf("command.wipe.schedule", StringComparison.Ordinal) < 0)
                return json;
            json = json.Replace("\"command\":\"command.wipe.schedule", "\"command\":\"cui.endtest WIPESCHEDULE");
            return json;
        }
