...
namespace AdminTime
{
    ...
    public class AdminTimeMod : IHarmonyModHooks
    {
        ...
        private static string[] ArgsAsStrings(ConsoleSystem.Arg arg) =>
            arg?.Args == null ? Array.Empty<string>() : arg.Args.Select(a => a.ToString()).ToArray();
        ...
        private void CmdMytime(ConsoleSystem.Arg arg)
        {
            ...
            RunMytime(player, ArgsAsStrings(arg));
        }
        ...
        private void CmdMyweather(ConsoleSystem.Arg arg)
        {
            var player = arg.Connection?.player as BasePlayer;
            if (player == null || player.IsNpc) return;
            RunMyweather(player, ArgsAsStrings(arg));
        }
        ...
        {
            var player = arg.Connection?.player as BasePlayer;
            RunStorm(player, ArgsAsStrings(arg));
        }
