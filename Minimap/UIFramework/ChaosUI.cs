...
public class ChaosUI
{
	...
	public static void Show(BasePlayer player, BaseContainer baseContainer, bool recycle = true)
	{
		...
		{
			UnityEngine.Debug.LogWarning("[Minimap] ChaosUI.Show skipped: CommunityEntity.ServerInstance is null");
			if (recycle && baseContainer != null)
				UIContainerPool.Free(ref baseContainer);
			return;
		}
		...
		// Clients only forward ConsoleGen commands; bridge minimap.callback through cui.endtest MINIMAP.
		arg = arg.Replace("\"command\":\"minimap.callback ", "\"command\":\"cui.endtest MINIMAP minimap.callback ");
		arg = arg.Replace("\"command\": \"minimap.callback ", "\"command\": \"cui.endtest MINIMAP minimap.callback ");
