 ...
 * Local DTOs so Minimap does not reference PermissionsHarmony types.
 */
using System;
...
namespace MinimapHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[Minimap] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[Minimap] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[Minimap] Permissions mod not loaded — permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[Minimap] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[Minimap] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[Minimap] Permissions bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[Minimap] Permissions Service bind: " + ex.Message);
            }
        }
