...
namespace Game.Rust.Cui
{
    ...
    public static class CuiHelper
    {
        ...
        public static bool AddUi(BasePlayer player, string json)
        {
            if (player?.net != null)
            {
                // Clients only forward ConsoleGen commands. Rewrite compat-style UI commands
                // to cui.endtest so ServerPanelHarmony.Patches.Cui_Endtest_Patch can route them.
        ...
        /// <summary>
        /// ServerPanel and ServerPanelPopUps put custom console commands on CUI buttons; under Harmony those
        /// never leave the client. Bridge them through cui.endtest markers. Plugin pages embed containers built
        /// by other Harmony mods (Shop, Kits, WipeSchedule), so their commands are rewritten to the markers those
        /// mods already listen for. Longer command names must be rewritten before their prefixes.
        /// </summary>
        private static readonly (string command, string marker)[] CommandMarkers =
        {
            // ServerPanel / PopUps (this mod)
            ...
            // Embedded plugin pages owned by other Harmony mods
        ...
        private static string RewriteHarmonyButtonCommands(string json)
        {
            ...
            for (int i = 0; i < CommandMarkers.Length; i++)
            {
                var (command, marker) = CommandMarkers[i];
