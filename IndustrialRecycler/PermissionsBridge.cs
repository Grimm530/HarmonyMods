 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to IndustrialRecyclerHarmony,
 * log tag changed to [IndustrialRecycler].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace IndustrialRecyclerHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[IndustrialRecycler] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[IndustrialRecycler] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[IndustrialRecycler] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[IndustrialRecycler] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] Permissions Service bind: " + ex.Message);
            }
        }
