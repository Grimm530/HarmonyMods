...
namespace IndustrialRecyclerHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("IndustrialRecycler_Runner");
    ...
    public class IndustrialRecyclerMod : IHarmonyModHooks
    {
        ...
        public const string AppDomainApiKey = "IndustrialRecycler_ApiType";
        public const string CuiMarker = "INDUSTRIALRECYCLER";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[IndustrialRecycler] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(IndustrialRecyclerMod)); }
            catch { }
            ...
            Debug.Log("[IndustrialRecycler] Harmony mod loaded. Config: HarmonyConfig/IndustrialRecycler.json");
        }
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] Permissions ready: " + ex.Message);
            }
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[IndustrialRecycler] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            catch (Exception ex) { Debug.LogWarning("[IndustrialRecycler] ResolvePluginReferences: " + ex.Message); }
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[IndustrialRecycler] Init: " + ex.Message); }
            try { plugin.CallLoaded(); }
            catch (Exception ex) { Debug.LogWarning("[IndustrialRecycler] Loaded: " + ex.Message); }
            try { plugin.CallOnServerInitialized(); }
            catch (Exception ex) { Debug.LogError("[IndustrialRecycler] OnServerInitialized: " + ex); }
        ...
        private static void InvokeChatMethod(OxidePlugin plugin, string methodName, BasePlayer player, string command, string[] args, bool attributedOnly = false)
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[IndustrialRecycler] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            try
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] cui.endtest INDUSTRIALRECYCLER: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[IndustrialRecycler] " + methodName + ": " + (ex.InnerException ?? ex).Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[IndustrialRecycler] cmd " + captured + ": " + ex.Message); }
                }
            };
