 ...
 * Adapted from AdminMenu/PermissionsBridge.cs — namespace changed to PlaytimeTrackerHarmony,
 * log tag changed to [PlaytimeTracker].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace PlaytimeTrackerHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[PlaytimeTracker] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[PlaytimeTracker] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[PlaytimeTracker] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[PlaytimeTracker] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[PlaytimeTracker] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[PlaytimeTracker] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[PlaytimeTracker] Permissions Service bind: " + ex.Message);
            }
        }
