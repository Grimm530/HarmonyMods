...
namespace Oxide.Plugins
{
    ...
    internal partial class SignArtist : RustPlugin
    {
        ...
        public class SignArtistConfig
        {
			...
			[JsonProperty(PropertyName = "Max in-memory image cache entries")]
			public int MaxImageCacheEntries { get; set; }
		...
		private class ImageDownloader : MonoBehaviour
        {
            ...
            private SignArtist signArtist => SignArtist.Instance;
			private readonly Queue<DownloadRequest> downloadQueue = new Queue<DownloadRequest>();
			// Coalescing: multiple requests for the same URL share a single network download
			private readonly Dictionary<string, List<DownloadRequest>> pendingByUrl = new Dictionary<string, List<DownloadRequest>>();
			private readonly HashSet<string> activeUrls = new HashSet<string>();
			// Simple LRU cache for downloaded raw bytes (pre-resize)
			private readonly Dictionary<string, byte[]> urlBytesCache = new Dictionary<string, byte[]>();
			...
			public void QueueDownload(string url, BasePlayer player, IPaintableEntity sign, uint textureIndex, bool raw, bool hor = false)
            {
				...
				byte[] cached;
				if (TryGetFromCache(url, out cached))
				{
					StartCoroutine(ProcessFromCache(url, new DownloadRequest(url, player, sign, raw, hor, textureIndex), cached));
					return;
				}

				// Coalesce by URL
				List<DownloadRequest> list;
				if (!pendingByUrl.TryGetValue(url, out list))
				{
					list = new List<DownloadRequest>();
					pendingByUrl[url] = list;
				}
				list.Add(new DownloadRequest(url, player, sign, raw, hor, textureIndex));

				// Only enqueue a single representative item per URL
				if (list.Count == 1)
				{
					downloadQueue.Enqueue(list[0]);
				}
            ...
            {
				// Mark URL active (if already active, another producer picked it up earlier)
				activeUrls.Add(request.Url);
                ...
                if (signArtist.Settings.HeaderCheck)
                {
                    ...
                    if (head.result != UnityWebRequest.Result.Success)
                    {
                        ...
                        {
                            // Cleanup URL state before exiting
                            pendingByUrl.Remove(request.Url);
				...
				// Cache original bytes (pre-resize) for reuse
				PutInCache(request.Url, data);

				// Collect all coalesced requests for this URL
				List<DownloadRequest> grouped;
				if (!pendingByUrl.TryGetValue(request.Url, out grouped) || grouped == null || grouped.Count == 0)
				{
					// Fallback to the original single request
					grouped = new List<DownloadRequest> { request };
				}

				// Process each request with shared bytes
				for (int i = 0; i < grouped.Count; i++)
				{
					try
					{
						ProcessRequestWithBytes(data, grouped[i]);
					}
					catch (Exception ex)
					{
						signArtist.PrintWarning($"Failed processing coalesced request: {ex.Message}");
					}
				}
			...
			private IEnumerator ProcessFromCache(string url, DownloadRequest request, byte[] data)
			{
				yield return null; // keep coroutine semantics
				try
				{
					ProcessRequestWithBytes(data, request);
				}
				catch (Exception ex)
				{
					signArtist.PrintWarning($"Failed processing cached request: {ex.Message}");
				}
			}

			private void ProcessRequestWithBytes(byte[] originalBytes, DownloadRequest request)
			{
				// Convert/normalize bytes if needed
				byte[] imageBytes = request.Raw ? originalBytes : GetImageBytes(originalBytes);
			...
			private bool TryGetFromCache(string url, out byte[] bytes)
			{
				if (urlBytesCache.TryGetValue(url, out bytes))
				{
					// touch LRU
					var node = lruOrder.Find(url);
					if (node != null)
					{
			...
			private void PutInCache(string url, byte[] bytes)
			{
				if (bytes == null) return;
				urlBytesCache[url] = bytes;
				lruOrder.AddFirst(url);
				// Evict if over capacity
				while (urlBytesCache.Count > Math.Max(0, signArtist.Settings.MaxImageCacheEntries))
				{
					var last = lruOrder.Last;
					if (last == null) break;
					urlBytesCache.Remove(last.Value);
					lruOrder.RemoveLast();
				}
			}
        ...
        private void Init()
        {
            ...
            ImageSizePerAsset = new Dictionary<string, ImageSize>
            {
                ...
                // Artist DLC ornate gold frames (assets/prefabs/misc/artist_dlc/ornate frame/goldframe.*)
                // Deployed frames use goldframe.<variant>; dropped item world models use goldframe.<variant>.item
                ["goldframe.small"] = new ImageSize(205, 256),
                ["goldframe.small.item"] = new ImageSize(205, 256),
                ["goldframe.medium"] = new ImageSize(256, 192),
                ["goldframe.medium.item"] = new ImageSize(256, 192),
                ["goldframe.large"] = new ImageSize(512, 256),
                ["goldframe.large.item"] = new ImageSize(512, 256),
                ["goldframe.standing"] = new ImageSize(128, 512),
                ["goldframe.standing.item"] = new ImageSize(128, 512),
                ["goldframe.xl"] = new ImageSize(512, 512),
                ["goldframe.xl.item"] = new ImageSize(512, 512),
                ["goldframe.xxl"] = new ImageSize(1024, 512),
                ["goldframe.xxl.item"] = new ImageSize(1024, 512),
        ...
        private void RestoreCommand(IPlayer iplayer, string command, string[] args)
        {
            ...
            var allSigns = BaseEntity.serverEntities.OfType<Signage>().ToArray();
