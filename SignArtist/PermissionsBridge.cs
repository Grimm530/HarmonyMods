 ...
 * Adapted from SignArtist/PermissionsBridge.cs — namespace changed to SignArtistHarmony,
 * log tag changed to [SignArtist].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace SignArtistHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[SignArtist] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[SignArtist] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[SignArtist] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[SignArtist] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[SignArtist] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[SignArtist] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[SignArtist] Permissions Service bind: " + ex.Message);
            }
        }
