 ...
 * Adapted from CombatClasses/PermissionsBridge.cs — namespace changed to VirtualItemsHarmony,
 * log tag changed to [VirtualItems].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace VirtualItemsHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[VirtualItems] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[VirtualItems] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[VirtualItems] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[VirtualItems] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[VirtualItems] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] Permissions Service bind: " + ex.Message);
            }
        }
