...
using UnityEngine;
using GrimmCuiHarmony;
using HarmonyPlugin = Harmony.Plugins.VirtualItems;
using GrimmCuiHarmony;
...
namespace VirtualItemsHarmony
{
    public class ModRunner : MonoBehaviour
    {
        ...
        public static void Ensure()
        {
            ...
            _go = new GameObject("VirtualItems_Runner");
    ...
    public class VirtualItemsMod : IHarmonyModHooks
    {
        public static VirtualItemsMod Instance { get; private set; }
        ...
        public const string AppDomainApiKey = "VirtualItems_ApiType";
        public const string CuiMarker = "VIRTUALITEMS";
        ...
        public void OnLoaded(OnHarmonyModLoadedArgs args)
        {
            ...
            {
                Debug.LogError("[VirtualItems] Failed to construct/config plugin: " + ex);
                return;
            }

            try { AppDomain.CurrentDomain.SetData(AppDomainApiKey, typeof(VirtualItemsMod)); }
            catch { }

            RegisterAttributedChatCommands();
            ChatSayBridge.Register("VirtualItems", OnChatCommand);
            ...
            Debug.Log("[VirtualItems] Harmony mod loaded. Config: HarmonyConfig/VirtualItems.json");
        }
        ...
        private void OnPermissionsReady()
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] Permissions ready: " + ex.Message);
            }
        }
        ...
        public void OnUnloaded(OnHarmonyModUnloadedArgs args)
        {
            ...
            Instance = null;
            Debug.Log("[VirtualItems] Harmony mod unloaded.");
        }
        ...
        private IEnumerator WaitForServerThenInit(int attempt = 0)
        {
            ...
            while (ItemManager.itemList == null || ItemManager.itemList.Count == 0)
            {
                if (attempt > 120)
                {
                    Debug.LogWarning("[VirtualItems] ItemManager timeout; proceeding.");
                    break;
                }
            ...
            catch (Exception ex) { Debug.LogWarning("[VirtualItems] ResolvePluginReferences: " + ex.Message); }
            try { plugin.CallInit(); }
            catch (Exception ex) { Debug.LogWarning("[VirtualItems] Init: " + ex.Message); }
            try { plugin.CallLoaded(); }
            catch (Exception ex) { Debug.LogWarning("[VirtualItems] Loaded: " + ex.Message); }
            try { plugin.CallOnServerInitialized(); }
            catch (Exception ex) { Debug.LogError("[VirtualItems] OnServerInitialized: " + ex); }
            RefreshDynamicCommands();
            _initCoroutine = null;
            Debug.Log("[VirtualItems] Server initialized.");
        }
        ...
        private static void InvokeChatMethod(HarmonyPlugin plugin, string methodName, BasePlayer player, string command, string[] args, bool attributedOnly = false)
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] InvokeChatMethod " + methodName + ": " + ex.Message);
            }
        }
        ...
        private void RegisterAttributedConsoleCommands()
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] RegisterAttributedConsoleCommands: " + ex.Message);
            }
        }
        ...
        private void RefreshDynamicCommands()
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] RefreshDynamicCommands: " + ex.Message);
            }
        }
        ...
        private void RegisterChatAliasConsole(string name)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                Call = a =>
                {
                    ...
                    {
                        Debug.LogWarning("[VirtualItems] chat alias " + localName + ": " + ex.Message);
                    }
                }
            };
            try
            ...
            {
                Debug.LogWarning("[VirtualItems] RegisterChatAliasConsole(" + localName + "): " + ex.Message);
            }
        }
        ...
        public void HandleCuiEndtest(ConsoleSystem.Arg args, Array a)
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] cui.endtest VIRTUALITEMS: " + ex.Message);
            }
        }
        ...
        private void InvokeConsoleMethod(string methodName, ConsoleSystem.Arg arg)
        {
            ...
            {
                Debug.LogWarning("[VirtualItems] " + methodName + ": " + (ex.InnerException ?? ex).Message);
            }
        }
        ...
        private void RegisterConsole(string name, Action<ConsoleSystem.Arg> handler, bool serverAdmin = false)
        {
            ...
            var cmd = new ConsoleSystem.Command
            {
                ...
                {
                    try { handler(a); }
                    catch (Exception ex) { Debug.LogWarning("[VirtualItems] cmd " + captured + ": " + ex.Message); }
                }
            };
