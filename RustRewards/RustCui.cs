...
namespace Oxide.Game.Rust.Cui
{
    ...
    public static class CuiHelper
    {
        ...
        /// <summary>
        /// RustRewards uses custom console commands on CUI buttons. Under Harmony those never leave the client.
        /// Bridge through cui.endtest with an RR marker so Cui_Endtest_Patch can rebuild and dispatch.
        /// Longer command names are rewritten first to avoid partial matches.
        /// </summary>
        private static string RewriteHarmonyButtonCommands(string json)
        {
            ...
            {
                string needle = "\"command\":\"" + cmd;
                if (json.IndexOf(needle, StringComparison.Ordinal) >= 0)
                {
                    any = true;
                    break;
                }
            ...
            foreach (var cmd in cmds)
            {
                json = json.Replace("\"command\":\"" + cmd, "\"command\":\"cui.endtest RR " + cmd);
            }
            return json;
        }
