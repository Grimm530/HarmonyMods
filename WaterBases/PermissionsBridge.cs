 ...
 * Adapted from AdminMenu/PermissionsBridge.cs — namespace changed to WaterBasesHarmony,
 * log tag changed to [WaterBases].  Same reflection API to PermissionsHarmony.PermissionsMod.
 */
using System;
...
namespace WaterBasesHarmony
{
    ...
    public static class PermissionsBridge
    {
        ...
        public static void RegisterReadyCallback(Action callback)
        {
            ...
            {
                Debug.LogWarning("[WaterBases] RegisterReadyCallback: " + ex.Message);
            }
            ...
            {
                Debug.LogWarning("[WaterBases] RegisterReadyCallback fallback: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                if (_permType == null || liveInstance == null)
                {
                    if (!_resolveAttempted)
                    {
                        ...
                        Debug.LogWarning("[WaterBases] Permissions mod not loaded - permission checks will fail until 0Permissions.dll is loaded.");
                    }
                    return;
                }
                ...
                {
                    _loggedLink = true;
                    Debug.Log("[WaterBases] Linked to Permissions Harmony mod.");
                }
                else
                    Debug.Log($"[WaterBases] Re-linked to Permissions Harmony mod (gen={gen}).");
            }
            catch (Exception ex)
            {
                ClearBind();
                Debug.LogWarning("[WaterBases] PermissionsBridge bind failed: " + ex.Message);
            }
        }
        ...
        private static void BindService()
        {
            ...
            {
                Debug.LogWarning("[WaterBases] Permissions Service bind: " + ex.Message);
            }
        }
