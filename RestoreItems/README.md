...
Harmony port of **RestoreItems 2.1.6** (Grimm530). Captures player inventory on death and restores via `/getstuff` (configurable), with Economics cost, RaidableBases zone detection, and dungeon API parity.
...
| Oxide | Harmony |
|-------|---------|
| `legacy/config/RestoreItems.json` | `HarmonyConfig/RestoreItems.json` |
...
After editing `.cursor/Harmony.Plugins.Cant-Use/RestoreItems.cs`:

```powershell
.\convert-from-legacy.ps1
.\build.ps1
```
